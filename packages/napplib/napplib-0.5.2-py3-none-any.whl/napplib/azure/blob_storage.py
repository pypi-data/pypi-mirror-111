...
class BlobStorage:
    ...
    @classmethod
    def get_latest_blob_by_name(self, account_name='', account_key='', path='', project='', blob_name='', store_name='', custom_encoding='utf8'):
        # Azure configs
        block_blob_service = BlockBlobService(
            account_name=account_name, 
            account_key=account_key
    ...
    def upload_blob(self, account_name='', account_key='', path='', project='', store_name='', output_file='', prefix=None, first_charge=False, is_sale=False, is_stock=False, root_folder=False):
        ...
        if store_name:
            project = f"{project}/{store_name}"

        if not root_folder:
            target = f'{project}/{year}/{month}/{day}/{filename}'
        else:
            target = f'{project}/{filename}'
