...
from aymara import lima_models
...
class DockerClient:
    ...
    def install_model(self, language: str) -> None:
        lima_models.install_language(language)
        #global debug
...
class HostedClient:
    ...
    def __init__(self, host="localhost", port="8080"):
        ...
        self.port = port
        client = self
        if host == "localhost" and not ps():
            run_limaserver(self.port)
    ...
    def install_model(self, language: str) -> None:
        ...
        if self.host == "localhost":
            if lima_models.install_language(language):
                if ps():
                    restart_limaserver(self.port)
                else:
                    run_limaserver(self.port)
        else:
            raise Exception("Cannot install models on non localhost")

    def list_models(self) -> None:
        lima_models.info()

    def list_installed_models(self) -> None:
        lima_models.list_installed_models()
...
class Client:
    ...
    def analyzeText(self,
        ...
        pipe : str, optional
            The pipeline to use to analyze the text (default is 'deepud')
        ...
        if lang.startswith("old-"):
            lang = lang[len("old-"):]
        elif not lang.startswith("ud-"):
            lang = f"ud-{lang}"
        url = f"http://{self.host}:{self.port}/?lang={lang}&pipeline={pipe}"
        if meta:
            url += f"&meta={meta}"
        global debug
        if debug:
...
def kill():
    ...
    name = "limaserver"
    # limaserver was started internally
    if limaserver:
        limaserver.kill()
        limaserver.wait(timeout=3)
        return True
    # if it exists, limaserver was started externally
    else:
        try:
...
def run_limaserver(port=None):
    global limaserver
    args = ["limaserver"]
    if port:
        args += ["--port", port]
    if not ps():
        limaserver = subprocess.Popen(args)
        time.sleep(10)
        return True
    else:
        print(f"limaserver is already running", file=sys.stderr)
        return False


def restart_limaserver(port=None):
    kill()
    return run_limaserver(port)
