...
# sys.path.append(os.environ['BOT_SOURCE_USR'])
# #from myBot import *
# from bot import *

if 'BOT_FOLDER_USR' in os.environ:
    
    botsFolder = os.environ['BOT_FOLDER_USR']
    
    sys.path.insert(0,botsFolder)
    from fbots import *
...
class Mate(object):
  ...
  @staticmethod
  def Mate(maleBot, femaleBot, args=None):

    
    active_species = maleBot.species
    active_env = maleBot.env
    botStr = eval(active_species)
    childBot = botStr(active_env, myspecies = active_species, myargs = args)
...
class SimpleTournamentRegenerate(RootTournament):
  ...
  def RankPopulation(self, output = 0 ):
    ...
    if len(self.rankings) < 4:
      n = len(self.rankings)
      print (f'NOT ENOUGH RANKINGS. 4 Required, got: {n}')
    else:
      n = len(self.rankings)
      print (f'ENOUGH RANKINGS. 4 Required , got: {n}')
  ...
  def RegeneratePopulation(self, args = None):
    ...
    childOne = Mate.Mate(maleParent, femaleParent, args)
    ...
    childTwo = Mate.Mate(maleParent, femaleParent, args)
