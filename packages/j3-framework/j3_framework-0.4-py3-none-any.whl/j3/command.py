...
from j3.config import J3
from j3.core import AbstractFastMSA, FastMSAError, FastMSAInitError
...
class FastMSACommand:
    def __init__(self):
        ...
        self.msa = cast(J3, J3.load_from_config(self.path))
    ...
    def info(self):
        """J3 앱 정보를 출력합니다."""
        dot = bold("-", YELLOW)
        self.banner(f"{bold('J3 Information')}", icon="💡")
...
class FastMSACommandParser:
    ...
    def __init__(self):
        ...
        self.parser = ArgumentParser(
            "msa",
            description=f"✨ {bold('J3 Framework')} : {fg('command line utility', CYAN_EX)}",
        )
