...
def run_script(client, bucketname:str, prefix_in:str, prefix_trunc:str, transformation_string:str, zf:zipfile.ZipFile, **kwargs:dict):
    """
    runs the specified transformation_string.  any function in the transform, annotated with @infinstor_inject_context, will be injected with input_dirs, output_dir and kwargs
    Note that the script can be run with input or without (in which case client, bucketname, prefix_in, prefix_trunc will be None)
    """
    ...
    temp_output_dir = tempfile.mkdtemp()
    # if transform has an input
    if client:
        # TODO: optimization: use AST to determine if the decorator @infinstor_inject_context is used in the transform.  Only if used, attempt to download all artifacts
        # identify all objects that need to be downloaded from s3 bucket
        ...
        # download all objects from s3 bucket to a local directory
    ...
    exec_with_infinstor_inject_context([temp_input_dir_root], temp_output_dir, compiledcode2)        
    #print('Globals=')
    #pprint.pprint(glb)
    #info("Execution of script complete. status_str=" + status_str)

    # TODO: optimization: use AST to determine if the decorator @infinstor_inject_context is used in the transform.  Only if used, attempt to upload all artifacts
...
def log_all_artifacts_in_dir(prefix_in:str, prefix_trunc:str, temp_output_dir:str):
...
def run_transform_string_inline_multi_input(clients:list, bucketnames:list, prefix_ins:list, prefix_truncs:list,
                    transform_string:str, zf:zipfile.ZipFile, **kwargs):
...
def run_transform_string_inline(client, bucketname, prefix_in, prefix_trunc,
        transform_string, zf:zipfile.ZipFile, **kwargs):
    if (look_for_transform(transform_string, TRANSFORM_RAW_PD)):
        ...
        run_script(client, bucketname, prefix_in, prefix_trunc, transform_string, zf, **kwargs)
