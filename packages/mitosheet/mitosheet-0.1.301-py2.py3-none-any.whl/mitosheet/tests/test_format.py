...
def test_sheet_json_displays_floats_correctly():
    ...
    assert sheet_json[0]['data'][0][0] == '1.0'
    assert sheet_json[0]['data'][1][0] == '2.0'
    assert sheet_json[0]['data'][2][0] == 'NaN'
...
def test_sheet_displays_dates_with_non_standard_dtype():
    ...
    sheet_json = json.loads(mito.mito_widget.sheet_json)
    print(sheet_json)
    assert sheet_json[0]['data'][0][0] == '2016-01-31 19:29:50'
    assert sheet_json[0]['data'][1][0] == '2016-01-31 19:29:50'
...
def test_sheet_json_holds_timed_deltas_non_standard_dtype():
    ...
    sheet_json = json.loads(mito.mito_widget.sheet_json)
    assert sheet_json[0]['data'][0][0] == '31 days 00:00:00'
    assert sheet_json[0]['data'][1][0] == '31 days 00:00:00'
    assert sheet_json[0]['data'][2][0] == '31 days 00:00:00'
    assert sheet_json[0]['data'][3][0] == '31 days 00:00:00'