...
from mitosheet.user.schemas import UJ_MITOSHEET_TELEMETRY, USER_JSON_DEFAULT
...
def check_user_json(
    ...
    """
    print(get_user_json_object())
    assert set(get_user_json_object().keys()) == set(USER_JSON_DEFAULT.keys())

    assert get_user_field(UJ_USER_JSON_VERSION) == 3
    ...
    assert get_user_field(UJ_MITOSHEET_TELEMETRY) == True
