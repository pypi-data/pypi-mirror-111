...
def saturate_filter_event(curr_step, event):
    ...
    event['has_non_empty_filter'] = has_non_empty_filter
...
def _reset_filter(wsc, sheet_index, column_header, ignore_last_step=False):
    ...
    2. Replay the steps after the filter step
    ...
    # If nothing to delete, we return early
    if deleted_step_idx is None:
        return

    # We delete the step before rerunning from that index, which makes sure
    # we don't enter an inifinite loop, doh
    wsc.steps.pop(deleted_step_idx)
    wsc.rerun_steps_from_index(index=deleted_step_idx)
...
FILTER_STEP = {
    ...
    'saturate': saturate_filter_event,
