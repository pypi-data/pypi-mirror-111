...
class DimLattice(DimGroup, LatticeBaseMixin):
    ...
    def generate_rect(
        ...
        # TODO: seed!
        return cls.generate(
            Rect(width=du*(nu-.5), height=dv*(nv-.5)) * unit, (du, 0) * unit, (0, -dv) * unit, dim_element, dim_center, predicate, explode, remove_outliers
        )
    ...
    def generate(
        ...
        elements = cls._generate_impl(
            boundary, u, v, center, element_gen, predicate, explode, remove_outliers, dim_vec_to_vec, vec_to_dim_vec, (0, 0)
        )
