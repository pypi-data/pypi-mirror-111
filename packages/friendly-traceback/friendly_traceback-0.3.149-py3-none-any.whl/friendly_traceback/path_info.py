...
class PathUtil:
    ...
    def shorten_path(self, path):  # pragma: no cover
        ...
        print("path = ", path, "ipykernel" in path)
        print("orig_path", orig_path, "ipykernel" in orig_path)

        if "ipykernel" in path:
            new_path = self.shorten_jupyter_kernel(orig_path)
            if new_path:
                return new_path
        else:
            print("ipykernel not in path")
    ...
    def shorten_jupyter_kernel(self, path):
        from .config import session
        from .source_cache import cache

        print("Entring shorten_jupyter_kernel", path)
