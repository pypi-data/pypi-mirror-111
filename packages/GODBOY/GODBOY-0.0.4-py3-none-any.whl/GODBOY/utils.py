...
from GODBOY import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, bot

from GODBOY.process import CancelProcess

from GODBOY.config import Config # Main Imports from here
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        mod.logger = logging.getLogger(shortname)
        # support for uniborg
        sys.modules["uniborg.util"] = GODBOY.utils
        mod.Config = Config
        mod.bot = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["userbot.mainfiles.events"] = GODBOY.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["userbot.plugins." + shortname] = mod
        LOGS.info("GODBOY imported " + shortname)
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/SleepersOP)"
