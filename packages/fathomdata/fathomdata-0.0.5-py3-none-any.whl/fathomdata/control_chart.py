...
def __plot_passed_points(df, the_parameter_name, the_chart):
    ...
    passed_points_tooltip = __get_tooltip_template(the_parameter_name, False)
    passed_points_hover_tool = HoverTool(renderers=[passed_points_renderer],
                                         tooltips=passed_points_tooltip,
                                         formatters={'@batch_date': 'datetime',
                                                     '@deviations': CustomJSHover(code="return value !== 'NaN' ? 'Deviation: ' + value : '';")})
...
def __plot_failed_points(df, the_parameter_name, the_chart):
    ...
    failed_points_tooltips = __get_tooltip_template(the_parameter_name, True)
    failed_points_hover_tool = HoverTool(renderers=[failed_points_renderer],
                                         tooltips=failed_points_tooltips,
                                         formatters={'@batch_date': 'datetime',
                                                     '@deviations': CustomJSHover(code="return value !== 'NaN' ? 'Deviation: ' + value : ''")})
    the_chart.add_tools(failed_points_hover_tool)


def __get_tooltip_template(the_parameter_name, is_failure):
    ...
    tooltip += "</div>"
    tooltip = tooltip.replace('|value_style|', "color: tomato; font-weight: bold;" if is_failure else "")
