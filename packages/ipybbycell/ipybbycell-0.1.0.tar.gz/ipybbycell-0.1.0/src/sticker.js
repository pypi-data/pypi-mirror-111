...
const sticker = {
  ...
  makeElementHtml: (tag, attr, innerHtml) => {
    ...
    if (tag === 'svg') {
      elementHtml +=
        'xmlns="http://www.w3.org/2000/svg" version="1.1" class="bbySvg" ';
    }
    let attrHtml = '';
    if (attr !== undefined) {
  ...
  renderSvg: (svgChildren, x, y, width, height, viewBox, arrowHeadRecord) => {
    ...
    let svgGenerator = $('#bbySvgGenerator');
    if (svgGenerator.length === 0) {
      $('body').append($('<div id="bbySvgGenerator"></div>'));
      svgGenerator = $('#bbySvgGenerator');
    }
  ...
  makeSimplePath: (opts) => {
    ...
    const strokeWidth = opts.strokeWidth === undefined ? 3 : opts.strokeWidth;
    const outerClass =
      opts.outerClass === undefined ? 'bby-sticker-inner' : opts.outerClass;
  ...
  makeRightSideMarker: (opts) => {
    return sticker.makeSimplePath(
      $.extend({}, true, opts, {
        ...
        fill: opts.color,
        outerClass: 'bby-right-side-marker',
        buffer: 0,
      })
    );
  },
