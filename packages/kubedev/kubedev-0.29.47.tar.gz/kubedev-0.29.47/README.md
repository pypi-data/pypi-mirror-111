...
```jsonc
{
    ...
    "deployments": {
        "mydeploy": { # An App `mydeploy' of type deployment
            ...
            # Defines the system test for this app. System tests must be defined in the directory ./systemTest/<app-name>/ and must include a Dockerfile:
            "systemTest": {
                "variables": {
...
**Note that** local kubedev invocations behave differently from its invocations
being inside a CI pipeline. When running `kubedev system-test <app>` locally,
all container images necessary for a given system-test are built. When running
in CI, only the system-test images themselves are built. Deployments &
CronJob images are expected to be pullable. Hence, in CI `system-tests` may never
preceed their corresponding `build-push` stage.
