...
class Invoice(object):
    ...
    def create(self,data):
        ...
        response = requests.post(url=url,json=data, headers=self.client.headers("application/json"))
        response.raise_for_status()
        if "ApiErrorCode" in response.headers:
                #print response.headers
                raise Exception("Error invite : %s - %s" % (response.headers["ApiErrorCode"],response.headers["ApiError"]))
