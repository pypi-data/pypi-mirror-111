  ...
  def _detection_loss(self, cls_outputs, box_outputs, labels, loss_vals):
    ...
    for level in levels:
      ...
      if self.config.data_format == 'channels_first':
        ...
        cls_targets_at_level = tf.reshape(cls_targets_at_level,
                                          [-1, width, height, self.config.num_classes * self.config.num_scales * len(self.config.aspect_ratios)])
      ...
      if class_loss_layer:
        ...
        if self.config.data_format == 'channels_first':
          ...
          cls_loss = tf.reshape(
              cls_loss, [-1, width, height, self.config.num_scales * len(self.config.aspect_ratios), self.config.num_classes])
        cls_loss *= tf.cast(
            tf.expand_dims(
                tf.not_equal(
                    labels['cls_targets_%d' % (level + self.config.min_level)],
