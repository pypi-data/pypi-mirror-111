...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        tensor, incr_state = self.self_attention(
            hidden_states=tensor, mask=decoder_mask, incr_state=incr_state, get_incr_state=get_incr_state)
...
class TransformerDecoder(Module):
    ...
    def process_decoder_mask(self, decoder_mask: torch.Tensor, decoder_length: int, dtype: torch.dtype) -> torch.Tensor:
        ...
        return ((~mask).to(dtype) * neg_inf(dtype == torch.bool)).unsqueeze(1).expand(
            -1, self.n_heads, decoder_length, -1)
    ...
    def forward(
        ...
        decoder_mask = self.process_decoder_mask(decoder_mask, decoder_length=decoder_length, dtype=tensor.dtype)
        encoder_mask = ((~encoder_mask).type_as(tensor) * neg_inf(tensor.dtype == torch.bool)).unsqueeze(1).unsqueeze(
            1).expand(-1, self.n_heads, decoder_length, -1)
        ...
        return tensor, torch.stack(new_incr_states, dim=2) if get_incr_state else None
