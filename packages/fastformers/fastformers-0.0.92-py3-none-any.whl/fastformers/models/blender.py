...
class Blender(torch.nn.Module):
    def __init__(
            ...
            n_encoder_layers: int, n_decoder_layers: int, num_labels: int, decode_step: int
    ):
        super().__init__()
    ...
    def from_pretrained(
            ...
            ffn_size: int, n_encoder_layers: int, n_decoder_layers: int, num_labels: int, decode_step: int = 1
    ):
        model = cls(
            ...
            num_labels=num_labels, decode_step=decode_step
        )
    ...
    def embedding_forward(self, tensor: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        return self.embeddings(tensor) + self.position_embeddings(positions)
    ...
    def decode(
        ...
        if incr_state is not None:
            decoder_ids = decoder_ids[:, -self.decode_step:]
            positions = positions[:, -self.decode_step:]
    ...
    def convert_old_state_dict(self, state_dict: dict):
        for p in 'weight', 'bias':
            ...
            state_dict[f'pre_decoder_layer.{p}'] = torch.cat(
                [state_dict.pop(f'decoder.layers.{ind}.encoder_attention.{name}_lin.{p}')
                 for ind in range(len(self.decoder.layers)) for name in 'kv'], dim=0)

            scale = math.sqrt(self.encoder.layers[0].attention.dim_per_head)

            for ind in range(len(self.encoder.layers)):
                state_dict[f'encoder.layers.{ind}.attention.q_lin.{p}'] /= scale
                state_dict[f'encoder.layers.{ind}.attention.pre_attention.{p}'] = torch.cat(
                    [state_dict.pop(f'encoder.layers.{ind}.attention.{name}_lin.{p}') for name in 'qkv'], dim=0)
            for ind in range(len(self.decoder.layers)):
                state_dict[f'decoder.layers.{ind}.self_attention.q_lin.{p}'] /= scale
                state_dict[f'decoder.layers.{ind}.self_attention.pre_attention.{p}'] = torch.cat(
                    [state_dict.pop(f'decoder.layers.{ind}.self_attention.{name}_lin.{p}') for name in 'qkv'], dim=0)
                state_dict[f'decoder.layers.{ind}.encoder_attention.q_lin.{p}'] /= scale
    ...
    def load_state_dict(self, state_dict: dict, strict=True):
        ...
        if 'encoder.pre_decoder_layer.weight' not in state_dict:
            self.convert_old_state_dict(state_dict)

        state_dict['embeddings.weight'] *= math.sqrt(self.embeddings.embedding_dim)
