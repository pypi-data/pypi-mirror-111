...
class BartModel(Module):
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor):
        ...
        encoder_outputs = encoder_outputs.view(
            encoder_outputs.shape[:2] + (self.decoder_layers, 2, self.encoder_heads, -1)).transpose(1, 4)
...
class BartForSequenceClassification(Module):
    ...
    def replace_encoder_keys(self, state_dict: dict):
        ...
        scale = math.sqrt(self.transformer.encoder.layers[0].attention.dim_per_head)

        for layer_ind in range(len(self.transformer.encoder.layers)):
            prefix = f'model.encoder.layers.{layer_ind}.'
            for w in ('weight', 'bias'):
                state_dict[f'{prefix}self_attn.q_proj.{w}'] /= scale
                state_dict[f'{prefix}attention.pre_attention.{w}'] = torch.cat([
    ...
    def replace_decoder_keys(self, state_dict: dict):
        ...
        scale = math.sqrt(self.transformer.decoder.layers[0].self_attention.dim_per_head)

        for layer_ind in range(len(self.transformer.decoder.layers)):
            prefix = f'model.decoder.layers.{layer_ind}.'
            for w in ('weight', 'bias'):
                state_dict[f'{prefix}self_attn.q_proj.{w}'] /= scale
                state_dict[f'{prefix}encoder_attn.q_proj.{w}'] /= scale
                state_dict[f'{prefix}self_attention.pre_attention.{w}'] = torch.cat([
                    state_dict.pop(f'{prefix}self_attn.{k}_proj.{w}') for k in ('q', 'k', 'v')
                ], dim=0)
    ...
    @classmethod
    def from_pretrained(cls, model_path: str) -> 'BartForSequenceClassification':
        model = cls(
            ...
            decoder_layers=12, decoder_heads=16, decoder_ffn_dim=4096, num_labels=3, eos_token_id=2
        )
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        return self.classification_head(tensor).softmax(dim=1)
