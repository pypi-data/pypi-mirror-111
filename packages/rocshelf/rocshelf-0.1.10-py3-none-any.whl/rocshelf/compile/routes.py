...
class CompileRoute(sync.process.OnProcessMixin):
    ...
    def processing_data_analyze(self, processingNode: template.nodes.ProcessingOutputNode):
        ...
        print(self.localizationName, self.routeKey, 1, chunks)
...
class CompileLocalization(sync.process.OnProcessMixin):
    ...
    compileController: controller.CompileLocalizationController

    def __init__(self, localizationName: _T.Optional[str] = None) -> None:
        self.processingRoutes = {}

        self.localizationName = localizationName
        self.compileController = controller.CompileLocalizationController(self.localizationName, GetRoute.list())
    ...
    def start(self):
        ...
        for routeKey in routes:
            inputQueue, outputQueue = self.compileController.queues(routeKey)
    ...
    def on_process(self):
        self.compileController.start_worker()
        self.start()
        self.compileController.stop_worker()
