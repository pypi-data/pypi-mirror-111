...
class IDDSInterface():
    ...
    @staticmethod
    def download_log(workload_id:int, request_id=None,
        ...
        print('INFO: Fetching iDDS log files for the task with taskID = {}'.format(workload_id))
        filename = client.download_logs(workload_id=workload_id, request_id=request_id, dest_dir='/tmp')
        if not filename:
            print('INFO: No iDDS log found for the task with taskID = {}'.format(workload_id))
        else:
            print('INFO: iDDS log files can be found at: {}'.format(filename))
        if original_proxy:
