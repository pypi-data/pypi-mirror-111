...
from warden_sdk.integrations.aws_lambda import AwsLambdaIntegration
...
class _User(object):
   ...
   def setup(self, request: Any) -> None:
      ...
      if options['debug'] and options['user_fid'] is None or options['user_scope'] is None:
         logger.warning('User cannot be setup. Missing parameters: user_fid or user_scope')
         return
      ...
      if options['debug']:
         ...
         # if flask_integration and aws_integration:
         if aws_integration:
   ...
   @scope.setter
   def scope(self, scope: Union[str, List[str]]):
