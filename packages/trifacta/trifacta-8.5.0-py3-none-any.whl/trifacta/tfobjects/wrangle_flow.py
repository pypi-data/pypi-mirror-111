...
class WrangleFlow:
    ...
    def run_job(self, pbar=None, execution='photon', recipe_name=None):
        ...
        if recipe_name is None:
            recipe_name = list(self.recipe_dict.keys())[0].name
    ...
    def bars_df_list(self, recipe_name=None):
        hist = self.profile(recipe_name=None)["profilerValidValueHistograms"]
    ...
    def _numeric_summary_df(self, col, name, recipe_name=None):
        ...
        row = {"column": name, "type": self.profile(recipe_name=None)["columnTypes"][name]}
    ...
    def summary(self, recipe_name=None):
        ...
        for col in hist:
            if "buckets" in hist[col]:
                data.append(self._numeric_summary_df(hist[col], col))
            elif "topk" in hist[col]:
                data.append(self._categorical_summary_df(hist[col], col))
            else:
                raise (
