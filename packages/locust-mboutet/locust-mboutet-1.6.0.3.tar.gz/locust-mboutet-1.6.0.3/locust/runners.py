    ...
    def user_classes_count(self) -> Dict[str, int]:
        ...
        for user_greenlet in self.user_greenlets:
            try:
                ...
                logger.debug(
                    "ERROR: While calculating number of running users, we encountered a user that didnt have proper args %s",
                    user_greenlet,
                )
                continue
    ...
    def shape_worker(self):
        ...
        while self.state == STATE_INIT or self.state == STATE_SPAWNING or self.state == STATE_RUNNING:
            ...
            if new_state is None:
                ...
                # TODO: This call is blocking, so if this call is not done when
                #       the shape is supposed to transition to another stage,
                #       this another stage will be delayed (and even skipped if previous stage ran for too long)
                # if user_count > self.user_count:
                #     expected_ramp_up_duration = (self.user_count - user_count) / spawn_rate
                # else:
                #     expected_ramp_up_duration = 0
                # buffer = 1
                # timeout = gevent.Timeout(expected_ramp_up_duration + buffer)
                # timeout.start()
                # try:
                #     gevent.spawn(self.start, user_count=user_count, spawn_rate=spawn_rate)
                #     self.shape_last_state = new_state
                #     gevent.sleep(expected_ramp_up_duration)
