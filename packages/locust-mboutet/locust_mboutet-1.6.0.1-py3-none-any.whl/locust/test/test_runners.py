...
class TestMasterWorkerRunners(LocustTestCase):
    ...
    # def test_distributed_shape_with_stop_timeout2(self):
    #     class TestUser1(User):
    #         weight = 1
    #
    #         @task
    ...
    #     ):
    #         stop_timeout = 5
    #         master_env = Environment(user_classes=user_classes, shape_class=TestShape(), stop_timeout=stop_timeout)
    #         master_env.shape_class.reset_time()
    #         master = master_env.create_master_runner("*", 0)
    #
    #         workers = []
    #         for i in range(10):
    #             worker_env = Environment(user_classes=user_classes)
    #             worker = worker_env.create_worker_runner("127.0.0.1", master.server.port)
    #             workers.append(worker)
    #
    #         # Give workers time to connect
    ...
    #
    #         self.assertEqual(STATE_INIT, master.state)
    #         self.assertEqual(10, len(master.clients.ready))
    #
    #         # Re-order `workers` so that it is sorted by `id`.
    #         # This is required because the dispatch is done
    #         # on the sorted workers.
    ...
    #         ts = time.time()
    #         while master.state != STATE_STOPPED:
    #             self.assertTrue(time.time() - ts <= 110, master.state)
    #             sleep(1)
    #
    #         master.stop()
