...
def plot_line(df0,colname,collabel,ylabeltxt,titletxt,footnote,datatag=False, \
    ...
    if power > 0:
        try:
            ...
            plt.plot(df.index, f(df.id),"r--", label=texttranslate("趋势线"),linewidth=1)
        except: 
...
def plot_line2_coaxial(df01,ticker1,colname1,label1, \
    ...
    #插值平滑
    df1=df_smooth_manual(df01,resample_freq=resample_freq)
    df2=df_smooth_manual(df02,resample_freq=resample_freq)
    ...
    if power > 0:
        try:
            ...
            f = np.poly1d(parameter)
            plt.plot(df1.index, f(df1.id),"r--", label=codetranslate(ticker1)+texttranslate("(趋势线)"),linewidth=1)
        except: pass
    ...
    if power > 0:
        try:
            ...
            f = np.poly1d(parameter)
            plt.plot(df2.index, f(df2.id),"r--", label=codetranslate(ticker2)+texttranslate("(趋势线)"),linewidth=1)
        except: pass
...
def plot_line2_twinx(df01,ticker1,colname1,label1, \
    ...
    if power > 0:
        ...
        parameter = np.polyfit(df1.id, df1[colname1], power)
        f = np.poly1d(parameter)
        ax.plot(df1.index, f(df1.id),"r--", label=codetranslate(ticker1)+texttranslate("(趋势线)"),linewidth=1)
    ...
    if power > 0:
        ...
        parameter = np.polyfit(df2.id, df2[colname2], power)
        f = np.poly1d(parameter)
        ax2.plot(df2.index, f(df2.id),"c--", label=codetranslate(ticker2)+texttranslate("(趋势线)"),linewidth=1)        
