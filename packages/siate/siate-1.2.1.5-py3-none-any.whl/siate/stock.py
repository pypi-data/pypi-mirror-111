...
from siate.common import *
from siate.translate import *
from siate.grafix import *
from siate.security_prices import *
#==============================================================================
...
if czxt in ['darwin']:
    ...
    mpfrc={'font.family': 'Arial Unicode MS'}
# 解决保存图像时'-'显示为方块的问题
...
def comp_1security_2measures(df,measure1,measure2,twinx=False):
    ...
    ylabeltxt=""
    titletxt=texttranslate("证券指标走势对比图：")+codetranslate(ticker)
    import datetime; today = datetime.date.today()
    footnote=texttranslate("数据来源：雅虎财经，")+str(today)
...
def comp_2securities_1measure(df1,df2,measure,twinx=False):
    ...
    titletxt=texttranslate("证券指标走势对比图：")+codetranslate(ticker1)+" vs "+codetranslate(ticker2)
    import datetime; today = datetime.date.today()
    footnote=texttranslate("数据来源：雅虎财经，")+str(today)
...
def candlestick(stkcd,fromdate,todate,volume=False,style='default',mav=0):
    ...
    if not (style in stylelist):
        print("  #Error(candlestick)，only support graphical styles of",stylelist)
        return
...
def candlestick_demo(stkcd,fromdate,todate,colorup='red',colordown='black',width=0.7):
    ...
    #绘制各个价格的折线图
    plt.plot(p.index,p['Open'],color='green',ls="--",label=texttranslate("开盘价"),marker='>',markersize=10,linewidth=2)
    plt.plot(p.index,p['High'],color='cyan',ls="-.",label=texttranslate("最高价"),marker='^',markersize=10,linewidth=2)
    plt.plot(p.index,p['Low'],color='k',ls=":",label=texttranslate("最低价"),marker='v',markersize=10,linewidth=2)
    plt.plot(p.index,p['Close'],color='blue',ls="-",label=texttranslate("收盘价"),marker='<',markersize=10,linewidth=2)
...
def stock_dividend(ticker,fromdate,todate):
    ...
    import datetime; today = datetime.date.today()
    print(texttranslate("数据来源: 雅虎财经,"),today)
...
def print_companyOfficers(sub_info):
    ...
    print(itemtxt)
    if len(alist)==0:
        print("  Error(print_companyOfficers): failed to get relevant info.")
