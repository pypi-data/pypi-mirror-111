...
from siate.financial_statements import *
...
def compare_tax(tickers,graph=True,axisamp=1.3):
    ...
    import siate.beta_adjustment as badj
    import pandas as pd
...
def get_PE(fsdf):
    ...
    import siate.security_prices as ssp
...
def compare_dupont(tickerlist,fsdate='latest',scale1 = 10,scale2 = 10,hatchlist=['.', 'o', '\\']):
    ...
    for t in tickerlist:
        try:
            ...
            print("  #Warning(compare_dupont): financial info not found for",t)
            continue
        if fsdate == 'latest': dpi=dpidf.tail(1)
        else: dpi=dpidf[dpidf['endDate']==fsdate]
        if len(dpi) == 0:
            print("  #Warning(compare_dupont): financial info not found for",t,'@',fsdate)
            continue
