...
class fluxSurfaces(SortedDict):
    ...
    def surfAvg(self, function=None):
        ...
        if 'P' in self['avg']:
            ...
            self['avg']['beta_t'] = (
                self.volume_integral(self['avg']['P'] / (Btvac ** 2 / 2.0 / 4.0 / np.pi / 1e-7)) / self['geo']['vol'][-1] * 100
            )
            i = self['CURRENT'] / 1e6
            a = self['geo']['a'][-1]
    ...
    def surface_integral(self, what):
        ...
        """
        return abs(integrate.cumtrapz(self['avg']['vp'] * self['avg']['1/R'] * what, self['geo']['psi'], initial=0)) / (2.0 * np.pi)
