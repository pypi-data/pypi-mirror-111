...
def parseOpenPose(session):
    ...
    headerLength = (
        bodyCols + 2 * handCols + faceCols
    )  # should be 411 for whatever version of openpose i was using on 11 Jan 2021
    numImgPoints = headerLength

    for bb in range(
        0, int(bodyCols / 3)
    ):  # loop through the number of body markers (i.e. #bodyCols/3)
    ...
    for hr in range(
        0, int(handCols / 3)
    ):  # loop through the number of handR markers (i.e. #handCols/3)
    ...
    for hl in range(
        0, int(handCols / 3)
    ):  # loop through the number of handL markers (i.e. #handCols/3)
    ...
    for ff in range(
        0, int(faceCols / 3)
    ):  # loop through the number of Face markers (i.e. #faceCols/3)
    ...
    assert len(dataFrameHeader) == headerLength, [
        "Header is the wrong length! Should be "
        + str(headerLength)
        + " but it is "
        + str(len(dataFrameHeader))
        + " Check version of OpenPose?"
    ]
    ...
    openPoseData_nCams_nFrames_nImgPts_XYC = np.ndarray(
        [numCams, numFrames, numMarkers, 3]
    )  # hardcoding for now because I am a bad person

    for thisCams_JsonFolderPath in track(
        session.openPose_jsonPathList,
        description="Parsing json's into a dataframe (per cam)",
    ):
        thisCamNum += 1
        # print('Parsing into a dataframe: ', thisCams_JsonFolderPath.name )
        jsonPaths = sorted(
            Path(thisCams_JsonFolderPath).glob("*.json")
        )  # glob is a "generator(?)" for paths to all the jason for THIS camara

        for (
            thisJsonPath
        ) in (
            jsonPaths
        ):  # loop throug all the json files and save their 'people' data to a dictionary (which will then be formatted into a pandas dataframe). NOTE - will be empty array if no hoomans visible in frame
            ...
            thisJsonData = thisJsonData[
                "people"
            ]  # #FEATURE_REQUEST -  at some point, we should check the openpose version (save it with the data somehow, verify everything is the same version, use different markernamlists for different versions, etc)
            ...
            if thisJsonData:  # if this json has data
                ...
                thisFrameRow = np.hstack(
                    (bodyData, handRData, handLData, faceData)
            ...
            assert thisFrameRow.size == headerLength, [
                ...
                + str(headerLength)
                + " but it is "
                + str(thisFrameRow.size)
                + " Check version of OpenPose?"
            ]

            if (
                frameNum < openPoseData_nCams_nFrames_nImgPts_XYC.shape[1]
            ):  # NOTE: THIS SHOULDN"T BE NECESSARY. CURRENT Vid Trim Methods can (apparently) produce vids with off-by-one numbers of frames
                openPoseData_nCams_nFrames_nImgPts_XYC[
                    thisCamNum, frameNum, :, :
                ] = np.reshape(
                    thisFrameRow, [137, 3]
                )  # hard coding b/c I'm a bad person
