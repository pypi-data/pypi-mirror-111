...
def TimeSync(timeStampData, numCamRange, camNames):
    ...
    masterTimelineEnd = np.floor(
        min(lastPoints)
    )  # find where the fastest camera ended, round down, use that as the end point
    ...
    n = 0
    # counter for going through camera names
    for x in numCamRange:
        ...
        n += 1
        currentCamTimeline = currentCam[
            currentCamStart:currentCamEnd
        ]  # grab the times from start to finish for each camera
        currentCamFrameInterval = np.mean(
            np.diff(currentCamTimeline)
        )  # calculate the interval between each frame and take the mean
        totalFrameIntvl.append(currentCamFrameInterval)  # add interval to list
        # print(camNames[x],currentCamStart,currentCamEnd)
    totalAverageIntvl = np.mean(
        totalFrameIntvl
    )  # find the total average interval across all cameras
    masterTimeline = np.arange(
        masterTimelineBegin, masterTimelineEnd, totalAverageIntvl
    )  # build a master timeline with the average interval
    ...
    delNum = []
    # stores number of deleted frames/camera
    bufNum = []
    # stores number of buffered frames/camera
    bufPercentList = []
    # stores percentage of deleted frames/camera
    delPercentList = []
    # stores percentage of buffered frames/camera

    count = 0  # Keeps track of what frame we're on (I think?)
    n = 0
    # I don't remember why I did this but I'm sure I'll figure it out later
    ...
    for y in numCamRange:
        ...
        for z in masterTimeline:  # for each point in the master timeline
            closestFrame = CloseNeighb(
                thisCam, z
            )  # find the closest frame in this camera to each point of the master timeline
            camFrames.append(closestFrame)  # add that frame to the list
            camTimes.append(
                thisCam[closestFrame]
            )  # find the time corresponding to this frame

        print(
            "starting detection:", camNames[n]
        )  # now to start finding deleted/buffered slides
        frameList = np.column_stack(
            (frameList, camFrames)
        )  # update our framelist with our new frames
        ...
        for i in range(0, len(camFrames) - 1):
            distanceBetweenFrames = (
                camFrames[i + 1] - camFrames[i]
            )  # find the distance between adjacent frames
