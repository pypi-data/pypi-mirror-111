...
class Actor:
    ...
    DELEGATED_ATTRIBUTES = [a for a in dir(rect.ZRect) if not a.startswith("_")] + ["collide_mask"]

    _anchor = _anchor_value = (0, 0)

    def __init__(self, image, pos=POS_TOPLEFT,_angle = 0,_size = 100,anchor=ANCHOR_CENTER, **kwargs):
    ...
    def __getattr__(self, attr):
        # print(attr)
        if attr in self.__class__.DELEGATED_ATTRIBUTES or attr == 'collide_mask':
    ...
    def Size(self,new_size):
        ...
        self._orig_surf = self._surf = pygame.transform.scale(self._surf,(int(self._w*new_size/100),int(self._h*new_size/100)))
        self._update_pos()
        self._size = new_size
    @property
    def pos(self):
    ...
    def image(self, image):
        ...
        int(self._w * self._size / 100), int(self._h * self._size / 100)))
        self._update_pos()
    ...
    def collide_mask(self,others):
        ...
        # return collide_mask(self,others)
    def move(self,steps):
