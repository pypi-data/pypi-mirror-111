...
class Evaluation:
    def __init__(self,code:str,scope:dict):
        ...
        def do_GET(self):
            ...
            if isinstance(code,list):
                ...
                for i,e in enumerate(code):
                    rp.fansi_print("CODE %i/%i: "%(i+1,len(code))+code,'green')
                evaluations=[Evaluation(x,scope) for x in code]
                response=[x.encode() for x in evaluations]
            else:
...
class Client:
    ...
    def evaluate(self,code:str='',**vars):
        ...
        assert not isinstance(result,bytes),'Client.evaluate: Bad response...please make sure the server and client are running on the same version of python and rp.'
        return result
