...
from ...constant import FOFTradeStatus, DividendCalcMethod, EMailParserType
            ...
            if row.event_type in (FOFTradeStatus.PURCHASE, FOFTradeStatus.SUBSCRIBE, FOFTradeStatus.DIVIDEND_VOLUME):
                ...
                for one in share_details:
                    ...
                    if one['acc_unit_value'] >= row.acc_unit_value:
                        ...
                        continue
                    carry = round(self._calc_carry(row.acc_unit_value, one['acc_unit_value'], fof_info.incentive_fee_type, fof_info.incentive_fee_str) * one['share'] / row.net_asset_value, 2)
