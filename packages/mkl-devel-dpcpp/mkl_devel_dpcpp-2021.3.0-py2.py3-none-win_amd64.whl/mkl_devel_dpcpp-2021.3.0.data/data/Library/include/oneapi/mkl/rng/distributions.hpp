...
public:
    ...
    param_type param() const {
        return param_type(dimen_, mean_, matrix_);
    }
...
public:
    ...
    struct param_type {
        param_type(std::int32_t s, std::int32_t m, std::int32_t l) : s_(s), m_(m), l_(l) {}
        std::int32_t s_;
        std::int32_t m_;
        std::int32_t l_;
    };
...
public:
    ...
    explicit multinomial(const param_type& pt) : ntrial_(pt.ntrial_), p_(pt.p_) {
        if (pt.ntrial_ < 0) {
            throw oneapi::mkl::invalid_argument("rng", "multinomial", "ntrial < 0");
        }
        if (pt.p_.size() < 1) {
...
public:
    ...
    explicit poisson_v(const param_type& pt) : lambda_(pt.lambda_) {
        if ((pt.lambda_.size() <= 0)) {
            throw oneapi::mkl::invalid_argument("rng", "poisson_v", "size of lambda < 0");
        }
    }
