...
def generate():
    """
    Generate template for VGS resource.
    """
    pass
...
def apply_service_account(ctx, organization, file):
    ...
    config_data = validate_yaml(file, "schemas/service-account-schema.yml")['data']
...
def generate_service_account(ctx, template):
    """
    Generate template for service account resource.
    """
    file_content = read_file('service_account_templates/{0}.yaml'.format(template))
    click.echo(file_content)


@cli.command()
@click.option('--idp', help='Log in with custom identity provider')
@click.pass_context
@configuration_option(section='login')
def login(ctx, idp: Optional[str]):
    """
    Login to VGS via browser.
    """
    auth.login(ctx, ctx.obj.env, idp)
