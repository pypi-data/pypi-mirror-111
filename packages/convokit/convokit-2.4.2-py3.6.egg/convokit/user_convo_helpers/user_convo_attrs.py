    ...
    def transform(self, corpus: Corpus):
        ...
        for user in corpus.iter_users():
            ...
            for convo_id, convo in user.meta['conversations'].items():
                if self.recompute or (corpus.get_user_convo_info(user.id, convo_id, self.output_field) is None):
                    utterance_attrs = [corpus.get_utterance(utt_id).meta[self.attr_name] 
                                       for utt_id in convo['utterance_ids']]
                    corpus.set_user_convo_info(user.id, convo_id, self.output_field, self.agg_fn(utterance_attrs))
        return corpus