...
# @Last Modified time: 2021-06-29 14:07:22
import serial
import time
		...
		if self.isOpenStatus():
			...
			if self.closeLockIsSuccess():
				...
				if self.isCloseStatus():
					...
					print("关锁成功,并复位")
					return {"lockStatus": "close","status": True}
				else:
					self.open_com.write("AT+SETUP_ROTATION_ANGLE:-80\r\n".encode())
					self.get_data(checkString="success")
					return {"lockStatus": "open","status": False}
			else:
				print("关锁失败")
				self.open_com.write("AT+SETUP_ROTATION_ANGLE:-80\r\n".encode())
				self.get_data(checkString="success")
				return {"lockStatus": "open","status": False, "msg": "下发关锁指令失败"}
		else:
			self.open_com.close()
			return {"lockStatus": "close","status": False}
...
if __name__ == '__main__':
	a = Worker('/dev/tty.usbmodem8D70118753531')
	a.isOpenStatus()
