...
def _validate_code(code):
    if "tecton.materialization_context" in code:
        raise Exception(
            "Cannot serialize `tecton.materialization_context()`. Please use `materialization_context()` along with `from tecton import materialization_context`."
        )
...
def _getsource(func):
    ...
    def recurse(name, obj, imports, modules, code_lines, seen_args, write_codelines):
        ...
        elif isinstance(obj, MaterializationContext):
            raise Exception(
                "tecton.MaterializationContext() is not a valid value here. Did you mean "
