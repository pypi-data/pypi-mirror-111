...
class RedshiftDSConfig(BaseBatchDataSource):
    """
    Configuration used to reference a Redshift table or query.

    The RedshiftDSConfig class is used to create a reference to a Snowflake table. You can also create a
    ...
    def __init__(
        ...
        :param table: The Redshift table for this Data source.
        :param raw_batch_translator: Python user defined function f(DataFrame) -> DataFrame that takes in raw
                                     PySpark data source DataFrame and translates it to the DF to be
                                     consumed by the Feature Package. See an example of
                                     raw_batch_translator in the `User Guide`_.
        :param query: A Redshift query for this Data source.
        ...
        :return: A Redshift class instance.

        .. _User Guide: https://docs.tecton.ai/overviews/data_sources.html
        """
    ...
    def _merge_batch_args(self, virtual_data_source_args: virtual_data_source_pb2.VirtualDataSourceArgs):
        virtual_data_source_args.redshift_ds_config.CopyFrom(self._args)
