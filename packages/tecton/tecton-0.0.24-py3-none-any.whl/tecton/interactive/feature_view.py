...
from tecton_spark.transformation import Transformation
...
class FeatureView(FeatureDefinition):
    ...
    @sdk_public_method
    def online_transform_dataframe(
        self, spine: DataFrame, spine_time_key: str = None, use_materialized_data: bool = True
    ) -> DataFrame:
        """
        Adds features from an OnDemandFeatureView to the spine.

        :param spine: Spark dataframe to compute the online features using.
        :param spine_time_key: (Optional) Name of the time column in spine. Needed if there are dependent FeatureViews.
        :param use_materialized_data: (Optional) Use materialized data if materialization is enabled

        :return: A spark dataframe containing the spine augmented with features.
        """
        if not self.is_online:
            raise errors.TRANSFORMATION_DATAFRAME_ONLINE_ONLY

        input_columns = [f"`{c.name}`" for c in spine.schema]

        # augment the spine with dependent feature values
        fvs = _find_dependent_feature_set_items(self._proto.pipeline.root, visited_inputs={})
        if len(fvs) > 0:
            from tecton.interactive.feature_set_config import FeatureSetConfig

            fsc = FeatureSetConfig()
            fsc._definitions_and_configs = fvs
            spine = fsc.get_feature_dataframe(
                spine=spine, timestamp_key=spine_time_key, use_materialized_data=use_materialized_data
            ).to_spark()
        ...
        # only project out the input spine columns + feature columns.
        return df.select(*input_columns, *[field.name for field in self._materialization_schema.to_spark()])
    ...
    def run(self, **mock_inputs: pandas.DataFrame):
        ...
        :param mock_inputs: Dictionary with expected same keys as the FeatureView's inputs parameter. Each input name maps to a Pandas DataFrame that should be evaluated for that node in the pipeline.
        """
        ...
        if self.is_online:
            ...
            return pipeline_to_dataframe(
                spark,
                self._proto.pipeline,
                False,
                [],
                self._proto.enrichments.transformations,
                None,
                None,
                mock_inputs,
            )
    ...
    def materialization_status(self, verbose=False, limit=1000, sort_columns=None, errors_only=False):
        """
        Displays materialization information for the FeatureView, which may include past and scheduled jobs,
        and job failures.

        This method returns different information depending on the type of FeatureView. It is not currently
        supported by class ``FeatureTable`` or ``OnDemandFeatureView``, as neither is materialized.
    ...
    def get_feature_dataframe(
        ...
        """
        Returns a Tecton DataFrame that contains the output Feature Transformation of the Feature View.
        ...
        :param spine_time_key: (Optional) Name of the time column in spine.
            If unspecified, will default to the name of the timestamp key of the FeatureView.
        :param use_materialized_data: (Optional) Use materialized data if materialization is enabled
        ...
        if timestamp_key is None and not self.is_online:
            timestamp_key = self.timestamp_key

        fsc = FeatureSetConfig()
        fsc._add(FeaturePackageOrView.from_fv(self._proto))

        return Dataset._maybe_create(
            df=tc.execute(
    ...
    def batch_materialization_schedule(self) -> Optional[str]:
        """
        This represents the existing parameter ``schedule_interval``.
        """
        if not self._proto.HasField("materialization_params"):
            return None
        return str(pendulum.from_timestamp(self._proto.materialization_params.schedule_interval.ToSeconds()))
    ...
    def get_features(
        ...
        from tecton.interactive import feature_retrieval

        fpov = FeaturePackageOrView.of(self._proto)

        return feature_retrieval.get_features(fpov, entities, start_time, end_time, from_source)
    ...
    def get_feature_vector(
        ...
        """
        if not self._proto.materialization_params.writes_to_online_store and not self.is_online:
...
@sdk_public_method
def get_feature_view(fv_reference: str) -> FeatureView:
    """
    Fetch an existing :class:`FeatureView` by name.

    :param package_reference: Either:
        - A name.
        - A hexadecimal feature view ID.
    ...
    return FeatureView(response.feature_view)


def _find_dependent_feature_set_items(
    node: PipelineNode, visited_inputs: Dict[str, bool]
    ...
    if node.HasField("feature_view_node"):
        ...
        from tecton import get_feature_view
        from tecton_spark.feature_package_view import FVBackedFeaturePackageOrView

        fv = get_feature_view(IdHelper.to_string(node.feature_view_node.feature_view_id))

        fpov = FVBackedFeaturePackageOrView(fv._proto)
        cfg = FeatureDefinitionAndJoinConfig(
            definition=fpov,
            name=fv.name,
            join_keys=[(x, x) for x in fv._proto.join_keys],  # TODO: this assumes no join_key_overrides
            namespace=f"_udf_internal_{node.feature_view_node.input_name}",
        ...
        for child in node.transformation_node.inputs:
            ret = ret + _find_dependent_feature_set_items(child.node, visited_inputs)
        return ret
    return []
