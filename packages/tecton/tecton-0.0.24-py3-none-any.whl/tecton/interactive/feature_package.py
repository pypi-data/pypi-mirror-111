...
class FeaturePackage(Fco):
    ...
    def _validate_and_return_use_materialized_data(self, use_materialized_data):
        ...
        if use_materialized_data and not self._writes_to_offline_feature_store:
            ...
            if self._feature_package.fco_metadata.workspace == "":
...
@sdk_public_method
def get_feature_package(package_reference: str) -> FeaturePackage:
    """
    Fetch an existing :class:`FeaturePackage` by name.

    :param package_reference: Either:
        - A name.
        - A hexadecimal feature package ID.
    :returns: Either :class:`TemporalFeaturePackage` or :class:`TemporalAggregateFeaturePackage` or :class:`OnlineFeaturePackage`
    """
