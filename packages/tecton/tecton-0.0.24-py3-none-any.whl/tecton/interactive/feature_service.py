...
def materializable_workspace(workspace: str) -> bool:
    # FIXME: this is a placeholder implementation. We'll need a real one once we "officially"
    # support materialization in non-prod workspaces
    return workspace == "" or workspace.startswith("integration_testing")
...
class FeatureService(Fco):
    ...
    FeatureServices are used to serve feature values from :class:`FeaturePackage`. Users can use FeatureServices
    to make offline requests (e.g. to fetch a training dataset) using the :py:meth:`get_feature_dataframe`
    method, and online requests (e.g. for online serving) using the :py:meth:`get_feature_vector`
    method. A FeatureService consists of a set of FeaturePackages, plus configuration options.
    """
    ...
    def features(self) -> List[str]:
        """
        Returns the features generated by the enclosed feature packages.
        """
        return self.feature_set_config.features

    @sdk_public_method
    def query_features(self, join_keys: Dict[str, Union[int, str, bytes]]) -> "tecton.DataFrame":
        """
        [Advanced Feature] Queries the FeatureService with partial set of join_keys defined in the ``online_serving_index``
        of the included FeaturePackages. Returns FeatureVectors for all matched records.

        :param join_keys: Query join keys, i.e., a union of join keys in the ``online_serving_index`` of all
            enclosed FeaturePackages.
        :return: A Tecton DataFrame
        """
        if not materializable_workspace(self.proto.fco_metadata.workspace):
            raise errors.UNSUPPORTED_OPERATION_IN_NON_PROD_WORKSPACE("query_features")
    ...
    def get_feature_vector(
        ...
        """
        if not materializable_workspace(self.proto.fco_metadata.workspace):
            raise errors.UNSUPPORTED_OPERATION_IN_NON_PROD_WORKSPACE("get_feature_vector")
        if not self.proto.online_serving_enabled:
            raise errors.UNSUPPORTED_OPERATION(
    ...
    def get_feature_dataframe(
        ...
        """
        Fetch a DataFrame of feature values from this FeatureService.

        This method will return feature values for each row in provided in the spine DataFrame. The feature values
        ...
        :param include_feature_package_timestamp_columns: (Optional) Include timestamp columns for each FeaturePackage in the FeatureService.
    ...
    def wait_until_ready(self, timeout="15m", wait_for_materialization=True, verbose=False):
        ...
        The FeatureService is considered ready once every FeaturePackage that has been added to it
        ...
        """
        if not materializable_workspace(self.proto.fco_metadata.workspace):
            raise errors.UNSUPPORTED_OPERATION_IN_NON_PROD_WORKSPACE("wait_until_ready")
        ...
        while True:
            ...
            http_response = requests.post(
                urljoin(conf.get("FEATURE_SERVICE") + "/", "v1/feature-service/get-feature-service-state"),
    ...
    def _summary_items(self):
        ...
        if "curlEndpoint" in items_map and "curlParamsJson" in items_map:
            service_url = urljoin(conf.get("API_SERVICE"), items_map["curlEndpoint"].value)
...
@sdk_public_method
def get_feature_service(name: str) -> FeatureService:
    """
