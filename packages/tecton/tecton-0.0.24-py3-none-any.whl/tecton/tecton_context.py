...
from tecton_spark.spark_helper import get_table_names
...
class TectonContext:
    ...
    @classmethod
    @sdk_public_method
    def set_logging_level(cls, level: Any):
        """
        Change Tecton's logging level.
        :param level: Desired logging object, imported from ``logging``. Example: ``logging.ERROR``.
        """
        logger_lib.set_logging_level(level)
    ...
    # dsc_map is required if spine is a SQL string.
    ...
    def _get_spine_df(
        ...
        dsc_map: Optional[Dict[str, DataSourceConfig]],
    ):
        df = None

        # sql
        if isinstance(spine, str):
            assert dsc_map is not None
            self._register_vds_temp_views_for_tables_in_sql(spine, dsc_map=dsc_map)
    ...
    def execute(
        ...
        dsc_map: Optional[Dict[str, DataSourceConfig]] = None,
        ...
        :param timestamp_key: (Optional) Name of the time column in the returning Dataframe. The column must be of type Spark timestamp.
            ...
            If spine is not provided, we default to one of the underlying FeaturePackage's time column name.
        ...
        :param dsc_map: (Optional) A Data source config map.
        :return: A Tecton DataFrame.
        """

        df = self._get_spine_df(spine, dsc_map=dsc_map)
    ...
    # Register temp views for VDS references from SQL. dsc_map must contain data source configs
    # for these.
    def _register_vds_temp_views_for_tables_in_sql(self, spine_sql: str, dsc_map: Dict[str, DataSourceConfig]):
        table_names = get_table_names(self._spark, spine_sql)
        for table_name in table_names:
            logger.debug(f"Registering VDS {table_name} as a temp view")

            if table_name in dsc_map:
                dsc = dsc_map[table_name]
            else:
                assert False, f"VDS for table '{table_name}' not found"

            self._register_temp_view_for_data_source(dsc.proto, dsc.virtual_data_source._proto, register_stream=False)

    def _register_temp_views_for_feature_package(
        self,
        feature_package_proto,
        register_stream=False,
        raw_data_time_limits: Optional[pendulum.Period] = None,
    ):
        virtual_data_sources = feature_package_proto.enrichments.virtual_data_sources
        vds_by_id = {IdHelper.to_string(vds.virtual_data_source_id): vds for vds in virtual_data_sources}

        for dsc in feature_package_proto.feature_transformation.data_source_configs:
    ...
    def _register_temp_views_for_feature_view(
        ...
        virtual_data_sources = feature_package_or_view.virtual_data_sources
    ...
    def _register_temp_views_for_feature_package_or_view(
        self,
        feature_package_or_view: FeaturePackageOrView,
        register_stream=False,
        raw_data_time_limits: Optional[pendulum.Period] = None,
        ...
        if feature_package_or_view.is_feature_view:
            self._register_temp_views_for_feature_view(feature_package_or_view, register_stream, raw_data_time_limits)
        else:
            self._register_temp_views_for_feature_package(
                feature_package_or_view.fp, register_stream, raw_data_time_limits
            )

    def _register_temp_view_for_data_source(
        self,
        data_source_config_proto,
        virtual_data_source_proto,
        register_stream=False,
        raw_data_time_limits: Optional[pendulum.Period] = None,
    ):
        from tecton_spark.data_source_helper import register_temp_view_for_data_source

        name = virtual_data_source_proto.fco_metadata.name
        ...
        register_temp_view_for_data_source(
            ...
            raw_data_time_limits=raw_data_time_limits,
            name=name,
            data_source_config=data_source_config_proto,
        )
