...
TILE_END_TIME = pendulum.datetime(2021, 5, 20, 0, 0, 0)
...
def validate_duplicate_feature_definitions(spark: SparkSession, request: ValidateDuplicateFeatureDefinitions):
    ...
    _validate_materializing_single_tile(spark, request, sample_vds_df, tile_length, tile_offset)
    _validate_materializing_multiple_tiles(spark, request, sample_vds_df, tile_length, tile_offset)
...
def _get_sample_data(
    ...
    sample_vds_df = data_source_helper.get_vds_dataframe(
        spark=spark, vds=vds, consume_streaming_data_source=False, raw_data_time_limits=time_range
...
def _validate_materializing_single_tile(
    ...
    single_tile_df = _construct_tile_df(sample_vds_df, request.vds_for_fp, TILE_END_TIME, tile_length)
...
def _validate_materializing_multiple_tiles(
    ...
    multi_tile_df = _construct_multiple_tile_df(
        sample_vds_df, request.vds_for_fp, TILE_END_TIME, tile_length, tile_offset
    )
...
def _construct_multiple_tile_df(
    ...
    second_tile = _construct_tile_df(sample_vds_df, vds, end_time, tile_length)
    ...
    if ts_column in fp_df:
        ...
        all_timestamps_adjusted = len(fv_df) == len(
            fv_df.loc[
                ...
                & (fv_df[ts_column].dt.microsecond == 999000)
            ]
        )
