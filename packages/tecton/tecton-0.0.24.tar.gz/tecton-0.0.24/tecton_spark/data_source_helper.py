...
def get_table_dataframe(
    ...
    if data_source.HasField("hive_table"):
        ...
        if data_source.file.format == FileDataSourceFormat.FILE_DATA_SOURCE_FORMAT_JSON:
            ...
            action = lambda: reader.csv(uri, header=True, inferSchema=True)
        else:
    ...
    # TODO debatably we shouldn't let users specify both timestamp_column_properties and raw_batch_translator
...
def get_redshift_dataframe(
    ...
    :return: The DataFrame created from the data source.
    """
    params = {"user": conf.get("REDSHIFT_USER"), "password": conf.get("REDSHIFT_PASSWORD")}
...
def get_vds_dataframe(
    ...
    if consume_streaming_data_source:
        ...
        if raw_data_time_limits is not None:
            df = apply_partition_and_timestamp_filter(df, vds.batch_data_source, raw_data_time_limits)
