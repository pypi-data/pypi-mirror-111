...
from .. import Connection, DataTransfer, copy_to_user_s3_bucket, AwsCredentialsFetcher
...
def _deserialize_export(aws_credentials: AwsCredentialsFetcher, export: ExportResponse) -> None:
    ...
    if path.startswith("s3://"):
        ...
        DataTransfer.download_to_local_files(
            Connection.session.download_config,  # type: ignore
...
def _deserialize_np_array(aws_credentials: AwsCredentialsFetcher, value):
    ...
    return np.load(
        DataTransfer.download_to_bytes(Connection.session.download_config, aws_credentials, value)  # type: ignore
    )
...
def _download_df(
    ...
    df: pd.DataFrame = pd.read_parquet(  # type: ignore
        DataTransfer.download_to_bytes(
            Connection.session.download_config, aws_credentials.get_credentials(), transfer_id  # type: ignore
        )
    )
