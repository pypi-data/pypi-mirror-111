...
from common_client_scheduler import (  # type: ignore
    StructRef,
    IndexColNames,
    PandasIndexMetadata,
...
from .. import DataTransfer, AwsCredentialsFetcher, Connection


def _serialize_ndarray(aws_credentials: AwsCredentialsFetcher, array: np.ndarray) -> str:
    data_bytes = BytesIO()
    # noinspection PyTypeChecker
    np.save(data_bytes, array)
    import_id = DataTransfer.upload_bytes(
        Connection.session.upload_config, aws_credentials.get_credentials(), data_bytes  # type: ignore
    )
    return import_id
...
def _upload_df(aws_credentials: AwsCredentialsFetcher, df: pd.DataFrame) -> Tuple[str, List[bool]]:
...
def _serialize_index(
    aws_credentials: AwsCredentialsFetcher, index: pd.Index
) -> PandasIndexMetadata:
...
def _serialize_series(
    aws_credentials: AwsCredentialsFetcher, series: pd.Series
) -> PandasSeriesMetadata:
...
def _serialize_df(aws_credentials: AwsCredentialsFetcher, df: pd.DataFrame) -> PandasDFMetadata:
    ...
    return PandasDFMetadata(
        ...
        col_names=list(df.columns),  # type: ignore
        col_names_name=df.columns.name,
    )


_encoder: Dict[Any, Callable[[AwsCredentialsFetcher, Any], Any]] = {
...
def encode(credentials_fetcher: AwsCredentialsFetcher, o):  # pylint: disable=invalid-name
    ...
    if isinstance(o, (Index, Series, DataFrame, DataFrameGroupBy)):
        ...
        return StructRef(id=o._id, type=o._class_name)  # type: ignore
    if type(o) in _encoder:  # pylint: disable=no-else-return
        return _encoder[type(o)](credentials_fetcher, o)
