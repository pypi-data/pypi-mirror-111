...
from common_client_scheduler import AwsCredentials  # type: ignore
...
def copy_to_user_s3_bucket(
    ...
    Args:
        ...
        destination_s3_path: a string of the form 's3://bucket/some/key/{}.data'. The '{}' will be replaced
            by integers (starting from 0) in the destination.
    """
    if not destination_s3_path.startswith("s3://"):
        raise ValueError("destination_s3_path must start with 's3://', got '{destination_s3_path}'")
    ...
    files = [
        (
            destination_s3_prefix_template.format(i),
...
def _copy_exported_files_to_s3_destination(  # pylint: disable=too-many-locals
    ...
    try:
        ...
        export_res: ExportToS3Response = Connection.poll_for_answer("exports/cloud", request)
