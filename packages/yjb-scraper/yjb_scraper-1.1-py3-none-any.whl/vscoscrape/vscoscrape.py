        ...
        while count>0:
            for url in z:
                if '%s.jpg' % str(url["upload_date"])[:-3] in os.listdir() or '%s.mp4' % str(url["upload_date"])[:-3] in os.listdir():
                    continue
                if round(time.time()) - int(str(url["upload_date"])[:-3]) > 1209600:
                    break
...
def main():
    ...
    ##parser.add_argument('lookback', help='Lookback time')
    ...
    ##parser.add_argument('-t', '--lookback', action="store_true", help='Get image with lookback greater than 2 weeks')
    ...
    # if args.lookback:
    #     scraper = Scraper(args.username)
    #     scraper.set_lookback(int(args.lookback) * 86400) #seconds in a day, so args takes number of days
    #     scraper.getImages()
