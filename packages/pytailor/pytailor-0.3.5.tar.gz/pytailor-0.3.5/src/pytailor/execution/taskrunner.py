...
class TaskRunner(APIBase):
        ...
        if download:
            ...
            for filetag in download:
                try:
                    ...
                    if not "Could not get download urls." in str(e):
                        raise

                    found = False
                    for wf_id in additional_wf_ids:
                        try:
                            self.__download_filetag_from_other_workflow(wf_id, filetag,
                                                                        use_storage_dirs)
                            found = True
                        except Exception as e:
                            pass
    ...
    def __download_filetag_from_other_workflow(self, wf_id: str, filetag: str,
                                               use_storage_dirs: bool):
        fileset_download = FileSetDownload(tags=[filetag])
        # get download links
        with RestClient() as client:
            fileset = self._handle_request(
                client.get_download_urls_from_wf_id,
                self.__project_id,
                wf_id,
                fileset_download,
                error_msg="Could not get download urls.",
