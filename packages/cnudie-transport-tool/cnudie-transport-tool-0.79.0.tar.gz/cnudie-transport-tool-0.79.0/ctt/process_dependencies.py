...
def process_upload_request(upload_request: processing_model.ContainerImageUploadRequest):
    ...
    if container.util.image_exists(tgt_ref):
...
def process_images(
    ...
    def process_job(processing_job: processing_model.ProcessingJob):
        ...
        if processing_mode is ProcessingMode.REGULAR:
            process_upload_request(processing_job.upload_request)
...
def main():
    ...
    parser.add_argument('-u', '--upload-mode',
                        choices=[
    ...
    component_descriptor_v2 = process_images(
        ...
        processing_mode=processing_mode,
        upload_mode=product.v2.UploadMode(parsed.upload_mode),
    )
