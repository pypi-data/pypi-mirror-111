...
class Datev(object):
    ...
    def export_to_template(self, df: pd.DataFrame,
        ...
        template_description = "[Satzbeschreibung]\n"\
                               f"100;u_lod_psd_mitarbeiter;{'pnr_betriebliche#psd' if use_alternative_employee_number else 'pnr#psd'};{'pnr#psd' if 'employee_id_datev' in df.columns.values else ''};duevo_familienname#psd;duevo_vorname#psd;gebname#psd;adresse_strassenname#psd;adresse_strasse_nr#psd;adresse_plz#psd;adresse_ort#psd;\n" \
        ...
        with open(f"{filepath}{filename}", 'w', encoding="latin-1", newline='\r\n') as file:
            ...
            for index, dfrow in df.iterrows():
                ...
                if check_if_column_in_dataset(required_columns_subset, df.columns):
                    body.append(f"100;{dfrow['employee_id']};{dfrow['employee_id_datev'] + ';' if 'employee_id_datev' in df.columns.values else ''}{dfrow['lastname']};{dfrow['firstname']};{dfrow['birthname']};{dfrow['street']};{dfrow['housenumber']};{dfrow['postalcode']};{dfrow['city']};" + "\n")
