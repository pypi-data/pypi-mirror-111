...
export default class WorkflowEditor extends Rete.NodeEditor {
  constructor(id, container) {
    ...
    // Register a custom zoom functionality, all other zoom events except the ones from the area plugin (which sets no
    // source) and our custom one are ignored.
    container.addEventListener('wheel', this._wheel.bind(this.view.area._zoom));
    this.on('zoom', ({source}) => !source || source === 'wheel_custom');
  }
  ...
  _wheel(e) {
    ...
    this.onzoom(delta, ox, oy, 'wheel_custom');
  }
