...
def get_selected_resources(
    ...
    populate and search through multi selection fields. Note that only accessible
    resources are returned. In this context having access to a resource means having
    read permission for that resource.
...
def get_resource_group_roles(resource, page=1, per_page=10, filter_term="", user=None):
    ...
    if include_all_groups:
        group_ids = {group.id for group in group_ids_query}
