...
def get_linked_resources(model, relationship, actions=None, user=None):
    ...
    :return: The resulting query of the linked resources, ordered by last modification
    ...
    resources_query = relationship.filter(model.id.in_(object_ids_query)).order_by(
        model.last_modified.desc()
    )
