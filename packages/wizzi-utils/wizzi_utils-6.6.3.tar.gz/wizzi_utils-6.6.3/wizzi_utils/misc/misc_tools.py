...
def get_cuda_version(ack: bool = False, tabs: int = 1) -> str:
    ...
    if cuda_path is not None:
        cuda_v = add_color('{}* CUDA Version: {}'.format(tabs * '\t', os.path.basename(cuda_path)), ops=SUCCESS_C)
    else:
...
def is_aarch64():
    ...
    see os_test()
    TODO *** NOT TESTED
    TODO if doesn't work - https://en.wikipedia.org/wiki/Uname
    """
    try:
...
def main_wrapper(
    ...
    if cv2_v:
        try:
            ...
            print(add_color('* {} - {}'.format(err, pip), 'r'))
    ...
    if tf_v:
        ...
        try:
            ...
            pip = '\n\twin os: pip install --extra-index-url https://google-coral.github.io/py-repo/ tflite_runtime'
            print(add_color('* {} - {}'.format(err, pip), 'r'))
    ...
    if is_float(var):
        ...
        if fp is not None and fp >= 0:
            ...
            if all((is_int(item) or is_float(item)) for item in var):
                if fp > 0 and all(is_int(item) for item in var):
                    fp = 0
                f_format = '{:,.%xf}' % fp
...
def wizzi_utils_requirements():
    ...
    print('\t{}'.format(pip.format('matplotlib')))
    print('\t{}'.format(pip.format('mpl_toolkits')))
    ...
    print('\t\t{} # also the test'.format(sub_pack.format(CVT_PACK)))
    ...
    print(LINES)
    url = 'https://download.pytorch.org/whl/cu90/torch_stable.html'
    print(pack.format(TT_PACK, TT_MOD, TT_SC))
    print('\ttorch options:')
    print('\t\t{} # cpu'.format(pip.format('torch')))
    print('\t\t{} # gpu(cuda 9 and 9.1)'.format(pip.format('torch==1.1.0 torchvision==0.3.0 -f {}'.format(url))))
    ...
    print('\ttensorflow options:')
    print('\t\t{} # cpu only'.format(pip.format('tensorflow')))
    print('\t\t{} # gpu cuda 9.1'.format(pip.format('tensorflow-gpu==1.12')))
    print('\t\t{} # gpu cuda>=10'.format(pip.format('tensorflow-gpu')))
    ...
    print('\t\t{} # windows 10'.format(pip.format('--extra-index-url {} tflite_runtime'.format(url))))
    print('\t{}'.format(sub_pack.format(ST_PACK)))
    print('\t{}'.format(sub_pack.format(PYPLT_PACK)))
    print('\t{}'.format(sub_pack.format(CVT_PACK)))
...
class FPS:
    ...
    def start(self, ack_progress: bool = False, tabs: int = 1) -> None:
        if ack_progress:
            print('{}Iter {} Started:'.format(tabs * '\t', len(self.times_elapsed) + 1))
        self.timer_begin = get_timer()
        self.timer_end = None
        return

    def update(self, ack_progress: bool = False, tabs: int = 1) -> None:
    ...
    #     rt = self.times_elapsed[-1]
    #     if rt < 0.001:  # 0.0001 -> 0.100ms
    #         rt_str = '{:.3f}ms'.format(rt * 1000)  # fp=6 digit
    #     elif rt < 0.1:
    #         rt_str = '{:.2f}ms'.format(rt * 1000)  # fp=6 digit
    #     elif rt < 1:
    #         rt_str = '{:.1f}ms'.format(rt * 1000)
    #     elif rt < 20:
    #         rt_str = '{:.3f}s'.format(rt)
    #     elif rt < 60:
    #         rt_str = '{:.1f}s'.format(rt)
    #     elif rt < 120:
    #         m, s = divmod(rt, 60)
    #         h, m = divmod(m, 60)
    #         rt_str = '{}:{:.1f}'.format(int(m), s)
    #     print('\trt={:.6f}s == {}'.format(rt, rt_str))
    #     return rt_str

    def so_far(self, tabs: int = 1) -> None:
