...
from wizzi_utils.open_cv.models_dnn_meta_data import MODELS_OPEN_POSE_META_DATA
...
class DnnObjectDetectionModels:
    ...
    MANDATORY_KEYS = [
        ...
        'swapRB', 'crop', 'threshold', 'nms_threshold', 'labels_dict',
        'URL'
    ]
    DEFAULT_COLOR_D = {
        'bbox': 'r',
    ...
    def __init__(self,
        ...
        if model_name not in MODELS_DNN_OBJECT_DETECTION_META_DATA:
            err = 'model name must be one of {}'.format(list(MODELS_DNN_OBJECT_DETECTION_META_DATA.keys()))
            mt.exception_error(err, real_exception=False, tabs=tabs)
            exit(-1)
        ...
        if threshold is not None:
            self.model_conf['threshold'] = threshold
        if nms_threshold is not None:
            self.model_conf['nms_threshold'] = nms_threshold
    ...
    def detect_cv_img(
        ...
        """
        classes, scores, boxes = self.model.detect(cv_img, self.model_conf['threshold'],
                                                   self.model_conf['nms_threshold'])
...
def get_all_pose_models_info(ack: bool = False) -> list:
    model_names = []
    for i, (m_name, m_dict) in enumerate(MODELS_OPEN_POSE_META_DATA.items()):
        if ack:
            mt.dict_as_table(table=m_dict['info'], title='{}: {}'.format(i + 1, m_name), tabs=0)
        model_names.append(m_name)
    return model_names
...
class DnnOpenPoseModels:
    # each MODELS_DNN_META_DATA entry should have this keys:
    MANDATORY_KEYS = [
        'job', 'family', 'num_points', 'point_pairs', 'in_dims',
        'scalefactor', 'mean', 'swapRB', 'crop', 'threshold',
        'URL'
    ]
    BAD_P = (-1, -1)
    ...
    def __init__(self,
        ...
        if model_name not in MODELS_OPEN_POSE_META_DATA:
            err = 'model name must be one of {}'.format(list(MODELS_OPEN_POSE_META_DATA.keys()))
            mt.exception_error(err, real_exception=False, tabs=tabs)
            exit(-1)
        else:  # supported model
            self.model_conf = MODELS_OPEN_POSE_META_DATA[model_name]
            mandatory_keys_list = self.MANDATORY_KEYS[:]
            current_keys_list = list(self.model_conf.keys())
            man_keys_not_found = []
            for man_k in mandatory_keys_list:
            ...
            if len(man_keys_not_found) > 0:
                mt.exception_error('model config must have the keys {}'.format(man_keys_not_found),
                                   real_exception=False, tabs=tabs)
                exit(-1)
            if self.model_conf['job'] != Jobs.OPEN_POSE.value:
                mt.exception_error('this class supports only {}'.format(Jobs.OPEN_POSE.value),
                                   real_exception=False, tabs=tabs)
                exit(-1)
        self.tabs = tabs
        tabs_s = self.tabs * '\t'
        self.name = model_name
        self.local_path = save_load_dir
        self.num_points = self.model_conf['num_points']
        self.point_pairs = self.model_conf['point_pairs']
        ...
        print('{}Loading {}'.format(tabs_s, self.name))
        if self.model_conf['family'] == DnnFamily.Caffe.value:
            model_prototxt = "{}/{}.prototxt".format(self.local_path, self.name)
            model_caffe = "{}/{}.caffemodel".format(self.local_path, self.name)
            DnnObjectDetectionModels._download_if_needed(local_path=model_prototxt, url_dict=self.model_conf['URL'],
                                                         file_key='prototxt')
            DnnObjectDetectionModels._download_if_needed(local_path=model_caffe, url_dict=self.model_conf['URL'],
                                                         file_key='caffemodel')
            print('{}\tprototxt({}): {}'.format(tabs_s, mt.file_or_folder_size(model_prototxt), model_prototxt))
            print('{}\tcaffemodel({}): {}'.format(tabs_s, mt.file_or_folder_size(model_caffe), model_caffe))
    ...
    def detect_cv_img(
        ...
        inpBlob = cv2.dnn.blobFromImage(
            image=cv2.resize(cv_img, (368, 368)),
            scalefactor=self.model_conf['scalefactor'],
            size=self.model_conf['in_dims'],
            mean=self.model_conf['mean'],
            swapRB=self.model_conf['swapRB'],
            crop=self.model_conf['crop'])
    ...
    def extract_results(
        ...
        if ack:
            title_suffix = '' if img_title is None else '{} '.format(img_title)
            title = '{} detection on image {}{}:'.format(self.name, title_suffix, cv_img.shape)
        ...
        for i in range(1):  # currently 1 object max
            label = 'pose {}'.format(i)
            joints_2d_points = []
            joints_count = 0
            conf_sum = 0.0
            for idx in range(self.num_points):
                probMap = outputs[idx, :, :]  # confidence map.
                minVal, prob, minLoc, point = cv2.minMaxLoc(probMap)  # Find global maxima of the probMap.
                # Scale the point to fit on the original image
                x = (img_w * point[0]) / W
                y = (img_h * point[1]) / H

                if prob > self.model_conf['threshold']:
                    joints_2d_points.append((int(x), int(y)))
                    joints_count += 1
                    conf_sum += prob
                else:
                    joints_2d_points.append(self.BAD_P)
            if joints_count > 0:
                score_frac = conf_sum / joints_count
                score_percentage = round(score_frac * 100, fp)
            else:
            ...
            detection_d = {
                'label': label,
                'score_percentage': score_percentage,  # avg conf
                'joints_2d_points': joints_2d_points,
            }

            detections.append(detection_d)
            if ack:
                d_msg = '{}\tDetected in {}: {}/{} joints with avg conf {}%: {}'
                print(
                    d_msg.format(tabs * '\t', label, joints_count, self.num_points, score_percentage, joints_2d_points))
        return detections
    ...
    def draw_detections(
        ...
        for detection in detections:
            ...
            # label = detection['label']
            # score_percentage = detection['score_percentage']
            points = detection['joints_2d_points']
            if draw_pose:
                for pair in self.point_pairs:
                    partA, partB = pair
                    if points[partA] != self.BAD_P and points[partB] != self.BAD_P:
                        cv2.line(cv_img, pt1=points[partA], pt2=points[partB], color=edge_c, thickness=2)
                        cv2.circle(cv_img, center=points[partA], radius=3, color=joint_c, thickness=-1)
            if draw_points:
                for i, joints_2d_points in enumerate(points):
                    if joints_2d_points != self.BAD_P:
                        cv2.circle(cv_img, center=joints_2d_points, radius=3, color=joint_c, thickness=-1)
                        joints_2d_points_text = (joints_2d_points[0] - 5, joints_2d_points[1])
                        add_text(cv_img, header="{}".format(i), pos=joints_2d_points_text, text_color=text_c,
                                 with_rect=False, bg_font_scale=2)
        return
