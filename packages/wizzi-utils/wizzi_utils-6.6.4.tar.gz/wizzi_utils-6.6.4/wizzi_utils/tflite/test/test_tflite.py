...
SAVE_SCOPE = 'TFL'
OD_BEST_MODEL = 'ssd_mobilenet_v2_mnasfpn'
POSE_BEST_MODEL = 'pose_landmark_heavy'
...
def __od_get_models(models_selection: str = cvtt.BEST_MODELS):
    ...
    for m_name in models_names:
        save_dir = '{}/{}/{}'.format(mtt.TFL_MODELS, tflt.MODELS_TFL_META_DATA[OD_BEST_MODEL]['job'], m_name)
...
def __pose_get_models(models_selection: str = cvtt.BEST_MODELS):
    if models_selection == cvtt.BEST_MODELS:
        # preparing 3 best models
        grid = (1, 3)
        display_im_size = (640, 480)
        models_names = [
            'pose_landmark_lite',
        ...
        # preparing all(3) models
        grid = (1, 3)
        display_im_size = (640, 480)
        models_names = tflt.get_all_pose_models_info(ack=False)
    else:
        # preparing 1 models - default is BEST_MODEL
        grid = (1, 1)
        display_im_size = (640, 480)
...
def od_best_model_images_test(m_name: str = OD_BEST_MODEL, ms: int = cvtt.BLOCK_MS_NORMAL):
    mt.get_function_name(ack=True, tabs=0)
    od_models_compare_images_test(
...
def pose_best_model_images_test(m_name: str = POSE_BEST_MODEL, ms: int = cvtt.BLOCK_MS_NORMAL):
    mt.get_function_name(ack=True, tabs=0)
...
def pose_models_compare_images_test(
    ...
    save_dir = '{}/{}/{}/{}'.format(mtt.IMAGES_OUTPUTS, mt.get_function_name(), SAVE_SCOPE, models_selection)
    mt.create_dir(save_dir)
    cvtt.__pose_models_images_test(
        models=models,
        images_names=images_names,
        cv_imgs_orig=cv_imgs_orig,
        grid=grid,
