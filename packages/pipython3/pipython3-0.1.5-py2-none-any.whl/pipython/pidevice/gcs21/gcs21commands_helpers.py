...
from ..common import gcsbasecommands

__signature__ = 0xf8bd9f96877266726b1835363f3cf503
...
class PIBlockKeys(Enum):
    ...
    RECORDER_TRIGGER_OVERVIEW = 'Recorder Trigger Overview'
...
PI_UMF_PARAMETER_IDS = {'MMU': {'MAX_TRACES': '0x2002'}}
...
def isgcs21_by_qidn(gcsmessages):
    ...
    if idn_split[1].strip() in gcsbasecommands.GCS1DEVICES:
        return False

    if idn_split[1].split('.')[0].strip() in gcsbasecommands.GCS2DEVICES:
        return False
...
def find_sublock_with_key(umf_dict, key):
    """
