...
from .gcs21.gcs21pitools import GCS21Tools, GCS21DeviceStartup
from .common.gcsbasepitools import GCSBaseTools


__signature__ = 0x93dac58bc8efe2642ca398a90b3c381e
...
def DeviceStartup(pidevice, **kwargs):
    ...
    the Type'pidevice' (GCS2Commands or GCS21Commands)
    :param pidevice: an istance to GCS2Commands or GCS21Commands object
    :type pidevice: GCS2Commands or GCS21Commands
    :param kwargs: Optional arguments with keywords that are passed to sub functions.
    :return: instance to a DeviceStartop object
    :rtype: GCS2DeviceStartup or GCS21DeviceStartup
    """
    if isdeviceavailable([GCS2Commands, ], pidevice):
        return GCS2DeviceStartup(GCS2Tools(pidevice), **kwargs)

    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21DeviceStartup(GCS21Tools(pidevice), **kwargs)
...
def startup(pidevice, stages=None, refmodes=None, servostates=True, **kwargs):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).startup(stages, refmodes, servostates, **kwargs)
...
def enableaxes(pidevice, axes, **kwargs):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).enableaxes(axes, **kwargs)
        return
...
def setservo(pidevice, axes, states=None, toignore=None, **kwargs):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).setservo(axes, states, toignore, **kwargs)
...
def getservo(pidevice, axes):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).getservo(axes)
...
def moveandwait(pidevice, axes, values=None, timeout=300):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).moveandwait(axes, values, timeout)
        return
...
def movetomiddle(pidevice, axes=None):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).movetomiddle(axes)
        return
...
def ontarget(pidevice, axes):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).ontarget(axes)
...
def stopall(pidevice, **kwargs):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).stopall(**kwargs)
        return
...
def getaxeslist(pidevice, axes):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        return GCS21Tools(pidevice).getaxeslist(axes)
...
def waitonready(pidevice, timeout=300, predelay=0, polldelay=0.1):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).waitonready(timeout, predelay, polldelay)
        return
...
def waitontarget(pidevice, axes=None, timeout=300, predelay=0, postdelay=0, polldelay=0.1):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).waitontarget(axes, timeout, predelay, postdelay, polldelay)
        return
...
def waitonreferencing(pidevice, axes=None, timeout=300, predelay=0, postdelay=0, polldelay=0.1):
    ...
    if isdeviceavailable([GCS21Commands, ], pidevice):
        GCS21Tools(pidevice).waitonreferencing(axes, timeout, predelay, postdelay, polldelay)
        return
