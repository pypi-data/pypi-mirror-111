...
def get_kaniko_manifest(bento, path, registry, s3_endpoint, uid=None, silent=False):
    if ':' in bento:
        name, version = bento.split(':')
    else:
        name = bento
    ...
    manifest = gen_kaniko_manifest(name, pod_name, image_name, version, path, registry, s3_endpoint)
