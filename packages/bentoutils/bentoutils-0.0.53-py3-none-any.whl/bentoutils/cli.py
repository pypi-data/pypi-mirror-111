...
@click.option('--s3_endpoint', help="S3 endpoint URL")
@click.option('--uid', help="unique ID")
@click.option('--insecure', is_flag=True, help='do not use SSL')
def get_kaniko_manifest(bento, path, registry, s3_endpoint, uid=None, silent=False, insecure=False):
    if ':' in bento:
        name, version = bento.split(':')
    else:
        name = bento
    ...
    manifest = gen_kaniko_manifest(name, pod_name, image_name, version, path, registry, s3_endpoint, insecure)
...
@click.option('--insecure', is_flag=True, help='do not use SSL')
@click.pass_context
def containerize(ctx, bento, registry, s3_endpoint, insecure):
    path = ctx.invoke(get_saved_path, bento=bento, silent=True)
    manifest, pod_name = ctx.invoke(get_kaniko_manifest, bento=bento, path=path, registry=registry, s3_endpoint=s3_endpoint, silent=True, insecure=insecure)
    from_yaml(manifest)
    click.echo(pod_name)
    return pod_name
