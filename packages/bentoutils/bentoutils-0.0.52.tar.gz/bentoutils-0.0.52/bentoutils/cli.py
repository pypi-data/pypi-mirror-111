...
@click.option('--s3_endpoint', help="S3 endpoint URL")
@click.pass_context
def containerize(ctx, bento, registry, s3_endpoint):
    path = ctx.invoke(get_saved_path, bento=bento, silent=True)
    manifest, pod_name = ctx.invoke(get_kaniko_manifest, bento=bento, path=path, registry=registry, s3_endpoint=s3_endpoint, silent=True)
    from_yaml(manifest)
    click.echo(pod_name)
    return pod_name
