...
def auto_rescale(img, dim):
    ...
    if img.shape[2] == 1:
        img_array = np.expand_dims(img, 3)
    else:
        img_array = img

    if raw_dims < dim:
        scaled_img = cv2.resize(
            img_array, scale, interpolation=cv2.INTER_CUBIC
        )
    else:
        scaled_img = cv2.resize(
            img_array, scale, interpolation=cv2.INTER_AREA
        )
...
class Image_Dataset:
    ...
    def augment_training_set(
        ...
        if self.split == 0:
            ...
            self.shadow["train"][0] = np.expand_dims(
                np.array(X_train_aug), 3
            )
