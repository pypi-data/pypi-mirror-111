...
class QC(object):
    def __init__(self,
        ...
        self.metric = metric
        self.random_stateQC = random_stateQC
    ...
    def dnaIntensityCutoff(data, self, args):
        ...
        # create save dir
        intensity_dir = os.path.join(self.out_dir, 'intensity')
        ...
        for name, group in df.groupby('Sample', sort=False):
            ...
            n, bins, patches = plt.hist(
                group[f'{dna1}_{self.mask_object}'], bins=bins,
            ...
            def submit(text):
                ...
                group_update = group[
                    (group[f'{dna1}_{self.mask_object}'] > lowerCutoff) &
                    (group[f'{dna1}_{self.mask_object}'] < upperCutoff)
                    ]
                ...
                if text == group['Sample'].unique():
                    ...
                    dna_intensity = (
                        group_update[f'{dna1}_{self.mask_object}'][
            ...
            if lowerCutoff == upperCutoff:
                lowerCutoff = group[f'{dna1}_{self.mask_object}'].min()
                upperCutoff = group[f'{dna1}_{self.mask_object}'].max()
            ...
            plt.hist(
                group_update[f'{dna1}_{self.mask_object}'], bins=bins,
    ...
    def dnaAreaCutoff(data, self, args):
        ...
        # create save dir
        area_dir = os.path.join(self.out_dir, 'area')
    ...
    def metaQC(data, self, args):
        ...
        reclass_dir = os.path.join(
            self.out_dir, f'clustering/reclassification'
            )
        if self.metaQC:
        ...
        selectROIs_data = pd.read_parquet(
            os.path.join(self.out_dir, 'checkpoints/selectROIs.parquet')
            )
        ...
        dnaIntensityCutoff_data = pd.read_parquet(
            os.path.join(
                self.out_dir, 'checkpoints/dnaIntensityCutoff.parquet')
            )
        ...
        dnaAreaCutoff_data = pd.read_parquet(
            os.path.join(self.out_dir, 'checkpoints/dnaAreaCutoff.parquet')
            )
        ...
        crossCycleCorrelation_data = pd.read_parquet(
            os.path.join(
                self.out_dir, 'checkpoints/crossCycleCorrelation.parquet')
            )
        ...
        pruneOutliers_data = pd.read_parquet(
            os.path.join(self.out_dir, 'checkpoints/pruneOutliers.parquet')
            )
        ...
        if self.metaQC:
            ...
            if os.path.exists(os.path.join(reclass_dir, 'QCData.pkl')):
                ...
                # transform data
                # add first DNA cycle
                abx_channels = [f'{dna1}_{self.mask_object}'] + abx_channels
            ...
            for e, chunk in enumerate(chunks):
                if e in range(chunk_index, len(chunks)):
                      ...
                      os.path.join(clustering_dir, 'embedding.npy')):
                        ...
                        if self.embeddingAlgorithmQC == 'TSNE':
                            ...
                            embedding = TSNE(
                                ...
                                random_state=self.random_stateQC,
                    ...
                    def submit(text):
                        ...
                        markers, dna1, dna_moniker, abx_channels = (
                            read_markers(
                            markers_filepath=os.path.join(
                                self.in_dir, 'markers.csv'),
                            mask_object=self.mask_object,
                            markers_to_exclude=self.markers_to_exclude,
                            )
                        abx_channels = [
        ...
        plt.savefig(
            os.path.join(
                self.out_dir, 'censored_by_stage.pdf'), bbox_inches='tight'
            )
        plt.close('all')
    ...
    def clustering(data, self, args):
        ...
        #####################################################################
        clustering_dir = os.path.join(self.out_dir, 'clustering/final')
        if not os.path.exists(clustering_dir):
            os.makedirs(clustering_dir)
        ...
        for i in range(
          len(list(self.sample_statuses.values())[0].split(', '))):
            ...
            frequency_dir = os.path.join(
                self.out_dir, 'clustering/final/frequency_stats',
                f"{test}_v_{control}"
                )
    ...
    def curateThumbnails(data, self, args):
        ...
        thumbnails_dir = os.path.join(
            self.out_dir, 'clustering/final/thumbnails'
            )
        ...
        for cluster in clusters_to_run:
            ...
            for sample_name in ome_tifs:
                ...
                if self.segOutlines:
                    seg_img = imread(
                        os.path.join(f'{self.in_dir}/seg/', sample_name),
                        key=0
                        )
