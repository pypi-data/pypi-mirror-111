...
class GitLabStateMaintainer:
    ...
    def heptapod_apply_changes(self, prune_reasons, changes):
        ...
        if txn is None:
            ...
            # TODO in Heptapod > 0.22, just make release hook the
            # default case.
            if 'merge_request.create' in push_opts:
                self.repo._afterlock(
                    lambda success: post_receive(push_opts=push_opts))
            else:
                txn.addpostclose(b'heptapod_git_sync',
                                 lambda txn: post_receive(push_opts=push_opts))
