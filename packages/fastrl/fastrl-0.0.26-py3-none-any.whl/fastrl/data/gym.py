...
__all__ = ['source_events', 'return_data', 'Source', 'return_history', 'TstCallback', 'GymLoop']
...
# Cell
def return_data(source,data:Dict): return data
...
class Source(Loop):
    ...
    def _history(self):
        ...
        return self.return_fn(self,self.data())
...
# Cell
def return_history(source,data):
...
class GymLoop(LoopCallback):
    ...
    def initialize(self):
        ...
        self.source.return_fn=return_history
    ...
    def before_history(self):
        if len(self.histories)<=1 and sum(self.done)>0:
            self.source.all_done=TensorBatch([[True]])
