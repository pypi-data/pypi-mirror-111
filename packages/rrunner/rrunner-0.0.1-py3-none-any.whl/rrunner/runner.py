...
from rrunner.common.handle_excel import Excel
from rrunner.common.handle_path import DATA_DIR, CASE_DIR, TESTS_DIR
from rrunner.common.handle_config import config
from rrunner.ext.uploader import prepare_upload_step
...
from rrunner import utils, exceptions
from rrunner.client import HttpSession
from rrunner.exceptions import ValidationFailure, ParamsError
#from rrunner.ext.uploader import prepare_upload_step
from rrunner.loader import load_project_meta, load_testcase_file
from rrunner.parser import build_url, parse_data, parse_variables_mapping
from rrunner.response import ResponseObject
from rrunner.testcase import Config, Step
from rrunner.utils import merge_variables
from rrunner.models import (
    TConfig,
    TStep,
    VariablesMapping,
    StepData,
...
class RRun(object):
    ...
    def with_project_meta(self, project_meta: ProjectMeta) -> "RRun":
        self.__project_meta = project_meta
        return self

    def with_session(self, session: HttpSession) -> "RRun":
        self.__session = session
        return self

    def with_case_id(self, case_id: Text) -> "RRun":
        self.__case_id = case_id
        return self

    def with_variables(self, variables: VariablesMapping) -> "RRun":
        self.__session_variables = variables
        return self

    def with_export(self, export: List[Text]) -> "RRun":
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        parsed_request_dict["headers"].setdefault(
            "Authorization",
            self.__project_meta.token['Authorization']
        )
        ...
        #url = "http://smartfactoryback.letsbim.net" + url_path
        url = config.get("env", "base_url") + url_path
        ...
        sheetname = self.__config.sheetname
        exclename = self.__config.name
        if exclename == "template.xlsx":
            excel = Excel(os.path.join(TESTS_DIR, exclename), sheetname)
        else:
            excel = Excel(os.path.join(CASE_DIR, exclename), sheetname)
        ...
        try:
            ...
            #write pass
            excel.write_excel(row=(step.case_id+1), column=13, value="通过")

        except ValidationFailure:
            session_success = False
            #write fail
            excel.write_excel(row=(step.case_id+1), column=13, value="失败")
            ...
            if hasattr(self.__session, "data"):
                # RRun.client.HttpSession, not locust.clients.HttpSession
    ...
    def run_testcase(self, testcase: TestCase) -> "RRun":
        """run specified testcase

        Examples:
            >>> testcase_obj = TestCase(config=TConfig(...), teststeps=[TStep(...)])
            >>> RRun().with_project_meta(project_meta).run_testcase(testcase_obj)
    ...
    def run_path(self, path: Text) -> "RRun":
    ...
    def test_start(self, get_token, param: Dict = None) -> "RRun":
        ...
        self.__project_meta.token = token
