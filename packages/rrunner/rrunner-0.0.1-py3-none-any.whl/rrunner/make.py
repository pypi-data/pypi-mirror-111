...
from rrunner import exceptions, __version__
from rrunner.parser import (
    convert_variables
)
# from RRun.compat import (
...
from rrunner.response import uniform_validator
from rrunner.utils import merge_variables, is_support_multiprocessing
...
__TEMPLATE__ = jinja2.Template(
    """# NOTE: Generated By RRun v{{ version }}
# FROM: {{ testcase_path }}
...
def __ensure_testcase_module(path: Text) -> NoReturn:
    ...
    with open(init_file, "w", encoding="utf-8") as f:
        f.write("# NOTICE: Generated By RRun. DO NOT EDIT!\n")
...
def make_config_chain_style(config: Dict) -> Text:
    ...
    if "sheetname" in config:
        config_chain_style += f'.sheetname("{config["sheetname"]}")'
...
def make_teststep_chain_style(teststep: Dict) -> Text:
    if teststep.get("request"):
        step_info = f'RunRequest("{teststep["name"]}",{teststep["case_id"]})'
