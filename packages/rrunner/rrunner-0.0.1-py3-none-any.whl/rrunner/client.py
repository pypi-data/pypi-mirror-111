...
from rrunner.models import RequestData, ResponseData
from rrunner.models import SessionData, ReqRespData
from rrunner.utils import lower_dict_keys, omit_long_data
...
def get_req_resp_record(resp_obj: Response) -> ReqRespData:
    ...
    # log request details in debug rrune
    ...
    # log response details in debug rrune
...
class HttpSession(requests.Session):
    ...
    to be able to log in and out of websites). Each request is logged so that RRun can
    display statistics.
    ...
    def request(self, method, url, name=None, **kwargs):
        ...
        response = self._send_request_safe_rrune(method, url, **kwargs)
    ...
    def _send_request_safe_rrune(self, method, url, **kwargs):
        """
        Send a HTTP request, and catch any exception that might occur due to connection problems.
        Safe rrune has been removed from requests 1.x.
        """
        try:
