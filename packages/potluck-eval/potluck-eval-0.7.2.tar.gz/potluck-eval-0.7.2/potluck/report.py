...
def setup(templates_directory=None, resources_directory=None):
    ...
    register_filter("timestamp", file_utils.at_time)
...
def get_css():
    ...
    if RESOURCES_DIR is None:
        ...
        return pkg_files.joinpath("potluck.css").read_text()
    else:
...
def get_js():
    ...
    if RESOURCES_DIR is None:
        pkg_files = importlib_resources.files("potluck.resources")
        return pkg_files.joinpath("potluck.js").read_text()
    else:
...
def load_template(template_name):
    ...
    try:
        ...
        logging.log(
            f"Syntax error while building Jinja2 template:"
            f" '{template_name}'\n"
            f"Error occurred on line: {e.lineno}\n"
            f"Error was: {str(e)}\n"
            '-' * 80
        )
        raise
        ...
        logging.log(
            f"Unexpected non-syntax error while building Jinja2"
...
def render_report(report, output_file, html_output_file):
    """
    ...
    report["timestamp"] = file_utils.timestamp()
...
def augment_report(report, blank=False):
    ...
    if "files" in report:
        for file_entry in report["files"]:
            ...
            # TODO: Something else for "raw" files?
...
def render_code(taskid, filename, raw_code):
    ...
    return f'<pre id="{taskid}_code_{filename}">' + annotated + '</pre>'
...
def annotate_line(taskid, filename, lineno, line_html):
    ...
    same line wrapped in a span which includes a line ID, as well as an
    onclick property that will highlight that line using the
    `focusCodeLine` function which is defined in `resources/potluck.js`.
    The returned span also includes a `linenum` span before the line text
    which includes a line number.
    """
    lineid = html_tools.line_id(taskid, filename, lineno)
    block_id = html_tools.block_id(taskid, filename)
    return (
        f'<span id="{lineid}"'
        f' class="codeline codeline_plain"'
        f' onclick="focusCodeLine(\'{block_id}\', \'{lineid}\');'
        f' return false">'
        f'<span class="linenum">{lineno} </span>'
        f' {line_html}'
        f'</span>'
    )


def render_rubric_table(table, blank=False):
    """
    Renders a rubric table (from the 'table' slot of an evaluation; see
    `potluck.rubrics.Rubric.evaluate`) as an HTML string. Returns an HTML
    string that builds a
    ...
    return '<div class="rubric_table">\n{}\n</div>'.format(
        '\n'.join(
            render_rubric_row(row, blank)
            for row in table
        )
    )


def render_rubric_row(row, blank=False):
    """
    Renders a single row of a rubric table into an HTML div (see
    render_rubric_table). The resulting div has class "rubric_row", plus
    an additional class depending on the status of the given row with the
    string 'status_' prefixed and any spaces replaced by underscores.
    There are also 'tag_' classes added for each tag that the row has.
    ...
    subtable = ''
    if row.get("subtable"):
        subtable = render_rubric_table(row.get("subtable"), blank)
