...
def create_run_function_payload(
    ...
    containing the function's result.
    ...
    def payload(context):
        ...
        return { "value": fn(*real_posargs, **real_kwargs) }
...
def run_for_base_and_ref_values(payload):
    """
    ...
    values where available and setting ref_* versions of the original
    payload's result slots. Note that when applying multiple payload
    augmentations, this one should be applied last.
    """
    def double_payload(context):
        ...
        collapsed_context = copy.copy(context)
        for key in collapsed_context:
            if key.startswith("ref_"):
                collapsed_context[key[4:]] = collapsed_context[key]

        # Get results from collapsed context
        ref_result = payload(collapsed_context)
...
def capturing_printed_output(payload, capture_errors=False):
    ...
    def capturing_payload(context):
        ...
        try:
            ...
            if capture_errors:
                # TODO: path reworking here, if not in html_tools!
...
def with_fake_input(payload, inputs, extra_policy="error"):
    ...
    called instead of actually prompting for values from stdin.
...
def tracing_function_calls(payload, trace_targets, state_function):
    ...
    in the outermost scope and is a dictionary with the following keys:
        ...
        args: A dictionary of arguments passed to the function, mapping
            argument names to their values. For calls to C functions
            (such as most built-in functions), arguments are not
            available, and this key will not be present.
        result: The return value of the function. May be None if the
            function was terminated due to an exception, but there's no
            way to distinguish that from an intentional None return.
            For calls to C functions, this key will not be present.
        pre_state: A state object resulting from calling the given
            state_function just before the traced function call starts,
            with the function name as its only argument. Calls made
            during the execution of the state function will not be
            traced.
        ...
        during: A list of trace entries in the same format representing
            traced function calls which were initiated and returned
            before the end of the function call that this trace entry
            represents.
...
def warp_turtle(context):
    """
    Disables turtle tracing. Use as a setup function with `with_setup`
    and/or via `specifications.HasPayload.do_setup`. Note that you MUST
    also use `finalize_drawing` as a cleanup function, or else some
    elements may not actually get drawn.
    """
    turtle.tracer(0, 0)
    return context
