...
TYPED_LIT_VAR_SPEC = (
    (1, 2),
    re.compile(r'\A_([a-zA-Z0-9]+)_(' + '|'.join(LIT_TYPES.keys()) + ')_\Z')\
# noqa W605
)
...
def node_is_bindable(node):
    ...
    result = isinstance(node, ast.expr) or isinstance(node, ast.stmt)
...
def permutations(node):
    """Generate all permutations of the binary and boolean operations
    in and below node, via associativity and mirroring, respecting purity."""
    if (
...
def imatches(node, pat, env1, seq):
    ...
    and node == pat
        ...
        if node_is_bindable(node):
            env2 = bind(env1, node_var(pat), node)
            if env2:
        ...
        if type(pat) == list:
            # Node an pattern are both lists.  Do positional matching.
            if len(pat) == 0:
...
def findall(node, pat, outside=[], **kwargs):
    """Search for all sub-ASTs matching pattern, returning
    list of (matched node, bindings)."""
    assert node is not None
    assert pat is not None
...
def count(node, pat, **kwargs):
    """Count all sub-ASTs matching pattern."""
...
def canonical_pure(node):
    ...
    # print('FUN', dump(fun))
    if type(node) == list:
        return NormalizePure().normalize_block([DesugarAugAssign().visit(stmt)
                                                for stmt in node])
    else:
