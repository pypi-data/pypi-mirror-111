...
class Goal:
    ...
    def __init__(
        ...
        """
        You must supply a description-tuple: the title of the goal, and
        a more detailed explanation that will be available if the user
        requests more information. The description-tuple may also
        include a third and/or fourth entry: these will be used instead
        of the first and second entry (respectively) when the Goal is
        being presented as part of graded feedback instead of as part of
        a blank rubric. This can be used to avoid showing exactly which
        tests are performed when the rubric is constructed, but include
        that information when feedback is given.
    ...
    def evaluate(self, base_context=None):
        ...
        """
        if self.test_in and len(self.test_in["contexts"]) > 0:
            credit = 0
            full_count = 0
            partial_count = 0
            notes = []
            ...
            # context pairs:
            explanations = {}
            for builder in self.test_in["contexts"]:
                # Construct context:
                ...
                try:
                    ...
                    err = html_tools.html_traceback("Unexpected Error:")
                    warnings.append(
                        "Context creation failed unexpectedly:<br>\n"
                      + html_tools.html_traceback('Error:')
                    )
...
class Rubric:
    ...
    """
    def __init__(self, goals, performance_metric):
        """
    ...
    def all_contexts(self):
        """
        Crawls our goals and their subgoals to find all relevant Context
        objects that might possibly be used by tests in this rubric.
        Returns a list in breadth-first traversal order of this rubric's
        goals, their contexts, and those contexts' dependencies.
        """
        # Map of object IDs
        idmap = {}
    ...
    def create_contexts_list(self, base_context=None):
        ...
        returned by list_and_render_contexts.
    ...
    def create_blank_report(self, task_info):
        ...
        The returned report is a dictionary with the following keys:
            ...
            summary: A description of the task that this rubric belongs
                to.
            table: A table (in the format returned by Goal.table)
                detailing each goal and subgoal.
            contexts: A list of context summary dictionaries in the
                format returned by list_and_render_contexts, which
                summarizes all contexts used by this rubric.
        """
        # Empty report:
        report = {
            "taskid": task_info["id"],
    ...
    def evaluate(self, task_info, username, submission_target):
        ...
        caches of any associated Context objects before it starts
        evaluating goals.

        The returned report dictionary has the following keys:

            taskid: The task ID (from the given taskspec)
            evaluation: A string summarizing the performance on the
                entire task (from the metric function).
            summary: An HTML string summarizing performance on the task
                (from the metric function).
            files: A list of dictionaries with 'filename' and 'code'
                slots containing the file names and raw code text of the
                submitted file(s).
            warnings: A list of warnings (from the metric function plus a
                few custom warnings if things are seriously wrong).
            table: A table (in the format returned by Goal.table)
                detailing each goal and subgoal (from the metric
                function).
            contexts: A list of context summary dictionaries in the
                format returned by list_and_render_contexts, which
                summarizes all contexts used by this rubric (see
                create_contexts_list).
        ...
        # TODO: Check for parse errors during normal context
        # construction phases...
        ...
        if os.path.isdir(submission_target):
            ...
            default_file = task_info["default_file"]
        ...
        # Build our files list based on FileContext objects
        all_filenames = set([base_context["default_file"]])
        for ctx in self.all_contexts():
            if isinstance(ctx, contexts.FileContext):
                if ctx.target_file is not None:
                    all_filenames.add(ctx.target_file)

        # Grab file contents
        for filename in all_filenames:
            full_path = os.path.join(submission_root, filename)
            if filename == base_context["default_file"]:
...
def foundational_core_extras_metric(goals, blank=False):
    ...
    feedback_row = {
        "description": ("Extra feedback (not graded):", ""),
    ...
    for gtype in GOAL_TYPE_RUBRICS:
        ...
        if blank:
            ...
            if row["status"] == "accomplished":
                ...
                row["explanation"] = "Accomplished all {} {} goals.".format(
                    "core and extra"
                        if extra_subrow["status"] == "accomplished"
                        else "core",
                    gtype
                )
            elif row["status"] == "partial":
                row["explanation"] = (
                    "Accomplished some of the core {} goals."
                ).format(gtype)
            else:
                row["explanation"] = (
                    "Did not accomplish the core {} goals."
                ).format(gtype)
...
class NoteGoal(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Only a description is required, but an explanation (shown only
        during feedback, but not on the rubric) may also be given. By
        default, the feedback_only tag will be applied.
        """
        tags = kwargs.setdefault("tags", [])
        if "feedback_only" not in tags:
            tags.append("feedback_only")
...
class JointGoal(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        You must provide a description, a list of parts (default empty
        list), and a number of parts required (default is the size of
        the given parts list).
...
class FailGoal(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Requires a just a subgoal, with optional description,
        explanations, and tags. The description should generally be
        phrased as the negation of the subgoal's description, and the
        default (if None is given explicitly) is to add "Do not " in
        front of the subgoal's description title and add " You need to
        avoid this." to the end of its details.
    ...
    def table(self, blank=False):
        """
        The table for a FailGoal is a copy of it's subgoal's table, with
        the status, description, and explanation from the FailGoal's
        result. This means that the FailGoal itself does not appear as a
        separate entry in rubric tables. Any tags for the FailGoal are
        added to the tags of the subgoal.
...
class PreconditionGoal(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        You must provide a description, a precondition goal, and a
        subgoal.
        """
        # Pre-specify goal type tag
        subgoal_types = set()
...
class ComparisonTest(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        In addition to a description and optional explanations and/or
        tags (see the Goal class), a checker function is needed, which
        should accept value and reference objects and return a goal
        result (a dictionary with status + explanation keys). The
        context_slot is used to determine which slot in the current
        context to check, and ref_slot specifies where to get the
        reference object, although if not given it will default to
        "ref_" + context_slot.
        ...
        if self.checker is None:
            ...
            try:
                ...
                self.set_explanation(
                    context,
                    status="crash",
                    default="Could not access '{}' for testing.".format(
                        self.context_slot
                    )
                )
                return self.result
            ...
            try:
                ...
                self.set_explanation(
                    ...
                    default=html_tools.html_traceback(
                        "Error while checking {}:".format(self.context_slot)
                    )
                )
...
class LintCheck(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        In addition to a description, a goal type may be supplied other
        than the default "style". "procedure" is the most likely
        alternative.
    ...
    def evaluate_in_context(self, context=None):
        ...
        try:
            ...
            self.set_explanation(
                ...
                status="crash",
                default=html_tools.html_traceback(
                    "Error while inspecting your code."
                )
            )
            return self.result
...
class ImplementationCheck(Goal):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        A description is required (see the `Goal` class). An appropriate
        `test_in` dictionary which will provide a "scope" slot is
        typically required. `ImplementationCheck` itself uses the
        following arguments:
            ...
            name: specifies a name for the construct being searched for.
              The plural will be constructed by adding 's', unless name
              is a tuple, in which case the first entry will be used as
              the singular and the second as the plural. May contain HTML
              code. If pattern is not a list, this can be left out, and
              the pattern will be used as the name.
            match: A function that accepts the entire submitted AST, the
              node being considered for a match right now, and the
              current binding environment. This function should return
              True or False, and any matches for which it does not return
            ...
            use/min/max: Either the 'use' argument, or one or both of the
              ...
              'max' is 0, the pattern is considered a negative pattern,
              and the goal will fail if any matches are found. Otherwise,
              the goal will succeed if the number of matches is between
              the given min and max values, inclusive. If none of these
              are given, the min defaults to 1 and the max to None (no
              limit).
            softmin/softmax: If one of these is true, the minimum (or
              maximum) restriction on the number of matches will be
              treated as a soft constraint, and if violated the goal will
              be treated as partially accomplished instead of failed. If
              they are exactly either the string "warn" or "note", then
              the goal will still count as fully accomplished if that
              constraint is violated, but a warning or note will be
              attached mentioning the unexpectedly low/high number of
              matches. They may also be integers or floats, in which case
              they establish an alternate min/max threshold for partial
              completion. For softmin, partial matches are counted as 0.5
              of a match towards achieving the threshold, but for softmax
            ...
            callees: If given as True, instead of simply searching within
              ...
              are called from within the given scope node.
              TODO: This is still (as of 2020-6) experimental/unstable.
            subrules: A list of ImplementationCheck goals to be tested
              within matches of this goal. Only matches where this goal
              and all of its subrules are accomplished (or partially
              accomplished) will be considered valid (respectively,
              partially valid). If this goal is a negative goal (max =
              0), it fails if there are any fully valid matches, and
              partial matches are ignored. On the other hand, if it is a
              positive goal (max != 0), it counts as accomplished if
              the number of fully valid matches is within the min and
              max limits (inclusive), and partially accomplished if the
              number of fully valid matches is below the min limit but
              the number of fully valid + partially valid matches is at
              least the min limit.
            match_identity: a function that returns a hashable object to
              represent the identity of a match for match-counting
              purposes. The function will be given the entire code
              context, the matching node, and a list of matching
              environments as input. It may return a list of identities
              instead of a single identity and each will be counted. By
              default this is a function which just returns the matching
              node, such that multiple matching environments based on the
              same node are not counted as separate matches. One
              reasonable alternative if you know what type of node you're
              matching would be to return some associated string (e.g.,
              the id of a Call node that has a Name as its func).
            subslip: A number of subgoals which are allowed to be
              violated and still count a potential match as a partial
              match. May be fractional, since partially-matched subgoals
              will be counted as 1/2 a point. By default this number is
              0: all subgoals must be fully accomplished within a match
              for that match to count as a valid match.
            normalize: default False; experimental mast option that tries to
              inline some local variable assignments into larger expressions
              for better matching.
            check_in_def: default False, this option changes the context
              within which the check occurs by default: the check will
              use the 'scope' element from the current context as usual,
              but will then assume that that AST node is a Call to a
              function defined in the same file (within the 'code'
              element) and will look up that definition, running the
              check in the context of that definition rather than in the
              original scope context given to it. This is useful for
              placing requirements on helper functions whose names aren't
              known ahead of time: a parent `ImplementationCheck` can be
              used to match the helper function call, with child checks
              using check_in_def that place requirements on the code in
              the helper function. The check will fail if the 'scope'
              context provided to it is not a Call node, or if it can't
              find the matching FunctionDef node in the 'code' tree of
    ...
    def evaluate_in_context(self, context=None):
        ...
        try:
            ...
            self.set_explanation(
                ...
                status="crash",
                default=html_tools.html_traceback(
                    "Error while checking implementation:"
                )
            )
            return self.result
        ...
        for (node, envs) in matches:
            for env in envs:
                ...
                for rule in self.subrules:
                    ...
                    # evaluate sub-rule
                    sub_result = rule.evaluate(this_sub_context)
...
class NoParseErrors(Goal):
    ...
    generates a warning for each parse error.
    """
    def __init__(
        self,
        description=(
        ...
        **kwargs
        ...
        """
        A default description is available. If generate_warnings is
        given as False, parse errors will not be turned into warnings,
        but in the default case, they will be.
        """
        super().__init__(description, **kwargs)
        self.set_default_goal_type("procedure")
...
#---------------------------#
# Specialized Goal subtypes #
#---------------------------#

class AllFunctionsHaveDocstrings(LintCheck):
    """
    A Goal which requires that all functions defined in the submitted
    module must have non-empty docstrings.
    """
    def __init__(self, exclude=None, **kwargs):
        """
        A list of function names to ignore may be given as `exclude`.
        All other keyword arguments are passed to the `LintCheck`
        constructor. If no description is specified, a default
    ...
    def check(self, context):
        ...
        """
        docmap = contexts.extract(context, "docstrings")
        empty_docstrings = []
        has_docstrings = []
...
class FunctionsArentNested(LintCheck):
    """
    A Goal which requires that no functions are defined inside other
    functions.
    """
    def __init__(self, exclude=None, **kwargs):
        """
        A list of function names to exclude from the check may be
        provided. These functions will be ignored if they are nested,
        and functions nested inside them will not be flagged.
        """
        self.exclude = exclude
    ...
    def check(self, context):
        ...
        """
        defsmap = contexts.extract(context, "defs")
        task_info = contexts.extract(context, "task_info")
...
class DoesntWasteFruit(LintCheck):
    """
    A linter that makes sure that any fruitful function or method calls
    get stored in variables or used as part of expressions. A fruitful
    function or method is one of:

        1. Defined in the submission itself with an interior return node
           that has an expression associated with it, which isn't inside
           a nested definition.
        2. One of the functions named in the `extra` list of strings, or
           a method named in that list with a '.' at the start.
    ...
    """
    def __init__(self, exclude=None, extra=None, **kwargs):
        """
        A list of strings specifying names of functions to exclude from this
        check may be given. The code in those functions won't be inspected
        for wasting fruit, but calls to those functions in other contexts
        will still be inspected if they're fruitful.
    ...
    def check(self, context):
        ...
        or statement.
        """
        scope = contexts.extract(context, "scope")
        task_info = context.extract(context, "task_info")
    ...
    def accumulate_function_and_method_calls(
        ...
        # Don't recurse if we're the definition of an excluded function
...
class DoesntWasteBoxes(LintCheck):
    """
    A lint checker function which looks for unused variables, excluding a
    list of strings (named functions won't be inspected at all and named
    ...
    other scopes.
    """
    def __init__(self, exclude=None, tolerance=2, **kwargs):
        """
        A list of strings specifying names of functions and/or variables
        to exclude from this check may be given. Excluded functions
        won't have their code inspected, and excluded variables won't be
        checked.
    ...
    def check(self, context):
        ...
        attended to).
        """
        node = contexts.extract(context, "scope")
        task_info = contexts.extract(context, "task_info")
