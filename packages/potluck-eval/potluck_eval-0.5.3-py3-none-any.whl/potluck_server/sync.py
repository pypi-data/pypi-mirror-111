...
# Bug thread of same behavior: https://bugzilla.redhat.com/show_bug.cgi?id=1717330
# Python bugfix: https://github.com/python/cpython/commit/e8a57b98ec8f2b161d4ad68ecc1433c9e3caad57
...
def init_sync(port=51723, key=None):
    ...
    'syncauth' must exist unless a key is given (should be a
    byte-string). Don't rely too heavily on the default port, since any
    two apps using sync with the default port on the same machine will
    collide.
    """
    ...
    while True:
        ...
        # Reduce multiprocessing's connection timeout value since we're pretty
        # sure we'll fail first and succeed later
        old_init_timeout = multiprocessing.connection._init_timeout
        multiprocessing.connection._init_timeout = lambda t=2.0: time.time() + t
        try:
