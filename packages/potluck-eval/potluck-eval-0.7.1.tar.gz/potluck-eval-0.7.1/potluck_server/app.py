...
To run the server, the current directory must contain `config.py`,
`secret`, and `syntauth` files; see `rundir` in the package directory for
an example. The `config.py` should be based on `config.py.example`, while
`secret` and `syncauth` are just secret keys and should be random bytes.
See `rundir/Makefile` for commands to create each. Note that the
`potluck` package must be installed, along with `flask`, `jinja2`, and
`flask_cas`.
...
# Set secret key from secret file (see rundir/Makefile):
