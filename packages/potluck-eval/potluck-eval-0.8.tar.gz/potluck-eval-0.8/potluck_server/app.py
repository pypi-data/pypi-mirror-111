...
To run the server, the current directory must contain
`ps_config.py`, `secret`, and `syntauth` files; see `rundir`
in the package directory for an example. Each file will be created if it
doesn't already exist:

- `ps_config.py` will be copied from `ps_config.py.example`, but if
  neither file exists, the server won't be able to start.
- `secret` and `syncauth` just contain secret keys; they'll be created
  automatically using `os.urandom` if they don't exist.

Note that the `potluck` package must be installed, along with `flask`,
`jinja2`, and `flask_cas`.
...
/<course>/<semester>/feedback/<username>/<phase>/<pset>/<task>
(route_feedback):
...
app = flask.Flask("potluck_server")

# Set up templating with a custom loader that loads templates from the
# potluck package if it can't find them in this package. This is how we
# share the report template between potluck_server and potluck.
...
try:
...
except Exception: # try copying the .example file?
    if os.path.exists('ps_config.py.example'):
        ...
        # for 2/3 compatibility instead of FileNotFoundError
        raise IOError(
            (
                "Neither 'ps_config.py' nor 'ps_config.py.example' is"
                "available; unable to find server configuration in run"
                "directory {}."
            ).format(os.getcwd())
        )

# Load potluck modules:
if app.config["IMPORT_POTLUCK_FROM"] is not None:
    IMPORT_DIR = os.path.join(
        os.getcwd(),
        app.config["IMPORT_POTLUCK_FROM"]
    )
    sys.path.insert(0, IMPORT_DIR)
else:
    IMPORT_DIR = None
import potluck.report # noqa: E402
potluck.report.setup() # default templates/resources directories
import potluck.html_tools # noqa: E402
import potluck.file_utils # noqa: E402

# Set secret key from secret file, or create a new secret key and write
...
def amend_task(course, semester, task_info, psid, task, username, phase):
    ...
    keys are added:
        ...
        "grade": A numerical grade value, derived from the evaluation via
            the EVALUATION_SCORES dictionary defined in the config file.
            If no grade has been assigned, it will be the string '?'
            instead of a number.
        ...
        "max_revision_score": A number representing the max score
            possible on a revision of this task (regardless of whether
            this is an initial or revised submission)
    """
    # Fetch basic info
