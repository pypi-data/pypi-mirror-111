...
def build_html_tabs(tabs):
    ...
    tab_pieces = [
        (
            """
<li
 class='tab{selected}' onclick='toggleTab();'
 tabindex="0"
...
def html_link_to_line(taskid, filename, lineno):
    ...
    """
    blockid = block_id(taskid, filename)
    lineid = line_id(taskid, filename, lineno)
    focus_call = "focusCodeLine('{blockid}', '{lineid}'); return true;"\
        .format(blockid=blockid, lineid=lineid)
    return (
        '<a class="lineref" href="#{lineid}"'
        ' onclick="{focus_call}">{lineno}</a>'
    ).format(
        lineid=lineid,
        focus_call=focus_call,
        lineno=lineno
    )
