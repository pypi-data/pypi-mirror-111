...
# TODO: This should be standard procedure!
...
def register_goal_provider(provider):
    """
    Registers a goal-provider (must have a zero-argument `provide_goal`
...
class HasPayload:
    ...
    def __init__(
        ...
        to the payload constructor (varies by subclass).
    ...
    def synthesize_payload_info(self, group=None):
        ...
        augmentations.update(self.default_augmentations)
        if group:
            args.update(group.payload_args)
            augmentations.update(group.augmentations)
        args.update(self.payload_args)
        augmentations.update(self.augmentations)
    ...
    def construct_payload(self, parent=None):
        ...
        result = self.payload_constructor(**args)
    ...
    def describe_payload(self, parent=None, obfuscated=False):
        """
        Returns an HTML description of the payload that will be
        constructed by `construct_payload` (with an equivalent `parent`
        argument). If `obfuscated` is set to True, the obfuscated
        version of the description will be provided (see
    ...
    def ensure_payload_constructor_arg(self, desired):
        ...
        """
        cobj = self.payload_constructor.__init__.__code__
    ...
    def ignore_output(self):
        ...
        del self.default_augmentations["capturing_printed_output"]
        del self.augmentations["capturing_printed_output"]
    ...
    def use_harness(self, harness_fn):
        ...
        if self.payload_constructor != harness.create_run_function_payload:
            raise TypeError(
                ...
                f" normally based on a function call ({self} is not)."
            )
    ...
    def do_setup(self, setup_fn):
        ...
        Note: `do_setup` and `do_cleanup` each replace any previous
        setup/cleanup function, rather than adding additional
        setup/cleanup steps. If you want multiple setup/cleanup steps,
        compose the steps yourself into single setup/cleanup functions.
        TODO: Better option for multiple setup/cleanup functions
        """
        self.augmentations["with_setup"] = { "setup": setup_fn }
        return self
    ...
    def do_cleanup(self, cleanup_fn):
        ...
        See `potluck.harness.with_setup` for details.
        """
        self.augmentations["with_cleanup"] = { "cleanup": cleanup_fn }
        return self

    def capture_output(self, capture_errors=False):
        """
        ...
        instead of actually breaking the test. Returns self for chaining.
        """
    ...
    def use_decorations(self, decorations, ignore_missing=False):
        ...
        function names to decorator functions that will be temporarily
        applied to those functions during testing. This has a lot of
        potential uses, like disabling a function ('decorate' it to
        return a new do-nothing function) or performing some operation on
        the result of the test function before comparison happens (by
        decorating the function being tested).

        If `ignore_missing` is set to True instead of the default
        (False), then if a decoration is supposed to apply to a function
...
class HasContext:
    ...
    def create_context(
        ...
        if hasattr(self, "describe_payload"):
            obfuscated = self.describe_payload(group, True)
            clear = self.describe_payload(group, False)
            if "description" not in args: # sad but unlikely default
                args["description"] = (
                    "An unknown test",
                    "We will run some kind of test.",
                    "An unknown test",
                    "We ran some kind of test."
                )
            # If we've only got a topic (shouldn't happen), double it
            if len(args["description"]) == 1:
                args["description"] += args["decription"]

            # If we're missing feedback versions, copy them from bases
            while len(args["description"]) < 4:
                args["descripton"] += ( args["description"][-2], )

            args["description"] = (
                args["description"][0],
                args["description"][1] + '<br>\n' + obfuscated,
                args["description"][2],
                args["description"][3] + '<br>\n' + clear
            )
    ...
    def describe_module_slot(self):
        ...
        self.default_context_args["description"] = (
            "The result of running your submission.",
            (
                "We will run your entire submitted file and test"
                " the module it creates."
            ),
            "The result of running your submission.",
            (
                "We ran your entire submitted file and captured"
                " the module it created."
            )
        )
...
class HasGoal:
    ...
    def __init__(
        ...
        """
        A goal constructor must be specified. Default goal args may be
        provided. Set the goal category and/or type via goal args.
        """
    ...
    def test_module(self, module_comparator):
        ...
        self.default_goal_args["description"] = (
            "Correct result from running your file.",
            (
                "Your file must do the same thing as the solution file"
                " when run."
            )
        )
    ...
    def test_output(self, capture_errors=False):
        ...
        if hasattr(self, "fn_name"):
            if self.fn_name == "__import__":
                self.default_goal_args["description"] = (
                    f"Your file prints the correct output",
                    (
                        f"The output printed when your entire file is"
                        f" run must match the solution output."
                    )
                )
            else:
                self.default_goal_args["description"] = (
                    f"<code>{self.fn_name}</code> prints the correct output",
                    (
                        f"The output printed when your"
                        f" <code>{self.fn_name}</code> function is run must"
                        f" match the solution output."
                    )
                )
        else:
            self.default_goal_args["description"] = (
    ...
    def test_with_harness(self, harness_fn):
        ...
        """
        if (
            not hasattr(self, "fn_name")
         or not hasattr(self, "use_harness")
        ):
            raise TypeError(
                f"Harness-based testing is only applicable for"
                f" Goal-generators which have a fn_name attribute and a"
                f" use_harness method ({self} does not)."
            )
    ...
    def test_trace(self, trace_targets, state_function):
        ...
        if (
         ...
         or not hasattr(self, "capture_trace")
            ...
            raise TypeError(
                ...
                f" and an fn_name attribute ({self} does not)."
            )
        ...
        self.goal_args["tags"]["goal_type"] = "process"

        # Set default (not explicit) description
        self.default_goal_args["description"] = (
            f"<code>{self.fn_name}</code> uses the correct process",
            (
    ...
    def check_trace_state(
        ...
        included as identifying information for trace entries).
        ...
        if hasattr(self, "fn_name"):
            self.default_goal_args["description"] = (
                (
                    f"<code>{self.fn_name}</code> makes the correct"
                    f" function calls"
                ),
                (
                    f"Your <code>{self.fn_name}</code> function must"
                  + f" call {targets} "
                  + ', '.join(conditions)
                )
            )
    ...
    def check_invariant(
        ...
        of tolerance for small changes. TODO: that, but more elegantly!
    ...
    def compare_using(self, comparator_fn):
        """
        Specifies an alternate comparator for this goal (only works for
        `potluck.rubrics.ComparisonTest` as the `goal_constructor`).
...
class TestCase(HasPayload, HasContext):
    """
    A `TestCase` is a single unit test, with a basic setup by default.
    Different behavior may be achieved by calling various specialization
    methods (for example, to specify stdin contents). The mere
    instantiation of a `TestCase` object adds it to the test registry
    which means it will appear on the rubric created by `rubric`.

    If you pass the special string '__import__' as the function name to
    test, instead of testing a specific function from the submitted
    module, the test imports the entire module. This changes various
    defaults (for example, the default test will be of printed output
    rather than return value).

    Most modification methods chain by returning the `TestCase` object.

    In terms of the rubric constructed by the `rubric` function, a
    `TestCase` is actually a placeholder for an
    ...
    `TestCase` is created, but which will be associated with multiple
    `TestCase` objects that share the same target function and group
    name.

    If you want to separate tests that share a function name into
    multiple goals, specify distinct `goal_name` values for the different
    `TestCase` objects you create.
    """
    def __init__(
        self,
        fn_name,
        ...
        keyword arguments may also be supplied. Supply '__import__' as
        the function name to create a test of the behavior of an entire
        module.
        ...
        things a bit more forgiving). The groups are also used when
        specifying a custom description (see `set_description`). The
        group name is thus arbitrary; the default group name is '_'.
        """
        args = args or []
        kwargs = kwargs or {}
        self.args = args
        ...
        # User-facing representation of the arguments we used
        self.args_repr = html_tools.args_repr_list(args, kwargs)

        self.group_name = group_name
        self.fn_name = fn_name

        if self.fn_name == "__import__":
            # Set up module import payload
            HasPayload.__init__(
                self,
            ...
            # Context construction setup
            HasContext.__init__(
                self,
                default_context_args={
                    # builder will be added elsehow
                    "description": (
                        "The output of running your submission.",
                        (
                            "We will run your entire submitted file and"
                            " record what gets printed."
                        ),
                        "The output of running your submission.",
                        (
                            "We ran your submitted file and recorded"
                            " what it printed."
                        )
                    ),
                    "display_product": (
                    ...
                    "depends": contexts.auto("filename", "file_path"),
                }
            )

        else: # not a module import test
            # Set up our payload
            HasPayload.__init__(
                self,
                payload_constructor=harness.create_run_function_payload,
            ...
            HasContext.__init__(
                ...
                default_context_args={
                    # builder will be added elsehow
                    "description": (
                        f"The result of <code>{fn_name}</code>",
                        (
                            f"We will run your <code>{fn_name}</code>"
                            f" function and compare the return value"
                            f" against the solution result."
                        ),
                        f"The result of <code>{fn_name}</code>",
                        (
                            f"We ran your <code>{fn_name}</code>"
                          + " function and compared the return value"
                          + " against the solution result. We ran your"
                    ...
                    # Capture auto-filename at instantiation time
                    "depends": contexts.auto("module"),
                }
            )

        # Register this test in the appropriate TestGroup, creating a
...
class Check:
    ...
    def __init__(
        ...
        """
        A check needs a list of patterns (could be length-one), a tuple
        of limits (min/max, either or even both may be None), and it may
        specify a name instead of deriving one automatically from the
        patterns (see `potluck.rubrics.ImplementationCheck`), a category
        string (e.g., 'extra' instead of defaulting to 'auto'), and/or a
        goal type (e.g., 'style' instead of defaulting to 'auto'). All
        other `potluck.rubrics.ImplementationCheck` details are specified
        via calling methods on the object after it is created. The name
        may be a string containing HTML code, or a 2-item tuple to
        explicitly specify singular and plural forms of the name
        (otherwise an 's' will be added to the end of the name to
        generate the plural form).
        """
    ...
    def build_implementation_checks(
        ...
        The prefix argument specifies a prefix to add to the description
        details to help keep things specific. A prefix will be
        automatically added to the calls to
        `build_implementation_checks` for any sub-rules, which will
        include the existing prefix.
...
class Import(Check):
    ...
    def __init__(
        ...
        what = f"the {mname} module"

        if import_names == '*':
            patterns = [ f"from {mname} import *" ]
            what = f"everything from the {mname} module"
            ...
            patterns = [
                ...
                # TODO: Test this: does mast allow ___ there?
                # TODO: Does this match 'as' imports?
            ]
            ...
            what = f"{names_desc} from the {mname} module"
...
class FunctionDef(Check):
    ...
    """
    def __init__(self, fn_name, args_spec=None, category='auto'):
        """
        You must specify the function name, and you may specify the
        arguments. If given, `args_spec` should be a string containing
        ...
        If `args_spec` is omitted, any function signature with the
        ...
        def_patterns = patterns.function_def_patterns(fn_name, args_spec)
...
class FunctionCall(Check):
    ...
    def __init__(
        ...
        The `limits` argument specifies the lower and upper limits on the
        number of calls required. Use `None` in the first (or second)
        position to specify no lower (or upper) limit. The default value
        is `[1, None]` which means "at least one."

        The `args_spec` can be used to require a certain arrangement of
        arguments (see `potluck.patterns.function_call_patterns`) and may
        be a string, a list of strings, or a pair of integers and/or None
        similar to 'limits' specifying how many positional arguments
        there should be (one element of the pair must be an integer for
        this to work). The `announce` argument can be used to override
        the name of the function in the default description, although a
        custom description could also be applied using the
        `set_description` method. Unlike a custom description, an
        `announce` value is also used in the construction of explanation
        strings.
        ...
        and len(args_spec) == 2
        and (isinstance(args_spec[0], int) or isinstance(args_spec[1], int))
        ):
            ...
            if args_limits[0] is None: # upper limit only
                ...
                args_desc = f"<{args_limits[0]}-{args_limits[1]} arguments>"
...
class Loop(Check):
    ...
    def __init__(
        ...
        super().__init__(loop_patterns, limits, name, category=category)
...
class Return(Check):
    ...
    def __init__(
        ...
        super().__init__(patterns, limits, name, category=category)
...
def group(fn_name, group_name="_", create=False):
    """
    Retrieves a `TestGroup` object for a particular group of tests,
    identified by the name of the function being tested and the group
    ...
    if result is None:
        if not create:
            raise KeyError(
                f"There are no tests for function '{fn_name}' in group"
            ...
            result = (
                TEST_GROUP_REGISTRY
                  ...
                  .setdefault(fn_name, {})
                  .setdefault(group_name, TestGroup(group_name, fn_name))
            )
...
class TestGroup(HasPayload, HasContext, HasGoal):
    ...
    def __init__(self, name, fn_name):
        """
        A group collects tests of a single function associated with a
        particular group name. It starts out empty but goals are added to
        ...
        HasGoal.__init__(
            ...
            default_goal_args={
                "description": (
                    f"<code>{fn_name}</code> returns the correct result",
                    (
                        f"The result returned when your"
                        f" <code>{fn_name}</code> function is run must"
...
class GroupClone(TestGroup):
    ...
    def __init__(self, parent):
        ...
        HasPayload.__init__(
            ...
            parent.default_payload_args,
            parent.default_augmentations
        )
        # Copy explicit args as well
        self.payload_args = parent.payload_args

        # Clone context info from parent
        HasContext.__init__(
            self,
            parent.default_context_args
        )
        # Copy explicit args as well
        self.context_args = parent.context_args

        # Clone goal info from parent
        HasGoal.__init__(
            self,
            parent.goal_constructor,
            parent.default_goal_args
        )
        # Copy explicit args as well
        self.goal_args = parent.goal_args
...
class RefinedTest(HasContext, HasGoal):
    ...
    the context(s) used for the parent object. Any `HasGoal` subclass can
    support refinement (and `HasGoal.refine` is the proper way to
    instantiate refined tests).

    Subclasses should override the `build_context` method to define what
    kind of additional processing they want to do to the contexts of the
    parent goal. This method needs to accept a context as its only
    argument (besides self) and return a dictionary of any new context
    slots it creates/updates, just like all context builder functions.
    ...
    def __init__(
        ...
        **kwargs
        ...
        Behavior may be controlled by the following keyword arguments:
            ...
            `pattern_desc`: A string that will be used to provide
                automatic context and goal descriptions. Use
                `set_context_description` and/or `set_goal_description`
                if this isn't expressive enough.
            `first_match`: If set to False, the result will be a
                (possibly empty) list of all matches. If set to True
                (the default) only the first match is used, and None is
                used if there are no matches.
            `match_details`: If True, the result will take the form of
                one or more re.Match objects instead of strings.
            `refine_ref`: If True, the "ref_" context slot that matches
                the target context slot will be refined in addition to
                the base slot.
...
def NoParseErrors(category="core"):
    ...
    """
    register_goal(
        rubrics.NoParseErrors(tags={ "category": category })
    )
...
def DontNestFunctions(exclude=None, category='core', description=None):
    ...
    register_goal(rubrics.FunctionsArentNested(**args))
...
def DontWasteFruit(
    ...
    register_goal(rubrics.DoesntWasteFruit(**args))
...
def DontWasteBoxes(
    ...
    register_goal(rubrics.DoesntWasteBoxes(**args))
...
def rubric():
    ...
    # Result
    return rubrics.Rubric(all_goals, rubrics.core_extras_categorized_metric)
