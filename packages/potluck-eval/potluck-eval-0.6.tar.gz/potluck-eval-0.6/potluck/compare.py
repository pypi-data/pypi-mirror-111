...
def strict_equality_checker(val, ref):
    """
...
def numbers_are_equalish(val, ref):
    ...
    if cmath.isclose(
        ...
        return {
            "status": "failed",
            "explanation": (
                "Value {val} does not match expected value {ref} to the"
                " required precision."
            )
        }
...
def very_fuzzy_string_compare(
    ...
    if val_folded == ref_folded: # full equality ignoring case
        return {
            "status": "accomplished",
            "explanation": "Strings are exactly the same.<br>\n{}".format(
                html_tools.html_output_details(val, "Value tested")
            )
        }
    elif val_compressed == ref_compressed: # equal when compressed
...
def make_hashable(dataStructure, memo=None):
    """
    Turns a data structure that may contain lists, tuples, dictionaries,
    ...
    # If this is a re-visit, return a stub string
    if id(dataStructure) in memo:
    ...
    if val == ref:
        ...
        and isinstance(ref, (int, float, complex))
            ...
            return {
                "status": "failed",
                "explanation": (
                    f"Your result was {phrasing.a_an(type(val))},"
                    f" but the correct result was"
                    f" {phrasing.a_an(type(ref))}."
                )
            }
            ...
            result = OMNI_ORDERED_COMP(val, ref)
            return result
        elif isinstance(val, (set, dict)): # both sets or dicts
            # TODO: More detailed explication of deep differences here?
            return OMNI_UNORDERED_COMP(val, ref)
...
OMNI_ORDERED_COMP = make_structure_comparator(
    order_matters=True,
    item_matcher=omni_compare
)
"""
Structure comparator that cares about ordering which uses omni_compare
to match items.
"""

OMNI_UNORDERED_COMP = make_structure_comparator(
    order_matters=False,
    item_matcher=omni_compare
)
"""
Structure comparator that doesn't care about ordering which uses
omni_compare to match items.
"""
