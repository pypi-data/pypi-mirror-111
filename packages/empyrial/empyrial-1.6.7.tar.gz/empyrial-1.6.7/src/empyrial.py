...
class Engine:
  ...
  def __init__(self,start_date, portfolio, weights=None, rebalance=None, benchmark=['SPY'], end_date=today, optimizer=None, max_vol=0.15, diversification=1,confidences=None, view=None, min_weights=None, max_weights=None):
    ...
    if self.rebalance!=None and self.optimizer !="BL":
      self.rebalance = make_rebalance(self.start_date, self.end_date, self.optimizer, self.portfolio, self.rebalance, self.max_vol, self.weights)

    if self.rebalance!=None and self.optimizer=="BL" and self.optimizer==None:
      self.rebalance = make_rebalance(self.start_date, self.end_date, False, self.portfolio, self.rebalance, self.max_vol, self.weights)
...
#--------------------------------------------------------------------------------
def make_rebalance(start_date, end_date, optimizer, portfolio_input, rebalance, maxvol,allocation):
