...
def list(
    ...
    table = AsciiTable([
        ['NUMBER', 'NAME', 'STATUS', 'CREATED', 'IMAGE', 'RESOURCE', 'START COMMAND', 'MESSAGE'],
        *[[e.number, e.name, e.status, parse_time_to_ago(e.created_dt),  short_string(e.kernel_image.name, 25),
           parse_resource_spec(e.kernel_resource_spec), f'"{short_string(e.start_command, 25)}"',
...
def run(
    ...
    organization_name: str = organization_name_option,
    project_name: str = project_name_option,
    message: str = typer.Option(None, "--message", "-m", callback=message_callback, help="Experiment message"),
