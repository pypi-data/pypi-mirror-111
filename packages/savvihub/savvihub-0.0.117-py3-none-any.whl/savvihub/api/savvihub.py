...
class SavviHubClient:
    ...
    def volume_file_list(
        self, volume_id, dataset_version='latest', path='', recursive=False, need_download_url=False,
    ...
    def volume_file_read(self, volume_id, path, dataset_version) -> ResponseFileMetadata:
        return self.client.volume_file_read_api(volume_id=volume_id, path=path, dataset_version=dataset_version)

    def volume_file_copy(self, volume_id, source_path, source_dataset_version, dest_path, recursive=False) -> VolumeVolumeFileCopyResponse:
    ...
    def dataset_version_read(self, volume_id, dataset_version_name) -> ResponseDatasetVersion:
        return self.client.dataset_version_read_api(volume_id=volume_id, dataset_version_name=dataset_version_name)
    ...
    def sweep_create(
        ...
        max_experiment_count, max_failed_experiment_count,
    ) -> ResponseSweepInfo:
    ...
    def sweep_run(
        self, organization, project, sweep_name, image_url, resource_spec_id, resource_spec, cluster_name,
        volume_mounts, env_vars, start_command,
    ) -> ResponseSweepInfo:
        return self.client.sweep_run_api(
            organization_name=organization,
            project_name=project,
            sweep=sweep_name,
            sweep_run_api_payload=SweepRunAPIPayload(
                image_url=image_url,
    ...
    def dataset_read(self, organization, dataset) -> ResponseDatasetInfo:
        try:
            return self.client.dataset_read_api(organization_name=organization, dataset_name=dataset)
        except NotFoundAPIException:
            raise NotFoundAPIException(f'Dataset `{dataset}` is not in the organization `{organization}`.')

    def dataset_create(self, organization, name, is_public, description) -> ResponseDatasetInfo:
    ...
    def dataset_s3_create(self, organization, name, is_public, description, s3_path, aws_role_arn) -> ResponseDatasetInfo:
