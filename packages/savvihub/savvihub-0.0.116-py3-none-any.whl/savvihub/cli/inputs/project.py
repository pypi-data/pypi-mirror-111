...
def _project_name_callback(ctx: Context, project_name: str, project_type_filter: str = None) -> str:
        ...
        if project_name:
            ...
            if len(matched_projects) == 1:
                ...
                ctx.project = inquirer.prompt([inquirer.List(
                    ...
                    choices=[(p.name, p) for p in matched_projects if project_type_filter is None or p.type == project_type_filter],
                ...
                ctx.project = inquirer.prompt([inquirer.List(
                    'project',
                    message='Select project',
                    choices=[(p.name, p) for p in projects if project_type_filter is None or p.type == project_type_filter],
                )], raise_keyboard_interrupt=True).get('project')
...
def project_name_callback(ctx: Context, project_name: str):
    return _project_name_callback(ctx, project_name)


def version_control_project_name_callback(ctx: Context, project_name: str):
    return _project_name_callback(ctx, project_name, project_type_filter='version-control')
...
def local_project_callback(ctx: Context, local_project_url: str) -> Optional[str]:
    ...
    if local_project_url:
        ...
        try:
            file = ctx.authenticated_client.volume_file_read(volume_id, path, dataset_version)
        except NotFoundAPIException:
...
version_control_project_name_option = typer.Option(None, '--project', callback=version_control_project_name_callback,
                                                   help='If not present, uses git repository name of the current directory.')
