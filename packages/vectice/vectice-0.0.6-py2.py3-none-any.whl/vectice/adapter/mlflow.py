...
class MlflowAdapter(Adapter):
    ...
    # Fluent usage
    def create_run(self, name: str, job_type: Optional[str] = None) -> RunnableJob:
        mlflow.set_experiment(name)
        return super().create_run(name, job_type)
    ...
    def save_run(
        ...
        if isinstance(run, mlflow.ActiveRun):
    ...
    def __save_experiment(self, experiment: MlflowExperiment) -> None:
        ...
        for run in runs:
            if run.data.tags[VECTICE_RUN_TAG] is None:
                self.__save_run(run)
            else:
                logging.info(f"cancelling save of run {run.id} as it is already saved in vectice")
    ...
    def _extract_outputs(self, mlflow_run: MlflowRun, experiment: MlflowExperiment) -> List[Artifact]:
        ...
        result = (
            ...
            .with_user_version(mlflow_run.info.run_id, mlflow_run.info.run_id)
