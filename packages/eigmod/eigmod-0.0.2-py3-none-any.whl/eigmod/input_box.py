...
class InputBox:
    def __init__(self,  x, y, width, height, input_color_active =(235,235,235) , input_color_passive = (130, 130, 130), 
                        ...
                        anfangs_anzeige_text = "", vergroserungs_richtung = "beide", font_size_height_rel = 0.8,
        ...
        self.input_rect = pygame.Rect(x - self.width /2, y - self.height/2 , self.width, self.height)
        ...
        self.anfangs_anzeige_text_list = self.get_str_or_list(anfangs_anzeige_text)
    ...
    def set_input_font_sizes(self):
        ...
        if self.font_path:
            ...
            if self.input_font_size:
                self.input_font = pygame.font.Font(font_name, self.input_font_size)
            else:
                input_font_height = int(self.height * self.font_size_height_rel)
                self.input_font = pygame.font.Font(font_name, input_font_height)
    
            if self.input_max_len_font_size:
                self.input_max_len_font = pygame.font.Font(font_name, self.input_max_len_font_size)
            else:
                input_max_font_height = int(input_font_height * self.font_size_height_rel)
                self.input_max_len_font = pygame.font.Font(font_name, input_max_font_height)
        else:
            font_name = "comicsans"

            if self.input_font_size:
                self.input_font = pygame.font.SysFont(font_name, self.input_font_size)
            else:
                input_font_height = int(self.height * self.font_size_height_rel)
                self.input_font = pygame.font.SysFont(font_name, input_font_height)

            if self.input_max_len_font_size:
                self.input_max_len_font = pygame.font.SysFont(font_name, self.input_max_len_font_size)
            else:
                input_max_font_height = int(input_font_height * self.font_size_height_rel)
                self.input_max_len_font = pygame.font.SysFont(font_name, input_max_font_height)

        self.input_font_test_width, self.input_font_test_height = self.input_font.size("A")
    ...
    def update(self):
        ...
        if self.mit_zeilen_umbruch and self.schonmal_gezeichnet:
            self.t_problem_zeile = self.input_font.render(self.input_text_list[self.input_text_index], True, (self.input_color_active))
            if self.t_problem_zeile.get_width() > self.width * self.zeilen_umbruch_rel:
                ...
                while self.t_problem_zeile.get_width() > self.width * self.zeilen_umbruch_rel:
                    ...
                    if len(self.input_text_list[problem_index].split(" ")) > 1:
                        last_characters = self.input_text_list[problem_index].split(" ")[-1]
                        split_index = len(self.input_text_list[problem_index]) - 1 - len(last_characters)
                    else:
                    ...
                    if len(self.input_text_list) - 1 == problem_index:
                        ...
                        self.input_text_list[self.input_text_index] = last_characters + self.input_text_list[self.input_text_index]

                    self.t_problem_zeile = self.input_font.render(problem_zeile, True, (self.input_color_active))
            ...
            if self.input_active:
                ...
                # if normal character is pressed
                    ...
                    if self.input_max_len_rendered:
                        text_width = self.input_font.render(self.input_text_list[self.input_text_index], True, (0,0,0)).get_width()
    ...
    def draw(self, win):
        ...
        if self.umrandung == "rect":
            ...
            pygame.draw.line(win, self.input_color, (self.input_rect.x, self.input_rect.y + self.input_rect.height), (self.input_rect.x + self.width, self.input_rect.y + self.input_rect.height))
        ...
        if self.input_text_ist_max_len and self.max_len_info_anzeigen:
            t_input = self.input_max_len_font.render(f"Maximale Länge Erreicht!", True, (255, 255, 255))
            win.blit(t_input, (self.anfangs_x - t_input.get_width()//2, self.input_rect.y + self.input_rect.height + t_input.get_height()//2))
