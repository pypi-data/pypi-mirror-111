...
.. [1] Jeewajee A, Barry C, Douchamps V, Manson D, Lever C, Burgess N.
    ...
    def performRegression(
        ...
        posD, runD = self.getPosProps(
            labels, peaksXY, laserEvents=laserEvents,
            plot=True)
    ...
    def getPosProps(self, labels, peaksXY,
        ...
        k = signal.boxcar(
            self.speed_smoothing_window_len) / float(
        ...
        filtLen = np.squeeze(np.floor((
            runEndIdx - runStartIdx + 1) * self.ifr_smoothing_constant))
        ...
        if plot:
            ...
            ax.plot(
                xy[0, fieldLabel > 0], xy[1, fieldLabel > 0], 'y.')
    ...
    def getThetaProps(self, **kwargs):
        ...
        spkEEGIdx = np.ceil(
            spikeTS * self.lfp_sample_rate).astype(int) - 1
    ...
    def getSpikeProps(self, runLabel, meanDir,
                      durationInPosBins):
