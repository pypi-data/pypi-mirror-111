...
from ntk.widgets.frame import Frame
from ntk.widgets.panedwindow import PanedWindow
from ntk.widgets.image import ImageFile
...
class SelectBox(Entry):
    ...
    def __init__(self,
        ...
        self.frame_master = PanedWindow(
            root, width=0, height=0, bg=boxbg, row=kwargs.get('row', 0), bd=1, padx=kwargs.get('padx', 0),
            column=kwargs.get('column', 0), orient='horizontal', sashwidth=0, pady=kwargs.get('pady', 0))

        image_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), "arrow.png")
        image = ImageFile(image_file).main

        kwargs['height'] = 1

        super(SelectBox, self).__init__(self.frame_master,
                                        bg=bg,
        ...
        self.select_icon = Button(
            self.frame_master, image=image, text='', compound='right',
            anchor='e', width=0, height=0, bg=bg, hoverbg=bg, abg=bg, justify='right')

        self.frame_master.add(self)
        self.frame_master.add(self.select_icon)
        ...
        self.select_icon.bind("<Button-1>", lambda e: self.show_selection(e))
        ...
        if default:
            ...
            if type(default) == bool:
                ...
                default = (values[0]) if len(values) > 0 else "Please select item"
    ...
    def show_selection(self, e=None, show=False, values=False):
        ...
        if not self.list:
            ...
            rx = self.frame_master.winfo_rootx()

            # set ry as select.winfo_rooty

            ry = self.frame_master.winfo_rooty()

            # set rw as select.winfo_width

            rw = self.frame_master.winfo_width()

            # set rh as select.winfo_height

            rh = self.frame_master.winfo_height()
