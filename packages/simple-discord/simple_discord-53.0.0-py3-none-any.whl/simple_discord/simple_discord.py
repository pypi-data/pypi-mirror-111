...
class simple_discord_bot:
    def add_command(self, command_to_add, response=None, react_to=None, reaction=None, delete=False, connect_to_voicechannel=False, source_to_play=None, DM=False, disconnect_from_voice_channel=False, function=None):
        if response == None and function == None:
            raise ValueError("Either the response or function parmameters must be supplied")
        if response != None and function != None:
            raise ValueError("response and function parameters can't be both supplied, only one can")
...
async def on_message(message):
    ...
    for c in command_list:
        if str(message.content).startswith(c):
            ...
            command_function = command_dict[c]["function"]
            
            if command_function != None:
                await command_function(message)

            bot_response = command_dict[c]["response"]

            if bot_response != None:
                bot_message = await message.channel.send(bot_response)
