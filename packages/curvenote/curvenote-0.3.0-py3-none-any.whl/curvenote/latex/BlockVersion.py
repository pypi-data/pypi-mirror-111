...
def skipping_msg(thing: str, version_id: VersionID):
    return f"Caught error trying to localize {thing} for block {str(version_id)}, skipping"
...
class LatexBlockVersion:
    ...
    def __init__(self,session: Session, assets_folder: str, reference_list: List[LocalMarker],
        figure_list: List[LocalMarker], version: BlockVersion):
    ...
    def _localize_images_in_content(self, content: str):
        try:
            return utils.localize_images_from_content_block(self.session, self.assets_folder, self.figure_list, content)
    ...
    def _localize_references_in_content(self, content: str):
        try:
            return utils.localize_references_from_content_block(self.session, self.reference_list, content)
        except Exception as err: # pylint: disable=broad-except
