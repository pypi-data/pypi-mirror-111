...
class DefBuilder():
    ...
    def compose(self, template_options: TemplateOptions):
        ...
        for name, schema_option in self.schema_options.items():
            ...
            if name in template_options.schema:
                ...
                logging.info("%s - using schema default option: %s", name, schema_option.default)
            ...
            if len(defs.setup) > 0:
                for setup in defs.setup:
                    full_path = get_def_template_file_path(
                        template_options.template_location, setup
                        ) if defs.in_template else get_def_file_path(setup)
    ...
    def write(self, target_path: str, passopts: str, packages: str, setup: str):
        with open(os.path.join(target_path, 'curvenote.passopts.def'), 'w') as defs_file:
            defs_file.write(passopts)
        with open(os.path.join(target_path, 'curvenote.packages.def'), 'w') as defs_file:
