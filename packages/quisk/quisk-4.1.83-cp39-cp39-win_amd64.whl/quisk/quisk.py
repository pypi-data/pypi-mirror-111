...
# All QUISK software is Copyright (C) 2006-2020 by James C. Ahlstrom.
...
class ConfigStatus(wx.ScrolledWindow):
  ...
  def MakeBitmap(self):
    ...
    if conf.config_file_exists:
      ...
      cfile = "Quisk user files are in %s" % application.QuiskFilesDir
...
class ErrorWindow(wx.PyOnDemandOutputWindow):
  destroy_on_exit = True
  def OnCloseWindow(self, event):
    if self.destroy_on_exit and application.main_frame:
      application.main_frame.Destroy()
    return wx.PyOnDemandOutputWindow.OnCloseWindow(self, event)
...
class App(wx.App):
  ...
  def OnInit(self):
    try:
      self.MyOnInit()
      ErrorWindow.destroy_on_exit = False
      return True
    except SystemExit:
      pass
    except:
