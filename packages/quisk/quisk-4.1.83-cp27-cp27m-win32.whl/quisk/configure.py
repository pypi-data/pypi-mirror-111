    ...
    if ok:
      ...
      if self.radio_name == Settings[1]:	# changed for current radio
        if name in ('hot_key_ptt_toggle', 'hot_key_ptt_if_hidden', 'keyupDelay', 'cwTone', 'pulse_audio_verbose_output',
                    'start_cw_delay', 'start_ssb_delay', 'maximum_tx_secs'):
          setattr(conf, name, x)
...
"If you use a transverter, you need to tune your hardware to a frequency lower than\
 ...
 Seven filter bits are sent, and the eighth (most significant) bit is the antenna selection.\
 The Rx and Tx bits are always sent as the "Alex" filters (Protocol 1, address 9).\
 They are also sent to the J16 interface (open collector outputs), and in this case\
