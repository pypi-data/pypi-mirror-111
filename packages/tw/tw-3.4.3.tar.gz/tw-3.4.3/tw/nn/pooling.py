...
class _ROIPool(Function):
  ...
  def backward(ctx, grad_output):
    ...
    spatial_scale = ctx.spatial_scale
    bs, ch, h, w = ctx.input_shape
    grad_input = _C.roi_pool_backward(grad_output, inputs, rois, argmax, spatial_scale, output_size[0], output_size[1], bs, ch, h, w)
...
class AtrousSpatialPyramidPooling(nn.Module):
  ...
  def __init__(self, in_channels, out_channels=256, output_stride=None, batchnorm=nn.BatchNorm2d):
    ...
    self.aspp1 = _ASPPModule(in_channels, out_channels, kernel_size=1, padding=0, dilation=dilations[0], batchnorm=batchnorm)
    self.aspp2 = _ASPPModule(in_channels, out_channels, kernel_size=3, padding=dilations[1], dilation=dilations[1], batchnorm=batchnorm)
    self.aspp3 = _ASPPModule(in_channels, out_channels, kernel_size=3, padding=dilations[2], dilation=dilations[2], batchnorm=batchnorm)
    self.aspp4 = _ASPPModule(in_channels, out_channels, kernel_size=3, padding=dilations[3], dilation=dilations[3], batchnorm=batchnorm)
