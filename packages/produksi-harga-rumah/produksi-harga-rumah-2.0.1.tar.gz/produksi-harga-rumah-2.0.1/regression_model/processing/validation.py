...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    """Mengecek input apakah ada nilai na, kalau ada didrop"""
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    """Mengecek validasi input"""

    # Mengonversi nama variabel yang diawali dengan angka (menghindari syntax error)
    ...
    try:
        # mengganti numpy nan agar bisa divalidasi oleh pydantic
...
class HouseDataInputSchema(BaseModel):
    ...
    YrSold: Optional[int]
    FirstFlrSF: Optional[int]  # sudah direname
    SecondFlrSF: Optional[int]  # sudah direname
    ThreeSsnPortch: Optional[int]  # sudah direname
