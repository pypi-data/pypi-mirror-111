...
def format_signup_as_text(records):
    ...
    for record in records:
        table.add_row(
            [
                record["time"].strftime("%Y-%m-%d %H:%M:%S %z (%Z)"),
                record["first_name"],
                record["last_name"],
                record["user_id"],
                record["email"],
...
def list_signups(ctx, **kwargs):
    ...
    for record in records:
        if record.source_ip not in _ips:
            try:
                _ips[record.source_ip] = ipapi.location(ip=record.source_ip)
            except:  # noqa: E722
                _ips[record.source_ip].country_name = "Unknown"
        ...
        output_record = {
            ...
            "country_name": _ips[record.source_ip]["country_name"],
