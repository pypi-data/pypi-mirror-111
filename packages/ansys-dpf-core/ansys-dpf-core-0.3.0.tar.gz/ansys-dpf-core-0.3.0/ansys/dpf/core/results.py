...
class Result:
    ...
    def __init__(self, model,result_info):
        ...
        try:
            ...
            if hasattr(operators, "result") and  hasattr(operators.result, self._result_info.name) :
                self._operator= getattr(operators.result, self._result_info.name)(server = self._model._server)
            else :
                self._operator= Operator(self._result_info.operator_name,server = self._model._server)
    ...
    def on_all_time_freqs(self):
        ...
        """
        self._time_scoping = list(range(1,len(self._model.metadata.time_freq_support.time_frequencies)+1))
        return self
    ...
    def on_mesh_scoping(self, mesh_scoping):
        ...
        if isinstance(mesh_scoping, list):
            mesh_scoping= Scoping(ids=mesh_scoping,location=self._result_info.native_scoping_location,
