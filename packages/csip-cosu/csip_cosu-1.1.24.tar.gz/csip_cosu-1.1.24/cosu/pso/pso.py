...
def eval_cost(x, i, step_param_names, step_objfunc, calib_params, req_queue, files, url, param, conf: Dict, r, s):
    ...
    while pfail_retry > 0:
        ...
        for i_particle in range(no_particles):
            req_queue.put((r, s, i, i_particle, x, step_param_names, calib_params, step_objfunc, res_queue))
        ...
        if failed_particles <= pfail_count:
            break
        print("Re-running particles, since ", failed_particles, ' out of ', no_particles,' particles failed.')
        pfail_retry -= 1
...
def global_best(steps: Dict, rounds: Tuple, args: Dict, n_particles: int, iters: int, options: Dict,
                ...
                ftol_iter: int = 1, full_trace: List = None, rounds_below: int = 1,
    ...
    best_cost = np.ones(len(steps)) * 100000
    ...
    for r in range(max_rounds):
        ...
        for s, step in enumerate(steps):
            ...
            cost, pos = optimizer[s].optimize(eval_cost,
                                              iters=step.get('iters', iters),
                                              **args)
            if cost is None:
                exit(1)
