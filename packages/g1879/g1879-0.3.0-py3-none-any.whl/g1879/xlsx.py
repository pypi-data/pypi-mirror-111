...
#     # 设置行高
#     sh.row_dimensions[sh.max_row].height = 28
#     # txt = sh.cell(sh.max_row, bt_col).value
#     # size = int((len(txt.encode('utf-8')) - len(txt)) / 2 + len(txt))
#     # tmp_height = 15 * ((size // 20) + int(bool(size % 20))) + 8
#     # if tmp_height > sh.row_dimensions[sh.max_row].height:
#     #     sh.row_dimensions[sh.max_row].height = tmp_height
#     # 意见行高
#     c = sh.cell(sh.max_row, yj_col)
#     txt = str(c.value)
#     if txt is not None and txt != '':
#         字符数 = int((len(txt.encode('utf-8')) - len(txt)) / 2 + len(txt))
#         宽度 = sh.column_dimensions[get_column_letter(yj_col)].width
#         要多少行 = ceil(字符数 / 宽度)
#         字大小 = c.font.sz
#         设置行高 = 字大小 * 要多少行 + 4
#         tmp_height = 设置行高
#         print(字符数, 宽度, 字大小)
#         # print(字符数)
#         # yu = 1 if size % 20 else 0
#         # tmp_height = 15 * ((size // 20) + int(bool(size % 20))) + 8
#         # print(tmp_height)
#         # print(sh.row_dimensions[sh.max_row].height)
#         if tmp_height > sh.row_dimensions[sh.max_row].height:
#             sh.row_dimensions[sh.max_row].height = tmp_height
