...
from lgt_data.model import UserModel, LeadModel, SlackHistoryMessageModel, BotModel
...
def get_system_slack_credentials(lead: LeadModel, bots: List[BotModel]=None) \
        -> Optional[SlackCredentialsResponse]:
    """
    ...
    cred = next(filter(lambda x: x.bot_name == lead.message.name, credentials), None)
...
def get_slack_credentials(user: UserModel, route: LeadModel, update_expired=True, user_bots=None) -> Optional[SlackCredentialsResponse]:
    """
    ...
    cred = get_dedicated_bot_credentials(user, route, update_expired)
    if cred:
        return cred
    ...
    if update_expired and (uctnow - cred_date_time).days > 5:
...
def get_dedicated_bot_credentials(user: UserModel, lead: LeadModel, update_expired=True) -> Optional[SlackCredentialsResponse]:
    bot = next(iter([bot for bot in BotMongoRepository().get() if bot.name == lead.message.name]), None)
    if not bot:
        return None

    slack_url = bot.slack_url.strip("/").lower()
    bot = next(iter([bot for bot in DedicatedBotRepository().get_user_bots(user.id)
