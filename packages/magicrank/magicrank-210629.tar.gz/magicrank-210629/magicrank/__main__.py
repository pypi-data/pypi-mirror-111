...
def portfolio(args):
    ...
    try:
        ...
        data = dict(timestamp=int(time.time()), data=download(screen))
        with open(filename, 'w') as fd:
            json.dump(data, fd)

    mcap_list = sorted([v['mar_cap_rs_cr'] for v in data['data'].values()],
                       reverse=True)
    mcap_threshold = mcap_list[min(args.top, len(mcap_list)) - 1]

    tmp = dict()
    for k, v in data['data'].items():
        if v['mar_cap_rs_cr'] < mcap_threshold:
            continue
    ...
    data = {k: v for k, v in data.items()
            if args.minmcap <= v['mar_cap_rs_cr'] <= args.maxmcap}
    ...
    mcap_count = {k: 0 for k in [mcap_list[min(i, len(mcap_list)-1)]
                                 for i in range(99, len(mcap_list)+100, 100)]}
    ...
    with open('magicrank.json', 'w') as fd:
        json.dump(dict(data=stock_list, date=int(time.time())),
    ...
    for i, k in enumerate(sorted(mcap_count, reverse=True)):
        if mcap_count[k] - prev:
            log('top(%d) mcap(%d) count(%d) total(%d)',
                min((i+1)*100, len(mcap_list), args.top), k,
...
def main():
    ...
    parser.add_argument('--minmcap', dest='minmcap', type=int, default=0)
    parser.add_argument('--maxmcap', dest='maxmcap', type=int, default=10**20)
    parser.add_argument('--amount', dest='amount', type=int, default=0)
    parser.add_argument('--count', dest='count', type=float, default=100)
    parser.add_argument('--top', dest='top', type=int, default=1000)
