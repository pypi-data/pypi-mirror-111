...
def test_dataframe_filters():
    ...
    assert (filter_by_list_of_selectors(df1, []) == df1).all().all()
    assert (filter_by_list_of_selectors(df2, list_of_selectors_2) == df2_filtered).all().all()
    assert (filter_by_list_of_selectors(df3, list_of_selectors_3) == df3_filtered).all().all()
    assert (filter_by_list_of_selectors(df4, list_of_selectors_4) == df4_filtered).all().all()
    try:
        filter_by_list_of_selectors(df1, [Selector.generate_from_str("att1 < 8")])
        assert (False)
    except KeyError:
        assert (True)
