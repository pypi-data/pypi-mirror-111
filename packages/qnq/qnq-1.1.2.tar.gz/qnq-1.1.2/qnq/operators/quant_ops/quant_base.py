...
class QuantBase:
    ...
    def quantize_params(self):
        ...
        try:
            ...
            # backup bias
            self.backup_bias = copy.deepcopy(self.module.bias)
            self.writer.add_histogram(
                self.name + "_bias",
    ...
    def trim_params(self, debug_bw=None, debug_fl=None, just_test=False):
        """[summary]

        Args:
            debug_bw ([type], optional): [description]. Defaults to None.
            debug_fl ([type], optional): [description]. Defaults to None.
            just_test (bool, optional): if you just test once, you do not need backup. Defaults to False.
        """
        # for debug
        bit_width = self.bit_width if debug_bw is None else debug_bw
        ...
        if self.weight_fl != -1:
            if just_test:
                self.module.weight.data = float2fixed(self.module.weight.data,
                                                      bit_width,
                                                      fraction_length)
                self.module.bias.data = float2fixed(self.module.bias.data,
                                                    bit_width, fraction_length)
            else:
                if self.backup_weight is not None:
                    self.module.weight.data = float2fixed(
                        self.backup_weight, bit_width, fraction_length)
                if self.backup_bias is not None:
                    self.module.bias.data = float2fixed(
                        self.backup_bias, bit_width, fraction_length)
