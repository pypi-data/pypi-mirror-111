...
def update_item_from_timestamp(item, timestamp):
    item['play.start_time'] = timestamp / GST_NANOSECONDS
    return item
...
def play(tracks: calliope.playlist.Playlist, audio_output: pathlib.Path) -> calliope.playlist.Playlist:
    """Render playlist to an audio file using `GStreamer <https://gstreamer.freedesktop.org/>`_.

    The audio output will be in WAV format, and will be written to
    `audio_output`.

    Returns:
        The input playlist, with an additional `play.start_time` field set.
