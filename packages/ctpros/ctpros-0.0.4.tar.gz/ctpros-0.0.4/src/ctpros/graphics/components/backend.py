...
import threading, time, math, types
import numpy as np
...
def gui_saveas(*args, **kwargs):
    """
    Returns the filename generated by the GUI file dialog.
...
def gui_open(*args, **kwargs):
    """
    Returns the filename(s) generated by the GUI file dialog.

    Expect a returned iterable.
    """
    root = tk.Tk()
    root.withdraw()
    root.wm_attributes("-topmost", 1)
    filenames = tk.filedialog.askopenfilenames(parent=root, *args, **kwargs)
    root.destroy()
    return filenames
...
class BackEnd_MixIn:
    ...
    def __init__(self, *imgs, withdraw=False, verbosity=2):
        ...
        self.__init_binds()
        self.add_imgs(*imgs)
        #     newimg = [img.ImgTemplate(myimg)]
        # if imgs:
        #     imgs = [
        #         img.ImgTemplate(myimg) if type(myimg) is str else myimg
        #         for myimg in imgs
        #     ]
        #     self.imgs = imgs
        #     self.imgselect.fill_selection(imgs)
    ...
    def get_selected_img_inds(self):
        ...
        return [
            options.index(var.get()) for var in self.selected_imgnames
        ]  # return self.img index if selection index is not 0, which equates to no selection
