...
class VisaSession(object):
	...
	def _narrow_down_opc_tout_error(self, command: str, is_query: bool, timeout: int) -> None:
		...
		if is_query:
			if stb & StatusByte.error_queue_not_empty:
				...
				InstrumentErrors.assert_no_instrument_status_errors(self.resource_name, self.query_all_syst_errors(), context, first_error=InstrumentErrors.TimeoutException)
	...
	def _narrow_down_io_tout_error(self, context: str, visa_timeout: int = 0) -> None:
		...
		if stb & StatusByte.error_queue_not_empty:
			InstrumentErrors.assert_no_instrument_status_errors(self.resource_name, self.query_all_syst_errors(), context + ' and ...', first_error=InstrumentErrors.TimeoutException)
