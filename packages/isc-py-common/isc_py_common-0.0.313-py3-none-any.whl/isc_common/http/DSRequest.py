...
class DSRequest(RPCRequest):
    ...
    def get_transaction_data(self, excluded_keys=['grid']):
    ...
    def get_transaction_old_data(self, excluded_keys=['grid']):
        data = self.get_old_data(excluded_keys=excluded_keys)
        if isinstance(data, dict):
            transaction = data.get('transaction')
            if isinstance(transaction, dict):
                return transaction.get('operations')
