...
from isc_common.common import closed
...
class Messages_theme_Manager(CommonTreeGridManager):
    ...
    def deleteFromRequest(self, request, removed=None, ):
        ...
        ids = list(map(lambda x: x[0], tuple_ids))

        top_messages = list(map(lambda x: x.id, Messages.objects.filter(theme_id__in=ids)))
        messages = list(Messages.objects1.get_descendants(id=top_messages, child_id='id'))
        ids = list(map(lambda x: x.id, messages))

        top_messages = list(map(lambda x: x.id, Messages.objects.filter(id__in=ids, state__code=closed)))
        if len(top_messages) > 0:
            raise Exception('В выбранном диапазоне присутствуют закрытые позиции.')

        top_messages = list(map(lambda x: x.id, Messages.objects.filter(id__in=ids).exclude(user=request.user)))
        if len(top_messages) > 0:
            raise Exception('В выбранном диапазоне присутствуют позиции cсозданные другими пользователями.')
