...
from isc_common.auth.models.user_upload_photo import DSResponse_User_UploadPhoto
from isc_common.auth.models.user_view import UserView
    ...
    return JsonResponse(
        DSResponse(
            ...
            data=User.objects.
                get_range_rows1(
                request=request,
                function=UserManager.getRecordPhoto
            ),
            status=RPCResponseConstant.statusSuccess).response)
...
@JsonResponseWithException()
def User_UploadPhoto(request):
    return JsonResponse(DSResponse_User_UploadPhoto(request).response)
