...
class Event:
    ...
    def send_message(self, message=None, users_array=None, progress=None, len=None):
        ...
        users = list(User.objects.filter(id__in=list(set(map(lambda x:x.id, self.users.copy())))))

        if isinstance(users_array, list):
            users = User.objects.filter(id__in=list(set(map(lambda x:x.id, self.users.copy())).union(map(lambda x:x.id, users_array))))
