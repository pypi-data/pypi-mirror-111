...
class lifepath:
    ...
    def lifepath_summary(self, params, ports_rt, annuity_pr,target_wage, conf=0.9):
    ...
    def score_fun(self, params, time_props, ports_rt, annuity_pr,target_wage, conf=0.9):
        cum_dist = beta.cdf(time_props, params[0], params[1])
        port_props = np.array([cum_dist[0]] + list(np.diff(cum_dist)))
        n_times=ports_rt.shape[0]
        years_per_port = np.rint(port_props*n_times)
    ...
    def deoptim(self, ports_rt, annuity_pr, target_wage, conf=0.9, beta_par_bnds=(0,5), popsize=50):
        bnds = (beta_par_bnds, beta_par_bnds)
        n_ports = ports_rt.shape[2]
        time_props = (np.arange(n_ports+1)/n_ports)[1:]
        sol = de(self.score_fun, bounds=bnds, popsize=popsize, strategy='best1bin', args=(time_props, ports_rt, annuity_pr,target_wage, conf))
        lp_summ = self.lifepath_summary(sol.x, ports_rt, annuity_pr,target_wage, conf)
        return sol.x, lp_summ

    def hyperoptim(self, ports_rt, annuity_pr, target_wage, conf=0.9, beta_par_bnds=(0,5), max_evals=50, par_interv=0.1):
        n_ports = ports_rt.shape[2]
        time_props = (np.arange(n_ports+1)/n_ports)[1:]
        ...
        tpe_best = fmin(fn=lambda x: self.score_fun(x, time_props, ports_rt, annuity_pr,target_wage, conf), space=space,algo=tpe_algo, trials=tpe_trials, max_evals=max_evals)
        solx = [par_range[tpe_best['x']], par_range[tpe_best['y']]]
        lp_summ = self.lifepath_summary(solx, ports_rt, annuity_pr,target_wage, conf)
        return solx, lp_summ

    def bayesopt(self, ports_rt, annuity_pr, target_wage, conf=0.9, beta_par_bnds=[(0,5),(0,5)], n_iter=50, init_points=5):
        n_ports = ports_rt.shape[2]
        time_props = (np.arange(n_ports+1)/n_ports)[1:]
        from bayes_opt import BayesianOptimization as bo
        pbounds = {'x': beta_par_bnds[0], 'y': beta_par_bnds[1]}
        optimizer = bo(lambda x,y:-self.score_fun([x,y], time_props, ports_rt, annuity_pr,target_wage, conf), pbounds)
