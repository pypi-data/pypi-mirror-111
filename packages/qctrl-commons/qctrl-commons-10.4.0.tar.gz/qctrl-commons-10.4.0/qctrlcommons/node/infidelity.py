...
class InfidelityPwc(Node):
    ...
    noise_operators : List[Pwc], optional
    ...
    name = "infidelity_pwc"
    args = [
        forge.arg("hamiltonian", type=types.Pwc),
        forge.arg("target_operator", type=types.Target),
        forge.arg("noise_operators", type=Optional[List[types.Pwc]], default=None),
    ]
    rtype = types.Tensor
    ...
    def create_node_data(cls, _operation, **kwargs):
        ...
        check_argument(
            isinstance(hamiltonian, PwcNodeData),
        ...
        batch_shape, _ = validate_batch_and_value_shapes(hamiltonian, "hamiltonian")
        for noise_operator in noise_operators or []:
            check_argument(
                isinstance(noise_operator, PwcNodeData),
                "Each of the noise_operators must be a Pwc.",
                {"noise_operators": noise_operators},
            )
            noise_operator_batch_shape, _ = validate_batch_and_value_shapes(
                noise_operator, "noise_operators"
            )
            check_argument(
...
class InfidelityStf(Node):
    ...
    def create_node_data(cls, _operation, **kwargs):
        ...
        for noise_operator in noise_operators or []:
            check_argument(
                isinstance(noise_operator, StfNodeData),
                "Each of the noise_operators must be a Stf.",
                {"noise_operators": noise_operators},
            )
            noise_operator_batch_shape, _ = validate_batch_and_value_shapes(
                noise_operator, "noise_operators"
