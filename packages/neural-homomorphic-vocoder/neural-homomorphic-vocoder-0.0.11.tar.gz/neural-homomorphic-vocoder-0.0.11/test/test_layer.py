...
def test_nhv_conv_type(conv_type):
    run_nhv(conv_type, None, None, True, True, True, True, True, False, None)


@pytest.mark.parametrize("postfilter_type", [None, "conv", "ddsconv"])
def test_nhv_postfilter_type(postfilter_type):
    run_nhv(
        "original", postfilter_type, None, True, True, True, True, True, False, None
    )
...
def test_nhv_ltv_postfilter_type(ltv_postfilter_type):
    run_nhv(
        "original", None, ltv_postfilter_type, True, True, True, True, True, False, None
    )


@pytest.mark.parametrize("use_causal", [True, False])
def test_nhv_use_causal(use_causal):
    run_nhv("original", None, None, use_causal, True, True, True, True, False, None)


@pytest.mark.parametrize("use_reference_mag", [True, False])
def test_nhv_use_reference_mag(use_reference_mag):
    run_nhv(
        "original", None, None, False, use_reference_mag, True, True, True, False, None
    )


@pytest.mark.parametrize("use_tanh", [True, False])
def test_nhv_use_tanh(use_tanh):
    run_nhv("original", None, None, False, False, use_tanh, True, True, False, None)


@pytest.mark.parametrize("use_uvmask", [True, False])
def test_nhv_use_uvmask(use_uvmask):
    run_nhv("original", None, None, False, False, False, use_uvmask, True, False, None)


@pytest.mark.parametrize("use_weight_norm", [True, False])
def test_nhv_use_weight_norm(use_weight_norm):
    run_nhv(
        "original", None, None, False, False, False, False, use_weight_norm, False, None
    )


@pytest.mark.parametrize("use_conv_dft", [True, False])
def test_nhv_use_conv_dft(use_conv_dft):
    run_nhv(
        "original", None, None, False, False, False, False, False, use_conv_dft, None
    )
...
def test_nhv_use_scaler_file(scaler_file):
    run_nhv(
        "original", None, None, False, False, False, False, False, False, scaler_file
    )
...
@pytest.mark.parametrize("x", [torch.randn((3, 100, 1024)), torch.randn((1, 1024))])
def test_fft(x):
    n_fft = 1024
    ...
    y_dft = dft(x)
    npt.assert_almost_equal(y_torch.numpy(), y_dft.numpy(), decimal=4)

    x_dft = dft(y_torch, inverse=True)
    npt.assert_almost_equal(x_torch.numpy(), x_dft.numpy(), decimal=4)
    npt.assert_almost_equal(x, x_torch.numpy(), decimal=4)
    npt.assert_almost_equal(x, x_dft.numpy(), decimal=4)
