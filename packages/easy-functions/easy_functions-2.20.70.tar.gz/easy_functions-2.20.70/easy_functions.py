...
"""
#更新状态:活跃
cmdlist = ['bigfont','literally','wait','cls','title','find_file','start','call','Vbs','color','mode','shield','choice','Code','mkdir','copy','pause','find_suffix','help','helpcmd','version','log'] #命令列表，以大写字母开头的为类(class)
__all__ = cmdlist
c = 0
d = 0
for i in cmdlist:
    if i.istitle():
        c += 1
d = len(cmdlist) - c
__version__ = str(c) + '.' + str(d) + '.' + '70' #easy_functions版本号
__author__ = 'Jerry\n我的QQ号：2711893794' #作者
...
class Vbs: #创建并调用弹窗，类用法(class)
    def __init__( self ):
        self.path = 'C:\\Users\\Administrator\\AppData\\Local\\Temp\\easy_functionsMSG.vbs'
    def shownormal(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #无图标
        title = title.replace('\n','')
    ...
    def warningaskcancel(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #显示警告图标并提供[确认]和[取消]
        title = title.replace('\n','')
        message = message.replace('\n','')
        title = title.replace('\r','')
        message = message.replace('\r','')
        with open(self.path,'w') as f:
            ...
            f.write('))')
        return call(self.path)
    def erroraskcancel(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #显示错误图标并提供[确认]和[取消]
        title = title.replace('\n','')
        message = message.replace('\n','')
        title = title.replace('\r','')
        message = message.replace('\r','')
        with open(self.path,'w') as f:
            ...
            f.write('))')
        return call(self.path)
    def infoaskcancel(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #显示信息图标并提供[确认]和[取消]
        title = title.replace('\n','')
        message = message.replace('\n','')
        title = title.replace('\r','')
        message = message.replace('\r','')
        with open(self.path,'w') as f:
            ...
            f.write('))')
        return call(self.path)
    def warningaskyesno(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #显示警告图标并提供[是]和[否]
        title = title.replace('\n','')
        message = message.replace('\n','')
        title = title.replace('\r','')
        message = message.replace('\r','')
        with open(self.path,'w') as f:
            ...
            f.write('))')
        return call(self.path)
    def errorretry(self, title='easy_functions msgbox', message='模块easy_functions，简单，实用'): #显示错误图标并提供[重试]和[取消]
        title = title.replace('\n','')
        message = message.replace('\n','')
        title = title.replace('\r','')
        message = message.replace('\r','')
...
def mode(cols, lines): #更改cmd外框大小
    ...
    try:
        ...
        return
    os.system('mode con cols=' + c + ' lines=' + l)
    return
...
def find_suffix(path, suffix): #查找指定后缀的文件
    ...
    with open('C:\\Users\\Administrator\\AppData\\Local\\Temp\\make_file_list.bat','w',encoding='ASCII') as f:
        f.write('@echo off\n')
        f.write('for /r ' + path + ' %%r in (*' + suffix + ') do (\n')
        f.write('    echo %%r >>%tmp%\maker.tmp\n')
        f.write(')')
    call('C:\\Users\\Administrator\\AppData\\Local\\Temp\\make_file_list.bat >nul')
    os.remove('C:\\Users\\Administrator\\AppData\\Local\\Temp\\make_file_list.bat')
    with open('C:\\Users\\Administrator\\AppData\\Local\\Temp\\maker.tmp','r') as f:
        for i in f.readlines():
            file_list.append(i.replace('\n',''))
    os.remove('C:\\Users\\Administrator\\AppData\\Local\\Temp\\maker.tmp')
    return file_list
def log(): #更新日志
    print('')
    ...
    print('2.对find_file功能进行了更改，现在调用文件时不会再生成ErrorOutPut.log文件，而是返回布尔值，可以直接储存在变量中')
    print('')
    ...
    print('1.增加了copy功能，具体请使用helpcmd()查看')
    print('')
...
def helpcmd( cmd ): #文件命令功能解释
    ...
    if cmd == 'Vbs':
        ...
        print("      变量.shownormal('标题','正文')/warningaskcancel('标题','正文')/erroraskcancel('标题','正文')/...")
