...
class connect_flow_FlowLogger:
    ...
    def getFormattedRequest(self,request,lastRequest,fmt):
        if (request != lastRequest):
            if (self.logger.getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                ...
                requestStr = (connect_util_Util.beautifyObject(diff,self.logger.isCompact(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG),self.logger.isBeautified()) if ((diff is not None)) else request)
    ...
    def writeStepSkip(self,willSave):
        if willSave:
            self.logger.write(connect_logger_Logger.LEVEL_INFO,"Skipping request. Trying to save step data.")
        else:
            self.logger.write(connect_logger_Logger.LEVEL_INFO,"Skipping request. Step data will not be saved because feature is disabled.")

    def writeStepSavedInConnect(self):
        self.logger.write(connect_logger_Logger.LEVEL_INFO,"Step data saved in Connect.")

    def writeStepSavedLocally(self):
        self.logger.write(connect_logger_Logger.LEVEL_INFO,"Step data saved locally.")

    def writeStepSaveFailed(self):
        self.logger.write(connect_logger_Logger.LEVEL_INFO,"Step data could not be saved.")
    ...
    def writeLoadedStepData(self,index,storageType):
        self.logger.write(connect_logger_Logger.LEVEL_INFO,(((("Resuming request from step " + Std.string(((index + 1)))) + " with ") + ("null" if storageType is None else storageType)) + "."))
...
class connect_models_TierConfigRequest(connect_models_IdModel):
    ...
    def getModifiedTcrParams(self):
        ...
        if (self.params.length() != oldTcr.params.length()):
            def _hx_local_0(p):
                return p.toString()
