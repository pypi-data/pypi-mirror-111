...
class TargetsPMakeupPlugin(pm.AbstractPmakeupPlugin):
    ...
    def process_targets(self):
        ...
        if self._model.should_show_target_help:
            ...
            # check that all targets are indeed present
            invalid_targets = list(filter(lambda t: t not in self._model.available_targets, self._model.requested_target_names))
            if len(invalid_targets) > 0:
                raise pm.PMakeupException(f"Targets {', '.join(invalid_targets)} are absent. Available targets are {', '.join(self._model.available_targets.keys())}")
