...
class TestContentStoreQueryAndCount(TestCaseWithMongoMock):
    def test_succeed_lt_gt_id(self):
        ...
        assert self.content_store.query(
            q={
                "attr1": True,
                "_id": {
                    "$lt": bson.ObjectId(lt[0]["_id"])
                }
            },
            limit=1,
            projection=["key"],
        ...
        assert self.content_store.count(
            q={
                "attr1": True,
                "_id": {
                    "$gt": bson.ObjectId(gt[0]["_id"])
                }
            },
            sort={
                "_id": pymongo.DESCENDING
    ...
    def test_succeed_multiple_sort(self):
        ...
        assert list(map(lambda d: d['key'], docs)) == ["value_5", "value_6", "value_4", "value_3", "value_1", "value_2"]
