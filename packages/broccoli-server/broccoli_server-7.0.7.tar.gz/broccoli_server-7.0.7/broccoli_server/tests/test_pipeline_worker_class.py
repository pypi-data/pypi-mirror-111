...
class TestPipelineWorkerClass(TestCaseWithMongoMock):
    ...
    def test_all_valid_single_transitions(self):
        ...
        def process(context, documents):
            return list(map(lambda oid: StateUpdate({"_id": oid}, "state2", {"$set": {"attr1": True}}), [oid_a, oid_b, oid_c]))
    ...
    def test_all_valid_multiple_transitions(self):
        ...
        def process(context, documents):
            return list(map(lambda oid: StateUpdate({"_id": oid}, "state2", {"$set": {"attr1": "a"}}), [oid_a, oid_b])) + \
                   list(map(lambda oid: StateUpdate({"_id": oid}, "state3", {"$set": {"attr2": "b"}}), [oid_c]))
    ...
    def test_all_valid_single_transitions_with_complex_update_doc(self):
        ...
        oid_b = ObjectId.from_datetime(now - datetime.timedelta(seconds=1))
        oid_c = ObjectId.from_datetime(now - datetime.timedelta(seconds=2))
        self.content_store.append({"_id": oid_a, "_state": "state1", "remove_me": True, "int": 1}, "_id")
        self.content_store.append({"_id": oid_b, "_state": "state1", "remove_me": True, "int": 2}, "_id")
        self.content_store.append({"_id": oid_c, "_state": "state1", "remove_me": True, "int": 5}, "_id")

        def process(context, documents):
            return list(map(lambda oid: StateUpdate({"_id": oid}, "state2", {"$set": {"attr1": True}, "$unset": {"remove_me": ""}, "$inc": {"int": 1}}), [oid_a, oid_b, oid_c]))
        demo_worker, demo_work_context = self.create_demo_worker(process, "state1", {"state2"})
        demo_worker.work(demo_work_context)
    ...
    def test_invalid_transition(self):
        ...
        def process(context, documents):
            return list(map(lambda oid: StateUpdate({"_id": oid}, "state2"), [oid_a, oid_b])) + \
    ...
    def test_no_transition(self):
        ...
        def process(context, documents):
            return list(map(lambda oid: StateUpdate({"_id": oid}, "state2"), [oid_a, oid_b, oid_c]))
