...
from broccoli_server.pipeline import Pipeline, PipelineState, PipelineModView, PipelineWorker, PipelineModViewToState
...
class TestPipelineAssertDag(TestPipeline):
    def test_assert_dag_straight_line_pipeline(self):
        ...
        pipeline.add_mod_view(PipelineModView("2to3", "state2", [PipelineModViewToState("state3", "state3")], [], ''))
    ...
    def test_assert_not_dag(self):
        ...
        pipeline.add_worker(self.create_demo_worker_instance("state4", {"state5"}))
        self.assertRaisesRegex(RuntimeError, 'has a cycle', pipeline.assert_pipeline_is_dag)
    ...
    def test_assert_missing_starting_state(self):
        ...
        pipeline.add_worker(self.create_demo_worker_instance("state1", {"state2"}))
        pipeline.add_mod_view(PipelineModView("2to3", "state2", [PipelineModViewToState("state3", "state3")], [], ''))
        self.assertRaisesRegex(RuntimeError, "starting state", pipeline.assert_pipeline_is_dag)
    ...
    def test_assert_undiscovered_states(self):
        ...
        pipeline.add_worker(self.create_demo_worker_instance("state1", {"state2"}))
        pipeline.add_mod_view(PipelineModView("2to3", "state2", [PipelineModViewToState("state3", "state3")], [], ''))
        self.assertRaisesRegex(RuntimeError, "undiscovered", pipeline.assert_pipeline_is_dag)
...
class TestPipelineAssertValidMigration(TestPipeline):
    ...
    def test_assert_missing_doc_invalid(self):
        ...
        self.assertRaisesRegex(
            RuntimeError, "Sum of all state docs does not match total count",
            pipeline.assert_valid_state_migrations,
            self.content_store, 2
        )
    ...
    def test_assert_duplicate_doc_invalid(self):
        ...
        self.assertRaisesRegex(
            RuntimeError, "is going to have both",
            pipeline.assert_valid_state_migrations,
            self.content_store, 2
        )
...
class TestPipelineMigrate(TestPipeline):
    ...
    def test_successful_idempotent_migration(self):
        ...
        self.content_store.update_many(filter_q={'attr1': True}, update_doc={"$unset": {"_state": ""}})
