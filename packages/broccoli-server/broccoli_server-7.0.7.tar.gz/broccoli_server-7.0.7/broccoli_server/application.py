...
from broccoli_server.pipeline import Pipeline, StateKey, PipelineModViewToState
...
class Application(object):
    ...
    def _configure_pipeline_workers(self):
        ...
        for pipeline_worker in self.pipeline.workers:
            ...
            self.worker_cache.register_pipeline_worker(worker_id=worker_id, func=pipeline_worker.process)
            ...
            if worker_id not in actual_workers:
                ...
                self.worker_config_store.add(WorkerMetadata(
        ...
        actual_all_pipeline_workers = self.worker_config_store.get_all(namespace='pipeline')
    ...
    def _configure_pipeline_mod_views(self):
        ...
        def generate_mod_view_projections(to_state: PipelineModViewToState) -> NamedModViewColumn:
        ...
        for mod_view in self.pipeline.mod_views:
            self.add_mod_view(
                ...
                mod_view=ModViewQuery(
                    query={
                        StateKey: mod_view.from_state
                    },
                    projections=mod_view.projections + list(map(generate_mod_view_projections, mod_view.to_states)),
