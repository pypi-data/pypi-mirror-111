...
class Pipeline(object):
    ...
    def assert_valid_state_migrations(self, content_store: ContentStore, migration_batch: int = 50):
        ...
        if sum_migrated_count < total_count:
            print(f"Sampling 100 object IDs that will miss migration {self._object_ids_missing_migration(content_store)[: 100]}")
            raise RuntimeError(f"Sum of all state docs does not match total count. sum={sum_migrated_count} < actual={total_count}")
            ...
            for i in range(migration_batches):
                ...
                for doc in content_store.query(migrate_q, skip=i * migration_batch, limit=migration_batch):
                    ...
                    if doc_id in doc_id_to_state:
                        raise RuntimeError(f"Document with id {doc_id} is going to have "
    ...
    def perform_state_migrations(self, content_store: ContentStore):
        ...
        for state in self.states:
            ...
            if content_store.count(new_filter_q) != 0:
                ...
                print(f"No document to migrate for pipeline {self.name} state {state.name}")
