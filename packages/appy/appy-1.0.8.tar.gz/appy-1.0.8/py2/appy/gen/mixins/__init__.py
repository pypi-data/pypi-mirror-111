...
class BaseMixin:
    ...
    def mayEditHistoryComment(self, event, user, isManager):
        ...
        class_ = self.getClass()
        if hasattr(class_, 'editHistoryComments'):
            r = class_.editHistoryComments
            # Call the potentially defined custom method
            if callable(r): r = r(self.appy(), event)
            # The comment may be edited, but not for any event, by any user
            if r:
                # Allow edition if logged user is a Manager (p_isManager is
                # True) or if he is the p_event's author.
