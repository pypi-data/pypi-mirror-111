    ...
    def feature_importance(self, df: pd.DataFrame, 
        ...
        ----------
            ...
            _model_id: (Optional) int:
                Collection ID for which to calculate feature importance.
                If passed, feature stats will be stored.
                If nothing is passed, dataframe stats will be stored.
        ...
        with tqdm(desc='Calculating Feature Importance', total=8, unit='step', file=sys.stdout, leave=False) as progress:
            ...
            if _model_id:
                response["modelId"] = _model_id
            json_payload = featureImportanceStats(**response)
            
            if _model_id:
                self.save.feature_importance_stats(id=_model_id, payload=json_payload)
            else:
                self.save.column_importance_stats(id=rasgo_df_id, payload=json_payload)

            if not return_cli_only and not _model_id:
                url = f'{self._environment.app_path}/dataframes/{rasgo_df_id}/importance'
                webbrowser.open(url)
            elif _model_id and not return_cli_only:
                url = f"{self._environment.app_path}/collections/{_model_id}"
                webbrowser.open(url)
