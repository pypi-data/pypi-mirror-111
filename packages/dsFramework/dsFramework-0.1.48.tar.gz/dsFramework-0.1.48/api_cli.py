...
def run_server():
    ...
    folder = currentPipelineFolder + '/server/server_run.py'
...
def create_project(project_name):
    ...
    create_tester_file(project_name, directories['server'], 'server_run', False)
    create_tester_file(project_name, directories['server'], 'test_server_post', False)
    create_tester_file(project_name, directories['server'], 'pool', False)
    create_init_file(directories['server'] + '/__init__.py', 'from server.pool import InstancePool\nfrom server.server_run import app')
...
def create_dvc_init_file():
    with open(os.path.join(__location__, 'cli/tester/deploy_files/dvc_init.sh'), 'r') as file:
        data = file.read()
        data = data.replace('generatedDirectory', directories['main'])
        new_file = directories['main'] + '/dvc_init.sh'
        if not os.path.exists(new_file):
            f = open(new_file, "w")
            f.write(data)
            f.close()
...
def run_dvc_init():
    ...
    subprocess.call('dvc_init.sh', shell=True)

def copy_deploy_files(main_dir):
    currentPipelineFolder = os.path.basename(os.getcwd())
    dir = os.path.join(__location__, main_dir, 'cli/tester/deploy_files/')
    listOfFiles = list()
