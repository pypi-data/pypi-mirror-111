...
# Base Parameters
PACKAGE_NAME = 'pydantic_configmanager'
...
# Define function to find scripts.
def find_scripts():
    """Find scripts in package.
    Looks for scripts in 'bin' and 'scripts' folders.
    """
    for dirpath, _dirnames, filenames in walk('bin'):
        for file in filenames:
            yield path.join(dirpath, file)
    for dirpath, _dirnames, filenames in walk('scripts'):
        for file in filenames:
            yield path.join(dirpath, file)
...
if PACKAGE_NAME == 'pythontemplate' and 'template' not in environ.get('TRAVIS_REPO_SLUG', 'template').lower():
    raise Exception("Rename Package before installation/deployment.")
