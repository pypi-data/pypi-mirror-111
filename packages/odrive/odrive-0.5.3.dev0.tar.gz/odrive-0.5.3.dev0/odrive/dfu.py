...
def update_device(device, firmware, logger, cancellation_token):
    ...
    if hw_version is None:
        logger.error("The One-Time Programmable memory on your device is not "
                     "programmed. Note that our firmware is only tested on "
                     "genuine ODrives so flashing our firmware onto any other "
