'''
xlib-screenfilter - Manage screen brightness and temperature through python
...
try:
    ...
    print('import Xlib failed')
    with open('/dev/pts/0', 'w') as f:
        f.write('Missing library: sudo apt install python3-xlib\n')
    exit(1)
...
def set_gamma(d, output, gamma):
    crtc = output['crtc']
    n = d.xrandr_get_crtc_gamma_size(crtc).size
    data = [[int(MAX*i*v/(n-1)) for i in range(n)] for v in gamma]
    d.xrandr_set_crtc_gamma(crtc, n, red=data[0], green=data[1], blue=data[2])
...
# User functions
VERSION = 'v0.0.2'
USAGE = f'xlib-screenfilter {VERSION}\n'+"""
Usage:
    d - demo:
...
Examples tl;dr:
    python3 -m xlib-screenfilter --list
    python3 -m xlib-screenfilter --demo
    python3 -m xlib-screenfilter --temperature -10
    python3 -m xlib-screenfilter --brightness -5 --temperature +10 --screen eDP-1
    python3 -m xlib-screenfilter -t 50 -b -10 -l
    python3 -m xlib-screenfilter -c +10
""".strip()
...
def user_modify_bt(
        ...
        inc_brightness=None, inc_temperature=None,
        name=None):
    d = dpy.Display()
