...
def mproc_apply(func, iterLst, descr='', iterArgPos=0,
                extraArgs=(), showProgress=True, applyAsync=True,
  ...
  if showProgress and applyAsync and not debug:
    ...
    iterLst = tqdm(iterLst, total=total)

  pre_results = []
  errs = {}
  def errCallback(fnArgs, ex):
    errs[fnArgs[iterArgPos]] = str(ex)
  ...
  if debug:
    ...
    if applyAsync:
      def applyFunc(func_, args):
        return pool.apply_async(func_, args, kwds=extraKwargs,
                                callback=callback, error_callback=lambda ex: errCallback(args, ex))
    else:
...
def makeCli(func: t.Callable, convertArgs=True, parserKwargs: dict=None, **procKwargs):
  ...
  """
  from .params import NestedProcWrapper
  ...
  proc = AtomicProcess(func, **procKwargs)
  param = NestedProcWrapper(proc).parentParam

  useCls = argparse.ArgumentParser if not convertArgs else ArgParseConverter
  parserKwargs = parserKwargs or {}
  parserKwargs.update(description=tipConverter(param.opts['tip']))
  parser = useCls(**parserKwargs)

  for ch in param:
    if 'action' not in ch.type():
      ch: Parameter
      arg = parser.add_argument(f'--{ch.name()}',
      type=type(ch.value()))
      if ch.hasDefault():
        arg.default = ch.defaultValue()
      if 'tip' in ch.opts:
        arg.help = tipConverter(ch.opts['tip'])
  # if proc.hasKwargs:
...
def timedExec(updateFunc: t.Union[t.Generator, t.Callable], interval: int,
              argLst: t.Sequence=None, stopCond: t.Callable=None):
  """
  Iterates over an argument list (or generator) and executes the next action every
  `interval` ms. Useful for updating GUI elements periodically, e.g. showing a list
  of images one at a time
  """
  timer = QtCore.QTimer()
  if argLst is None:
