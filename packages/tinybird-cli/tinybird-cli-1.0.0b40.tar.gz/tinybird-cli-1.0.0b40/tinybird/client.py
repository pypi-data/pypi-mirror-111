...
class TinyB(object):
    ...
    async def _req(self, endpoint, data=None, method='GET', **kwargs):  # noqa: C901
    ...
    async def connections(self, connector=None):
        ...
        if connector:
            return [{'id': c['id'], 'connector': c['service'], **c['settings']} for c in connectors if c['service'] == connector]
        return [{'id': c['id'], 'connector': c['service'], **c['settings']} for c in connectors]
    ...
    async def connection_create_kafka(self, bootstrap_server, username, password, registry_url=None):
        params = {
            'service': 'kafka',
        ...
        if registry_url:
            params['kafka_schema_registry_url'] = registry_url
        return await self._req(f"/v0/connectors?{urlencode(params)}",
                               method='POST')
    ...
    async def sql_get_used_tables(self, sql, raising=False):
        params = {
            'q': sql,
