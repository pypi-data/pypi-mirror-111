...
from tinybird.config import get_config, write_config, FeatureFlags, VERSION, CURRENT_VERSION, SUPPORTED_CONNECTORS, PROJECT_PATHS, \
    DEFAULT_HOST, MIN_WORKSPACE_ID_LENGTH, DEFAULT_UI_HOST
...
@click.pass_context
@coro
async def push(ctx, prefix, filenames, dry_run, check, push_deps, debug, force, populate, fixtures, wait, skip_table_checks, yes):
...
@click.pass_context
@coro
async def auth_rm(ctx, workspace):
...
async def datasource_ls(ctx, prefix, match):
    ...
    columns = ['prefix', 'version', 'name', 'row_count', 'size', 'created at', 'updated at', 'connector']
...
@datasource.command(name="connect", hidden=True)
...
@connection.group(name="create", hidden=True)
@click.pass_context
def connection_create(ctx):
    '''Connection Create commands'''


@connection_create.command(name="kafka", hidden=True)
@click.option('--bootstrap-server', help="Kafka bootstrap server in form mykafka.mycloud.com:9092")
@click.option('--username', help="Kafka SASL plain username")
@click.option('--password', help="Kafka SASL plain password")
@click.option('--registry-url', default=None, help="Avro Confluent Schema Registry URL", hidden=True)
@click.pass_context
@coro
# Example usage: tb connection create kafka --bootstrap-server google.com:80 --username a --password b
async def connection_create_kafka(ctx, bootstrap_server, username, password, registry_url):
    "Add a Kafka connection"
    bootstrap_server and validate_kafka_bootstrap_server(bootstrap_server)
    username and validate_kafka_username(username)
    password and validate_kafka_password(password)
    registry_url and validate_kafka_registry_url(registry_url)
    if not bootstrap_server:
        bootstrap_server = click.prompt("Kafka bootstrap server")
        validate_kafka_bootstrap_server(bootstrap_server)
    if not username:
        username = click.prompt("Kafka SASL username")
        validate_kafka_username(username)
    if not password:
        password = click.prompt("Kafka SASL password", hide_input=True)
        validate_kafka_password(password)

    client = ctx.obj['client']
    result = await client.connection_create_kafka(bootstrap_server, username, password, registry_url)
    id = result['id']
    click.echo(FeedbackManager.success_connection_created(id=id))


def validate_kafka_registry_url(registry_url):
    if not is_url_valid(registry_url):
...
def validate_kafka_bootstrap_server(host_and_port):
...
def validate_kafka_username(s):
    if not isinstance(s, str):
        raise click.ClickException("bad_username")


def validate_kafka_password(s):
    if not isinstance(s, str):
        raise click.ClickException("bad_password")
...
async def connection_rm(ctx, connection_id, force):
    ...
    try:
        ...
        raise click.ClickException(FeedbackManager.error_exception(error=e))
    click.echo(FeedbackManager.success_delete_connector(connection_id=connection_id))
...
async def connection_ls(ctx, connector):
    ...
    if not connector:
        ...
        columns = ['connector', 'id'] + [setting.replace('tb_', '') for setting in getattr(DataConnectorSettings, connector)]
