...
class EngineDetails:
    ...
    >>> ed = EngineDetails({"engine_full": "Join(ANY, LEFT, id)", "engine": "Join", "partition_key": "", "sorting_key": "", "primary_key": "", "sampling_key": ""})
    >>> ed.engine_full
    'Join(ANY, LEFT, id)'
    >>> ed.engine
    'Join'
...
def engine_full_from_dict(engine: str, args: dict, schema: str = None, columns: list = None):
    ...
    for arg in args:
        if not hasattr(EngineDetails, arg):
