...
class AlgorithmContainerConfigParser:
    ...
    # 第五步，vsource_deploy_kube.yaml
    def gen_deployment_kube(self):
        try:
            with open(os.path.join(self.cur_dir, './algorithm-deploy-kube.template'), 'r') as f:
                templates = f.read()
            templates = templates.format(
                self.algorithm_name, self.algorithm_name, self.algorithm_name,
                self.algorithm_name, self.algorithm_name, self.algorithm_name, self.algorithm_name,
            )
            with open(os.path.join(self.service_dir, 'vsource_deploy_kube.yaml'), 'w') as f:
                f.write(templates)
            self.logs.append('[Step 5/6] Deployment kube file has been successfully generated!')
        except Exception as e:
            err_msg = traceback.format_exc()
            self.logs.append('[Step 5 Error, Deployment kube file Generated Failed]: ' + err_msg)
            traceback.print_exc()

    # 第六步，deployment-compose.yaml
    def gen_deployment_compose(self):
        try:
            ...
            with open(os.path.join(self.service_dir, 'vsource_deploy_compose.yaml'), 'w') as f:
                f.write(templates)
            self.logs.append('[Step 6/6] Deployment compose file has been successfully generated!')
        except Exception as e:
            err_msg = traceback.format_exc()
            self.logs.append('[Step 6 Error, Deployment compose file Generated Failed]: ' + err_msg)
            traceback.print_exc()
...
if __name__ == '__main__':
    config = {
        ...
        'input-params': {
            'lower': 'lower limit for this function',
            'upper': 'upper limit for this function'
        },
        'output-params': {
