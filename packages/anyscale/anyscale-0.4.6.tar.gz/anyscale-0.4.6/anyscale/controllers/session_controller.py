...
class SessionController:
    ...
    def start(
        ...
        message = (
            ...
            "using `anyscale.connect()` for your use case and let the Anyscale team "
    ...
    def push(
        ...
        if (source or target) and not no_warning:
            message = (
                "Warning: The --source and --target options will soon be deprecated "
                "for `anyscale push`. Going forward, Anyscale will only support syncing "
                "the project directory files (which can be done without passing any "
                "--source any --target arguments). Please let the Anyscale team know if "
                "this will block your use case."
            )
            print("\033[91m{}\033[00m".format(message), file=sys.stderr)
