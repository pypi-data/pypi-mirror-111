...
def fft(y,
    ...
    y=y-y.mean()
    res=[]
    if if_octave==False: #不考虑倍频程低频无数据问题
        for i in np.arange(n_zong):
            ...
            if len(y_)>0:
                ...
                try:  #经测试先计算最后统一平均的方式和dasp频率特性一致，所以
                    ...
                    res_y=fft_y[range(int(N/2))]
        fft_size_=fft_size
    else: #通过加0的方式将分析频率精度调密，然后再求倍频程
        frec_num=sample_rate/fft_size
        frec_mubiao=0.2
        fft_size_mubiao=sample_rate/fft_size/frec_mubiao
        fft_size_=math.ceil(fft_size*fft_size_mubiao)
        for i in np.arange(n_zong):
            pass
            y_=y[int(i*round((1-cdxs),5)*fft_size):int(i*round((1-cdxs),5)*fft_size+fft_size)][:int(fft_size)]   
            if len(y_)>0:
                if len(y_)<fft_size_: #长度不够进行补零处理
                    y_=np.append(y_, np.array([0]*(int(fft_size_)-len(y_))), axis=0)
                N=fft_size_
                fft_y=np.abs(np.fft.fft(y_*choose_windows(name=window,N=len(y_),fix_meth=fix_meth)))/(0.5*len(y_))
                res.append(fft_y)
    ...
    return list(np.arange(0,sample_rate/2,sample_rate/fft_size_)),list(res_y)#pd.DataFrame({'frac':res_x,'fft':res_y}).set_index('frac')
...
if __name__ == '__main__':
    ...
    a_bpc,b_bpc=octave_3(y=data[i][j],
                 sample_rate =float(info[i][j]['采样频率']),
                 fft_size = float(info[i][j]['采样频率']),
                 cdxs=0.75,
                 meth='线性平均')
    a2_bpc,b2_bpc=octave_3(y=data[i][j],
                 sample_rate =float(info[i][j]['采样频率']),
                 fft_size = float(info[i][j]['采样频率']),
                 cdxs=0.75,
                 if_octave=True,
