...
def etl_files_are_stalled():
    ...
    vm_workflow = \
        re.fullmatch(r"^.*(kb_etl_workflow|host_list_etl_workflow|host_list_detection_etl_workflow).*$",
                     str(target_method_in_module_to_run))
    global_ai_workflow = \
        re.fullmatch(r"^.*(global_ai_etl_workflow).*$",
                     str(target_method_in_module_to_run))
    try:
        for etl_file_path in etl_files:
            if Path(etl_file_path).is_file():
                host_list_detection_match = re.fullmatch(r"^.*(/kb|/host_list).*$", str(Path(etl_file_path)))
                global_ai_match = re.fullmatch(r"^.*(/global_ai).*$", str(Path(etl_file_path)))
                if vm_workflow and host_list_detection_match:
        ...
        if etl_files_max_time_exceeded_flag:
            if vm_workflow:
                etld_lib_functions.logger.info(f"vm_workflow is stalled.")
            elif global_ai_workflow:
                etld_lib_functions.logger.info(f"global_ai_workflow is stalled.")
