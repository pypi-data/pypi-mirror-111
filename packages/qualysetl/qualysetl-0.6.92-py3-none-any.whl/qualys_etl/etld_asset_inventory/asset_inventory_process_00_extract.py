...
def asset_inventory_extract():
    ...
    authorization = cred_dict['bearer']
    use_cookie = etld_lib_credentials.use_cookie  # If true, use cookie auth
    url = f"https://{cred_dict['api_fqdn_server']}/rest/2.0/search/am/asset/"

    payload = {'assetLastUpdated': '2021-06-25T00:00:00Z',
               'lastSeenAssetId': '0'}
    ...
    json_file_prefix = re.sub("_rename_me_to_batch.xml", "", str(etld_lib_config.asset_inventory_json_file))
    json_batch_assetLastUpdated = re.sub(':', '_', f"{payload.get('assetLastUpdated')}")
    json_file_utc_run_datetime_filename = re.sub(':', '_', f"{json_file_utc_run_datetime}")
    json_file_name = f'{Path(json_file_prefix).name}_utc_run_datetime_{json_file_utc_run_datetime_filename}_utc_assetLastUpdated_{json_batch_assetLastUpdated}_{multi_proc_batch_number}.xml'
