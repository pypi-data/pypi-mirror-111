...
def _get_geo_region(nationalID: int):
    ...
    try:
        ...
        logger.error(f'Error getting geo region of nation {nationalID}')
        logger.error(e)
    finally:
        return result
...
class ReservationSync:
    ...
    def _get_booking_type(self, arrival_date: str, created_date: str):
        ...
        try:
            ...
            logger.error('Error while calculating booking type')
            logger.error(e)
        finally:
            return result
    ...
    def _get_duration_type(self, departure_date: str, arrival_date: str):
        ...
        try:
            ...
            logger.error('Error calculating stay duration type')
            logger.error(e)
        finally:
            return result
    ...
    def _get_booking_level(self, orgId, clientId, departure_date: str, arrival_date: str, spend, booking_type):
        ...
        try:
            ...
            model = get_model_from_cloud_storage(str(orgId), f'booking_categorization_{clientId}.pkl')
            ...
            logger.error('Error calculating booking_level of reservation')
            logger.error(e)
        finally:
            return result
    ...
    def _retrieve_data(self, id, name, url):
        result = None
        try:
            logger.debug(f'Extracting {name} for reservation: {id}')
            header = {
            ...
            if response.status_code == 200:
                ...
                logger.error(f'Error getting {name} for reservation {id} due to status code {response.status_code}')
                logger.error(url.format(id))
        except Exception as e:
            logger.error(f' {name} failed due to {e}')
        finally:
            return result
    ...
    def extract_reservation_details(self, record):
        ...
        try:
            logger.info(f"Extracting reservation {record.get('id')} additional details.")
            ...
            logger.error(f'Error retrieving addition reservation details for {record.get("id")}')
            logger.error(e)

    def transform_reservation(self, record):
        try:
            logger.info(f'Transforming reservation {record.get("id")}')
            ...
            logger.error(f"Exception while transforming reservation {record.get('id')}")
            logger.error(e)

    def transform_guest(self, record):
        try:
            logger.info(f'Transforming guests of reservation {record.get("id")}')
            ...
            logger.error(f'Error while transforming guest for reservation {record.get("id")}')
            logger.error(e)
    ...
    def extract_reservation(self, record):
        ...
        if response.status_code == 200:
            try:
                ...
                logger.error(f'{record} load failed due to {err}')
        else:
            logger.error(f"RMS returned status code {response.status_code} "
    ...
    def _load_to_mongodb(self, record):
        ...
        try:
            logger.info(f'Adding reservation {_id} to db {orgId}')
            self.mongo.get_collection(self.mongo.RESERVATION_COLLECTION).update_one({'_id': _id},
                                                                          {'$set': record}, upsert=True)
        except Exception as e:
            logger.error(f'Error storing to respective {_id} to db {orgId}')
            logger.error(e)
    ...
    def process(self, reservationId):
        ...
        if not validate_key(self.token.validity):
            logger.info('Refreshing RMS token as it is about to expire')
