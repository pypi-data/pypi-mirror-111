...
def rotate_scene(t):
    ...
    return mlab.screenshot(
        figure=myfig, mode="rgb", antialiased=True
    )  # return a RGB image
...
)  # force exceptions to be re-raised in Traits
root = tk.Tk()
root.withdraw()
...
file_path = filedialog.askopenfilename(
    initialdir=homedir, title="Select q values", filetypes=[("NPZ", "*.npz")]
)
...
if crop_symmetric:
    ...
    qy = qy[indices_qy]
    data = data[
        indices_qx.min() : indices_qx.max() + 1,
    ...
    rgi = RegularGridInterpolator(
        (
            np.linspace(qx.min(), qx.max(), num=nz),
    ...
    half_labels = int(
        min_range // tick_spacing
    )  # the number of labels is (2*half_labels+1)

    new_z, new_y, new_x = np.meshgrid(
        np.linspace(
            qx_com - tick_spacing * half_labels,
            qx_com + tick_spacing * half_labels,
            num=nz,
        ),
        np.linspace(
            qz_com - tick_spacing * half_labels,
            qz_com + tick_spacing * half_labels,
            num=ny,
        ),
        np.linspace(
            qy_com - tick_spacing * half_labels,
            qy_com + tick_spacing * half_labels,
            num=nx,
        ),
        indexing="ij",
...
if crop_symmetric:
    grid_qx, grid_qz, grid_qy = np.mgrid[
        ...
        + tick_spacing * half_labels : 1j * nz,
        qz_com
        - tick_spacing * half_labels : qz_com
        + tick_spacing * half_labels : 1j * ny,
        qy_com
        - tick_spacing * half_labels : qy_com
        + tick_spacing * half_labels : 1j * nx,
    ]
else:
    grid_qx, grid_qz, grid_qy = np.mgrid[
        qx.min() : qx.max() : 1j * nz,
...
if make_movie:
    if output_format == "mp4":
        animation = mpy.VideoClip(rotate_scene, duration=duration).resize(
