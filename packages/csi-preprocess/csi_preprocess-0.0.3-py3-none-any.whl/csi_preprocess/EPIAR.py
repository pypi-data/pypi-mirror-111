...
class EPIAR:
    def __init__(
        self, subcarriers_index, num_path=20, T=3.2e-6, lambda_parameter=0.01, Fc=5.0e9
    ):
        """
    ...
    def _construct_model(
        ...
        """
        assert (
            len(Tp) == self.num_path
        ), "the length of Tp must be equal to the number of paths"
        assert (
            len(Ap) == self.num_path
        ), "the length of Ap must be equal to the number of paths"
        ...
        for m in range(self.num_subcarriers):
            for p in range(self.num_path):
                t_matrix[m, p] = np.exp(
                    -1j * 2 * np.pi * self.subcarriers_index[m] * (Tp[p] + SFO + PDD)
                )
    ...
    def _model_loss(self, initial_parameter):
        """
        ...
        t_matrix, r_vector, t_r = self._construct_model(
            Tp,
            Ap,
            SFO,
            PDD,
        ...
        loss = np.linalg.norm(self.h - t_r) + self.lambda_parameter * np.linalg.norm(
            r_vector, ord=1
        )
        return loss
    ...
    def find_optimal(self, h, initial_parameter):
        ...
        """
        self.h = h
        optimal_parameters = scipy.optimize.fmin_bfgs(
            self._model_loss, initial_parameter
        )
        ...
        t_matrix_optimal, r_vector_optimal, t_r_optimal = self._construct_model(
            Tp,
            Ap,
            SFO,
            PDD,
    ...
    def denoise(self, h, initial_parameter):
        """
        :param h: the raw CSI vector
        :param initial_parameter: the initial parameter of CSI model
        :return: the denoised CSI
        """
        t_matrix_optimal, r_vector_optimal, t_r_optimal = self.find_optimal(
            h, initial_parameter
        )
        ...
        for m in range(self.num_subcarriers):
            X[m, :] = (
                np.conj(t_matrix_optimal[m, max_amp_path]) * t_matrix_optimal[m, :]
            )
...
def pod(
    ...
    """
    :param x: the raw CSI sample (complex matrix numpy.array) with shape: (batch,time,subcarrier)
    :param Tp: the time delay of the p-th path (initial_parameter)
    :param Ap: the attenuation factor of the p-th path (initial_parameter)
    :param initial_parameters: SFO (the sampling frequency offset), PDD (the packet detection delay),
                               CFO (the channel frequency offset), PPO (the phase-locked loop), and PA (the phase ambiguity)
    :param subcarriers_index: the indices of the captured sub-channel
    :param num_path: the number of signal paths
    :param T: the duration of OFDM symbol, constantly 3.2e-6
    :param lambda_parameter: the optimization loss wight
    :param Fc: the center frequency of the WiFi channel (~2.4G or ~5G)
    :return: the denoised CSI
    """
    model = EPIAR(
    ...
    initial = Tp + Ap + initial_parameters
    batch_size = np.shape(x)[0]
    time_len = np.shape(x)[1]
    result = np.ones((batch_size, time_len, len(subcarriers_index)), dtype=np.complex)
    for i in range(batch_size):
        for j in range(time_len):
            h = x[i, j, :]
            result[i, j, :] = model.denoise(h, initial)
    return result
...
if __name__ == "__main__":
    ...
    x = [
        [
            [
                5.91552818 + 5.37260757e-09j,
                5.91344323 - 1.49513892e-01j,
                5.90719052 - 2.98899580e-01j,
            ],
            [
                5.91552818 + 5.37260757e-09j,
                5.91344323 - 1.49513892e-01j,
    ...
    np_x = np.array(x, dtype=np.complex)
    print(np.shape(np_x))
