...
class te_services:
    ...
    def getPerformanceProjectData(self, tableName, count=1, consumed=True,
                                  queryJSON=None, projectName=None, isE2E=False):
        _projectName = projectName if projectName is not None else self.te_config_json[
            "Project"]
        requestUR = self.te_config_json["Server"] + "/" + \
            self.te_constant["ServerAPI"]["PROJECT_DATA"] + \
    ...
    def getPerformanceMasterData(self, tableName, count=1, consumed=True,
                                 queryJSON=None, projectName=None, isE2E=False):
        _projectName = projectName if projectName is not None else self.te_config_json[
            "Project"]
        requestUR = self.te_config_json["Server"]+"/" + \
                                    ...
                                    if os.path.exists(screenshotPath):
                                        with open(screenshotPath, "rb") as f:
                                            base64_data = base64.b64encode(
                                                f.read())
                                            step["Screenshot"] = base64_data
                            reportJSON.append(reportObj)
                else:
