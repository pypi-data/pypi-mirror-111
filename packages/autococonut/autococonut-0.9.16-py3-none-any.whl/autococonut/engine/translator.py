        ...
        if not self.report:
            ...
            for timestamp in self.report:
                ...
                # In case of pressing various keys.
                    ...
                    action = data['action']
                    key = data['key'] 
                    # In GUI, several formatting attempts are possible. In OpenQA mode, some key names
                    # are changed to comply to OpenQA. This will make sure that the keys are renamed
                    # back when we translate again with non-openqa mode.
                    if key in self.openqa_keys.values():
                        for right, wrong in self.openqa_keys.items():
                            if key == wrong:
                                data['key'] = right
                    # If the translation is in OpenQA mode. the key names should be changed for compatibility.
                    if lang == 'openqa': 
