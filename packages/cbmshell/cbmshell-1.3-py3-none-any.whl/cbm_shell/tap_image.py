...
from cbm_shell.tap_image_path_read import TapImagePathRead
from cbm_shell.tap_image_path_write import TapImagePathWrite
    ...
    def path(self, name):
        ...
        for header, data in self.cache:
            if name == header.unique_name:
                return TapImagePathRead(self.drive, header, data)
        # new file to append
        return TapImagePathWrite(self, name)
    ...
    def expand(self, name):
        ...
        for header, data in self.cache:
            if header.unique_name.startswith(name):
                yield TapImagePathRead(self.drive, header, data)
