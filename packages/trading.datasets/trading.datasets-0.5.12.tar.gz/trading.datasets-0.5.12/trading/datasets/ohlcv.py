...
class OHLCV(pd.DataFrame):
    ...
    def from_exchange(
        ...
        if cache_filepath.is_file() and use_cache:
            ...
            # Get the timerange of the cache and the actual relevant timeranges
            input_timerange = dtrangelib.DateTimeRange(start, end)
            cache_timerange = dtrangelib.DateTimeRange(cache_start, cache_end)

            try:
                cached_timerange = input_timerange.intersection(
                    cache_timerange)

                fetch_timeranges = input_timerange.subtract(cached_timerange)
        ...
        for timerange in fetch_timeranges:
            ...
            current_ohlcv = cls(
                current_raw_ohlcv, columns=cls._DEFAULT_COLUMNS)
            current_ohlcv[cls._DEFAULT_TIMESTAMP_COLUMN] = (
                datetime_utils.get_datetime(
                    current_ohlcv[cls._DEFAULT_TIMESTAMP_COLUMN]))

            current_ohlcv = current_ohlcv.set_index(
                cls._DEFAULT_TIMESTAMP_COLUMN)
        ...
        if not ohlcv[start:end].empty and has_new_data and use_cache:
    ...
    @classmethod
    def from_csv(cls, file: pathlib.Path | str, **kwargs) -> OHLCV:
        """Returns an instance of an OHLCV based on the given parameters.
        ...
        ohlcv[cls._DEFAULT_TIMESTAMP_COLUMN] = datetime_utils.get_datetime(
            ohlcv[cls._DEFAULT_TIMESTAMP_COLUMN])
        ohlcv = ohlcv.set_index(cls._DEFAULT_TIMESTAMP_COLUMN)
        ohlcv = ohlcv.sort_index()

        return ohlcv

    @classmethod
    def from_parquet(cls, file: pathlib.Path | str, **kwargs) -> OHLCV:
        """Returns an instance of an OHLCV based on the given parameters.
