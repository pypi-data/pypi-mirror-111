...
class DFS():
	...
	def __dfs(self, G, source): 
		...
		dfs_stk.append(self.__DFSUtil(G, source, visited, sl))
		for i in range(len(G.nodes())):
			if visited[i] == False:
				sl = []
				dfs_stk.append(self.__DFSUtil(G, i, visited, sl))
		return dfs_stk
	...
	def __DrawDFSPath(self, G, dfs_stk):
	...
	def depth_first_search(self, filename):
		G, source = self.__CreateGraph(filename)
		dfs_stk = self.__dfs(G, source)
		self.__DrawDFSPath(G, dfs_stk)
		plt.show()
