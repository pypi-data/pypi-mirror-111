...
class FvExtractor(Extractor, Mesh):
    ...
    def write_time_series_file(self, out_file, locations, variables=None):
        """
        ...
        else:  # Need to include lfz and stat in variable list for compatability with FvTimeseries
            for var in ["layerface_Z", "stat"]:
                variables.append(var) if var not in variables else variables
        ...
        for var in variables:
            vardims = self.nc.variables[var].shape
            if (vardims[1] == self.nc2) & (len(vardims) == 2):
                num_dim[var] = 2
                num_subvars[var] = 0
            elif vardims[1] == self.nc3:
                num_dim[var] = 3
            elif (vardims[1] == self.nc2) & (len(vardims) == 3):  # 2D var with 3rd dim (e.g. BED_MASS with sed fracs)
                num_dim[var] = 2
                num_subvars[var] = vardims[-1]
                nc_subdims[var] = self.nc[var].dimensions[-1]

        # Get 2D cell index (ii) of each point
        idx = {}
        for name, point in locations.items():
            idx[name] = self.get_cell_index(point[0], point[1])[0]
            if idx[name] == -999:
        ...
        for name in locations:
            ...
            for var in variables:
                ...
                elif (num_dim[var] == 2) & (num_subvars[var] == 0):
                    size = (self.nt, 1)
                elif (num_dim[var] == 2) & (num_subvars[var] > 0):
                    size = (self.nt, num_subvars[var])
        ...
        for var in variables:
            for ii in range(self.nt):
                ...
                for name in locations:
                    if idx[name] == -999:  # point outside model domain
                        continue
                    elif var == 'layerface_Z':
        ...
        for name in locations:
            ...
            for dim in set(nc_subdims.values()):  # Add sub-dims (e.g. sediment fractions)
                nc[name].createDimension(dim, self.nc.dimensions[dim].size)
            ...
            for var in variables:
                if var == 'layerface_Z':
                    ...
                    dims = ('time', self.nc[var].dimensions[-1])
                nc[name].createVariable(var, 'f8', dimensions=dims, fill_value=np.nan)
                nc[name].variables[var].units = self.nc[var].units
                nc[name].variables[var].long_name = self.nc[var].long_name
                nc[name].variables[var][:] = profile[name][var]

        nc.close()
        return

    def write_data_to_ascii(self, out_file, data, resolution, precision=2, bbox=None, grid_index=None):
        """
