...
class ConfItem:
    ...
    @classmethod
    def decode(cls, parser, json_data: dict, pkg_prefix: str, filename: str):
        """
        ...
        if not key.islower():
            # logger.warning('Conf id_ should be lowercase: %s' % id_)
            key = key.lower()

        item = ConfItem(parser)
        item.id = '%s.%s' % (pkg_prefix, key)
...
class IConf(ABC):
    ...
    def version(self, update=False):
        ...
        if old is None:
            old = ConfItem()
            old.name = 'Confit options version identifier'
            old.raw_sys_value = 0
