...
class ConfFS(IConf):
    ...
    def __init__(self, conf_root: str, parser: ValueParser = None):
    ...
    def _load_conf_file(self, filename: str) -> List[ConfItem]:
        ...
        for json_item in json_content:
            item = ConfItem.decode(self.parser, json_item, pkg_prefix, filename)
            if not item:
                continue
            items.append(item)
