...
def detect_image(network, class_names, image, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    # convert the image to valid input (1. rgb 2.resize)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    image_resized = cv2.resize(image_rgb, (width, height), interpolation=cv2.INTER_LINEAR)

    # copy image to darknet_image
    darknet_image = make_image(width, height, 3)
    copy_image_from_bytes(darknet_image, image_resized.tobytes())

    pnum = pointer(c_int(0))
    predict_image(network, darknet_image)
    detections = get_network_boxes(network, darknet_image.w, darknet_image.h,
    ...
    return width, height, sorted(predictions, key=lambda x: x[1])
...
def prepare_batch(images, width, height, channels=3):
    darknet_images = []
    for image in images:
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
...
def detect_one_batch(network, class_names, images, batch_size=4, thresh=.5, hier_thresh=.5, nms=.45):
    """
        Returns a list with highest confidence class and their bbox
    """
    width = network_width(network)
    ...
    darknet_images = prepare_batch(images, width, height)
    ...
    return width, height, batch_predictions
...
if os.name == "nt":
    ...
    if not 'DARKNET_PATH' in os.environ:
        assert False, 'please add place of libdarknet.so to DARKNET_PATH'
