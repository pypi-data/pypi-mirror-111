...
def _plot_complex_arg(ax, cmap, facecolor="black", title=False):
    ...
    if title:
        ...
        ax.set_title(r"Argument of $(z^2 - 1)(z - 2 - j)^2$", color=title_color, fontsize=14)
...
def _plot_e_field(ax, cmap, n_neg_charges=2, facecolor="black", title=False):
    ...
    ax.streamplot(x, y, Ex, Ey, color="black", linewidth=.5,
                  density=0.5, arrowstyle='fancy', arrowsize=1)
    ...
    for q, pos in charges:
        ...
        ax.annotate(charge_label[q > 0], xy=pos, fontsize=30, ha="center", va=label_align[q > 0])
    ...
    ax.set_aspect('equal')
    if title:
        ax.set_title(r"Angle of $\vec{E}$ with the x-axis", color=title_color, fontsize=14)
...
def _plot_examples(color_map, images, arr_3d, figsize, facecolor, cname, cblind=True, norm=False):
    ...
    for c_map, im in itertools.product(c_maps, images):
        ...
        if isinstance(im, str) and ('3D' in im):
            ...
            ax = _plot_e_field(ax, c_map, n_neg_charges=2, facecolor=facecolor, title=axi <= n_cols)
...
def _plot_examples_qual(color_map, dict_arr, figsize, facecolor, cname, year):
    ...
    for c_map, d in itertools.product(c_tabs, dict_arr):
        ...
        if axi in range(1, n_rows * n_cols, len(dict_arr)):
            ...
            scatter = ax.scatter(x, y, cmap=ListedColormap(np.clip(c_map, 0, 1)), s=s, c=c)
