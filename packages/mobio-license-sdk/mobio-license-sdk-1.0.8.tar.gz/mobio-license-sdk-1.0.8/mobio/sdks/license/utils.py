...
class Utils:
    ...
    def create_number_mess_need_used(
        license_key, merchant_id, number_mess_need_used=None, day_of_month=None
    ):
        ...
        try:
            ...
            if data_mess_month_now:
                ...
                if (mess_base - mess_base_used) >= number_mess_need_used:
                    ...
                    Utils.save_history_license_used_mess(data_mess_month_now)
                else:
                ...
                if (mess_base - mess_base_used) >= number_mess_need_used:
                    ...
                    history_id = Utils.save_history_license_used_mess(data_mess_save)
                    create_on = convert_date_to_format(get_utc_now())
                    for detail_mess in data_detail_mess:
                        detail_mess[LicenseDetailMessUsedModel.history_id] = history_id
                        detail_mess[
    ...
    @staticmethod
    def save_history_license_used_mess(data_mess):
        try:
            ...
            ] = convert_date_to_format(get_utc_now())
    ...
    def check_merchant_expire(license_key, merchant_id):
        ...
        try:
            result = CryptUtil.get_license_info(
                license_key, merchant_id
            )
            if result and result.get("expire_time") and convert_timestamp_to_date_utc(result.get("expire_time")):
                time_stamp_now = convert_date_to_timestamp(get_utc_now())
                if result.get("expire_time") > time_stamp_now:
                    merchant_expire = False
                else:
                    print("license_sdk::check_time_merchant_expire license merchant expire_time")
            else:
                print("license_sdk::check_time_merchant_expire license expire_time not found")
