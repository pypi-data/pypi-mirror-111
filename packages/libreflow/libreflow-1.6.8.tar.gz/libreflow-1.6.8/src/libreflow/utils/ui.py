...
class TaskFileItem(QtWidgets.QTreeWidgetItem):
    ...
    def submit_playblast_job(self):
        ...
        kwargs = dict(
            ...
            use_simplify=(self.checkState(2) == QtCore.Qt.Checked),
        )
        
        return self._tree.parentWidget().submit_playblast_job(**kwargs)
    
    def _update(self):
        f = self.file
        self.setText(0, f['name'])
        self.setText(2, '')
        self.combo_box_revisions.addItems(f['revisions'])
        self.combo_box_revisions.setCurrentIndex(
            self.combo_box_revisions.count() - 1
        )
...
class ShotsItem(QtWidgets.QTreeWidgetItem):
    ...
    def _update(self):
        ...
        shots_data = self._tree.parentWidget().get_shots_data()
...
class RenderSequencePlayblastsWidget(CustomPageWidget):
    ...
    def get_shot_task_names(self):
        return self.session.cmds.Flow.call(
            ...
            args={}, kwargs=dict(fnpattern='*.blend')
        )
    
    def submit_playblast_job(self, **kwargs):
        kwargs['pool_name'] = self.combobox_pool.currentText()
        kwargs['priority'] = int(self.lineedit_priority.text())
        
        return self.session.cmds.Flow.call(
            oid=self.oid,
            method_name='submit_playblast_job',
            args={}, kwargs=kwargs
        )
