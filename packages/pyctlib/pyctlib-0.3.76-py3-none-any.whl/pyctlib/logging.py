...
class Logger:
    ...
    def __init__(self, stream_log_level=logging.DEBUG, file_log_level=None, deltatime: bool=False, name: str="logger", c_format=None, file_path=None, file_name=None, f_format=None, disable=False, autoplot_variable=False, notion_page_link=None):
        ...
        self.__update_notion_buffer = vector()
    ...
    def update_notion(self, variable_name: str, variable):
        try:
            self.__update_notion_buffer.append((variable_name, variable))
            self.__update_notion()
        except TimeoutError as e:
            self.warning("update notion faild: time out", self.__update_notion_buffer)
            touch(lambda: delattr(self, "_notion_client"))
            touch(lambda: delattr(self, "_notion_page"))
        else:
            assert self.__update_notion_buffer.length == 0
    ...
    def __update_notion(self):
        ...
        try:
            while self.__update_notion_buffer.length > 0:
                variable_name, variable = self.__update_notion_buffer[0]
                ...
                self.__update_notion_buffer.pop(0)
        except:
            self.warning("update notion page failed", self.__update_notion_buffer)
            raise TimeoutError
