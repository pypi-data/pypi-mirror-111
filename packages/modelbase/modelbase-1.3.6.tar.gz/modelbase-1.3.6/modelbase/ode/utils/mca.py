...
def _find_steady_state(
    *,
    model: Model,
    y0: Union[List[float], np.ndarray, Dict[str, float]],
...
def get_response_coefficients(
    model: Model,
    parameter: str,
    y: Union[Dict[str, float], np.ndarray, List],
    normalized: bool = True,
    displacement: float = _DISPLACEMENT,
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    conc_resp_coef: np.ndarray = (ss[0] - ss[1]) / (2 * displacement * old_value)
    flux_resp_coef: np.ndarray = (fluxes[0] - fluxes[1]) / (2 * displacement * old_value)

    if normalized:
        conc_resp_coef *= y_ss_norm
        flux_resp_coef *= fluxes_array_norm
...
def get_concentration_response_coefficient(
    ...
    return get_response_coefficients(
        model=model, parameter=parameter, y=y, normalized=normalized, displacement=displacement
    )[0]
...
def get_response_coefficients_array(
    ...
    crcs = np.full(shape=(len(parameters), len(model.get_compounds())), fill_value=np.nan)
