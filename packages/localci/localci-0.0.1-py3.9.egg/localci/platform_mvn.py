...
class Maven(Platform):
    def make(self) -> None:
        ...
        confirmation = Prompt.ask(f'Building Maven project: {application_name}',
                                  choices=["Y", "N"])
        if confirmation != "Y":
        ...
        with console.status("[bold green] Building Maven project...") as status:
            ...
            # Build with maven
            status.update(status='Building package with Maven...')
            if call('mvn clean package') != 0:
                console.print("There has been an error with [bold]mvn clean package[/bold]", style="bold red")
                exit()

            console.log(':white_check_mark: Built application package with Maven.')
