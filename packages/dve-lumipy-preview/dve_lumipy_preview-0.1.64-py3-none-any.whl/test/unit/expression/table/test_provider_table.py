...
class TestProviderClasses(unittest.TestCase):
    ...
    def test_provider_instance_ctor(self):
        ...
        for cls in self.atlas.list_providers():

            if any(p.get_type() == SqlValType.Table for p in cls.list_parameters()):
                # Just test ctors that take scalar values
                continue

            param_input = generate_params_input(cls)
    ...
    def test_provider_instance_columns(self):
        ...
        for cls in self.atlas.list_providers():
            ...
            self.assertEqual(len(inst.get_columns()), len(cls.list_columns()))
    ...
    def test_provider_wrong_params_failure(self):
        ...
        self.assertIn(cls.get_table_name().replace('.', ''), msg)
