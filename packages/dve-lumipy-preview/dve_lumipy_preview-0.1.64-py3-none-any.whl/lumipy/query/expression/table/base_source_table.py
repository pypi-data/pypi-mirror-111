...
class BaseSourceTable(BaseTable):
    ...
    def outer_join(self, other, on, left_alias='lhs', right_alias='rhs'):
        left = self.left_join(other, on, left_alias, right_alias).select('*')
        right = other.left_join(self, on, right_alias, left_alias).select('*')
        return left.union(right).to_table_var()
