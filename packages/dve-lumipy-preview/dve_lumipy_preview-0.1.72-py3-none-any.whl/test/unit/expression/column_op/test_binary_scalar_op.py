...
class TestColumnBinaryScalarOp(unittest.TestCase):
    ...
    def test_char_index(self):
        ...
        char_index_0 = self.test_text_col.char_index('lu')
        test_prefix_insertion(self, self.appreq, char_index_0)
        self.assertEqual(char_index_0.get_sql(), "charindex([RequestId], 'lu', 0)")

        char_index_2 = self.test_text_col.char_index('lu', 2)
        test_prefix_insertion(self, self.appreq, char_index_2)
        self.assertEqual(char_index_2.get_sql(), "charindex([RequestId], 'lu', 2)")

    def test_python_str_method_analogues(self):

        # starts with
        starts = self.test_text_col.starts_with('test', case_sensitive=True)
        self.assertEqual(starts.get_sql(), "[RequestId] GLOB 'test*'")
        starts = self.test_text_col.starts_with('test', case_sensitive=False)
        self.assertEqual(starts.get_sql(), "[RequestId] LIKE 'test%'")

        # Ends with
        ends = self.test_text_col.ends_with('test', case_sensitive=True)
        self.assertEqual(ends.get_sql(), "[RequestId] GLOB '*test'")
        ends = self.test_text_col.ends_with('test', case_sensitive=False)
