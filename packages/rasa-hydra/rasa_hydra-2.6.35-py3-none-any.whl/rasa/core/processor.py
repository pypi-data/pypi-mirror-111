...
class MessageProcessor:
    ...
    async def _parse_message(self, message, tracker: DialogueStateTracker = None):
        ...
        logger.debug("[{}] - Received parse_data '{}'".format(message.sender_id, parse_data))
        ...
        if message_type == "SPOKEN" and len(partials) > 0 and (
            ...
            logger.debug("Entering partials strategy with metadata: {}".format(message.metadata))
            logger.debug("Entering partials strategy with parse_data: {}".format(parse_data))
            ...
            logger.info(
                "[{}] - Beginning partials strategy with '{}' as partials".format(message.sender_id,
                                                                                  self.convert_pci_message(str(partials))))
            for partial in partials:
                parse_data = await self.interpreter.parse(partial, message.message_id, tracker)
                if parse_data["intent"]["name"] and parse_data["intent"]["confidence"] > confidence_threshold and (
                        parse_data["intent"]["name"] != "action_handle_error" or
                        parse_data["intent"]["name"] != "out_of_scope"):
                    logger.info(
                        "[{}] - Received user message '{}' with intent '{}' "
                        "and entities '{}' from partial".format(
                            message.sender_id, self.convert_pci_message(partial), parse_data["intent"], parse_data["entities"]
                        ))
    ...
    def get_last_bot_msg(self, tracker: DialogueStateTracker):
        for event in reversed(tracker.events):
            if (event.__class__.__name__ == "BotUttered"):
                return event
        return None
    ...
    def create_metadata(
        ...
        if last_bot_msg and last_bot_msg.metadata and "template_name" in last_bot_msg.metadata:
    ...
    def convert_pci_message(self, message):
        pci_msg = message
        if len(pci_msg)>0 and pci_msg[-1] in string.punctuation:
            pci_msg = pci_msg[:-1]
        pci_msg = t2d.convert(pci_msg.replace(',', ''))
    ...
    def convert_pci_entities(self, message):
        ...
        for i in pci_msg:
            if i['entity'] == 'number':
                num_list.append(i)
            elif len(num_list) != 13 and len(num_list) !=15 and len(num_list) != 16 and len(num_list) != 19:
                num_list = []

        for e in pci_msg:
            e['value'] = re.sub(r"(?<!\d)(\d{13}|\d{15}|\d{16}|\d{19})(?!\d)", '****************', str(e['value']))
        if len(num_list) == 13 or len(num_list) == 15 or len(num_list) == 16 or len(num_list) == 19:
            for e in pci_msg:
                if e['entity'] == 'number' and filter(lambda x: x['start'] == (e['start'] and  x['end'] == e['end']), num_list):
                    e['value'] = "****"
                    e['text'] = "****"
        return pci_msg
    ...
    async def _handle_message_with_tracker(
        ...
        if should_redact:
            ...
            if original_text:
                redacted_original_text = self.convert_pci_message(original_text)
                message.metadata['original_text'] = UserUttered.redact_msg(redacted_original_text)
            if partials and len(partials) > 0:
                redacted_partials = []
                for partial in partials:
                    if partial and len(partial) > 0:
                        redacted_partial = self.convert_pci_message(partial)
                        redacted_partials.append(UserUttered.redact_msg(redacted_partial))
                message.metadata['partials'] = redacted_partials
