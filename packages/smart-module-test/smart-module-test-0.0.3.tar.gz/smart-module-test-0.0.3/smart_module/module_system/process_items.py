...
def save_python_header(src_dir):
  file = open(src_dir + "ID_items.py","w",encoding="utf-8")
  for i_item in range(len(items)):
    file.write("itm_%s = %d\n"%(convert_to_identifier(items[i_item][0]),i_item))
  file.close()

def write_items(variable_list,variable_uses,tag_uses,quick_strings,export_dir):
  ...
  for item in items:
    if (item[3] & itp_merchandise) > 0:
      id_no = find_object("itm",convert_to_identifier(item[0]))
...
def processItems(context):
    ...
    write_items(variables,variable_uses,tag_uses,quick_strings,export_dir)
