...
def write_actions(action_set,num_action_codes,action_codes,file_name,export_dir):
...
def save_python_header(action_codes,src_dir):
  ofile = open(src_dir + "ID_animations.py","w",encoding="utf-8")
...
def processAnimations(context):
  ...
  ## 深拷贝
  animations_new = copy.deepcopy(animations)

  action_codes = compile_action_sets(animations_new)
  save_python_header(action_codes,src_dir)
  write_actions(animations_new,len(action_codes),action_codes,"actions.txt",export_dir)
