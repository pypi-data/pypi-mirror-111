...
def write_python_header(sounds,src_dir):
  file = open(src_dir + "ID_sounds.py","w",encoding="utf-8")
  for i_sound in range(len(sounds)):
    file.write("snd_%s = %d\n"%(sounds[i_sound][0],i_sound))
  file.write("\n\n")
  file.close()

def write_sounds(sound_samples, sounds,export_dir):
...
def processSounds(context):
  ...
  write_sounds(sound_samples, sounds,export_dir)
  write_python_header(sounds,src_dir)
