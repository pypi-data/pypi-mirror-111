from smart_module.module_system.module_animations import *
from smart_module.module_system.module_factions import *
from smart_module.module_system.module_game_menus import *
from smart_module.module_system.module_items import *
...
from smart_module.module_system.module_particle_systems import *
from smart_module.module_system.module_parties import *
...
from smart_module.module_system.module_scene_props import *
...
from smart_module.module_system.module_sounds import *
from smart_module.module_system.module_strings import *
from smart_module.module_system.module_tableau_materials import *
...
def save_statement(ofile, opcode, no_variables, statement, variable_list, variable_uses, local_vars_list,
                   local_var_uses, tag_uses, quick_strings):
    if no_variables == 0:
    ...
    for i in range(lenstatement):
        operand = process_param(statement[i + 1], variable_list, variable_uses, local_vars_list, local_var_uses,
...
def compile_global_vars_in_statement(statement, variable_list, variable_uses):
    opcode = 0
    if ((type(statement) != list) and (type(statement) != tuple)):
        opcode = statement
    else:
        opcode = statement[0]
...
def save_statement_block(ofile, statement_name, can_fail_statement, statement_block, variable_list, variable_uses,
                         tag_uses, quick_strings):
    local_vars = []
    local_var_uses = []
    ...
    for i in range(len(statement_block)):
        ...
        save_statement(ofile, opcode, no_variables, statement, variable_list, variable_uses, local_vars, local_var_uses,
                       tag_uses, quick_strings)
