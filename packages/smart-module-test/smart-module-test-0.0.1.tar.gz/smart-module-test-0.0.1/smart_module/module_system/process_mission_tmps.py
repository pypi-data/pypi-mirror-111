...
from smart_module.module_system.process_tags_unused import tag_uses
...
def save_mission_template_group(file,entry):
  if (len(entry[5]) > 8):
    ...
    raise RuntimeError("ERROR: Too many item_overrides!")
...
def save_mission_templates(variables,variable_uses,tag_uses,quick_strings,export_dir):
  file = open(export_dir + "mission_templates.txt","w",encoding="utf-8")
...
def processMissionTmps(context):
  ...
  save_mission_templates(variables, variable_uses, tag_uses, quick_strings,export_dir)
