...
def save_scene_props(variable_list,variable_uses,tag_uses,quick_strings,export_dir):
...
def save_python_header(src_dir):
  file = open(src_dir + "ID_scene_props.py","w",encoding="utf-8")
  for i_scene_prop in range(len(scene_props)):
    file.write("spr_%s = %d\n"%(scene_props[i_scene_prop][0],i_scene_prop))
  file.close()


def processSceneProps(context):
  ...
  save_scene_props(variables,variable_uses,tag_uses,quick_strings,export_dir)
