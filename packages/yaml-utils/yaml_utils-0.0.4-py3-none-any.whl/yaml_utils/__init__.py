...
def get_parent_key_and_target_value(obj, target_key):
    """
    Method to get parent key and value of target for passed target key
...
def get_parent_and_dynamic_target_value(obj, target_key, dynamic_value_list, replaceable_word="replace"):
    """
    Method to get parent key and target value of target for passed target key
    and replace target value in run time using dynamic values
    :param obj: dict or list[dict]
    :param target_key: str
    :param dynamic_value_list: list[str]
    :param replaceable_word: str (default_value: replace)
    :return: list[str]
    """
    loc, val = get_parent_key_and_target_value(obj, target_key)
    word_count = val.count(replaceable_word)
    if word_count == len(dynamic_value_list):
        for dynamic_val in dynamic_value_list:
            val = val.replace(replaceable_word, dynamic_val, 1)
    else:
        raise Exception(f"Given dynamic value list '{dynamic_value_list}'"
                        f" count not match with replaceable word occurrence count '{val}'")
    lst = [loc, val]
    return list(lst)
