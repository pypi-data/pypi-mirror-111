...
def _wrap_class(  # noqa: C901
    ...
    logical_table = (
        ...
        + hashlib.sha1((str(cls.__dict__.keys).encode())).hexdigest()[:15]
    )
...
class DynamoCrudRepository(CrudRepository):
            ...
            if hasattr(self.entity_type, "dynamo_pk"):
                ...
                scan_response = self.table.scan(
                    FilterExpression=Attr(
                        self.entity_type.dynamo_sk_name_on_table()
                    ).begins_with(self.entity_type.dynamo_logical_table_name())
                )
        except Exception as ex:
            print(ex)
    ...
    def item2instance(self, item):
        ...
        if not hasattr(
            entity_instance, "dynamo_pk"
        ):  # If using id instead of pk and sk
            pk = item[self.entity_type.dynamo_pk_name_on_table()]
            setattr(entity_instance, self.entity_type.dynamo_id(), _pk2id(pk))
