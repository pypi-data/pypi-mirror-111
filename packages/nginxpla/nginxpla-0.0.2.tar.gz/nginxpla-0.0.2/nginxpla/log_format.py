...
from nginxpla.error import error_exit, choose_one
...
def detect_config_path():
    ...
    try:
        ...
        return os.path.abspath(os.path.join(__file__, os.pardir)+'/../nginx.conf')
...
def get_access_logs(config):
    ...
    """
    access_log = Literal("access_log") + ZeroOrMore(parameter) + Literal(';').suppress()
...
def get_log_formats(config):
    ...
    log_format = Literal('log_format') + parameter + Group(OneOrMore(parameter)) + Literal(';').suppress()
