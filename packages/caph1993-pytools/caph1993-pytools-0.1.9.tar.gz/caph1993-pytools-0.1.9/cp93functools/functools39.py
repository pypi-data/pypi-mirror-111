...
def reduce(function, sequence, initial=_initial_missing):
  ...
  if initial is _initial_missing:
    try:
      ...
      raise TypeError(
          "reduce() of empty sequence with no initial value") from None
  else:
    value = initial
...
class partial:
  ...
  def __reduce__(self):
    return type(self), (self.func,), (self.func, self.args, self.keywords or
                                      None, self.__dict__ or None)
...
class partialmethod(object):
  ...
  def __repr__(self):
    ...
    return format_string.format(module=self.__class__.__module__,
                                cls=self.__class__.__qualname__, func=self.func,
...
def _make_key(args, kwds, typed, kwd_mark=(object(),), fasttypes={int, str},
...
def _c3_mro(cls, abcs=None):
  ...
  for base in abcs:
    if issubclass(cls,
                  base) and not any(issubclass(b, base) for b in cls.__bases__):
...
def _compose_mro(cls, types):
  ...
  def is_related(typ):
    return (typ not in bases and hasattr(typ, '__mro__') and
            issubclass(cls, typ))
...
def _find_impl(cls, registry):
  ...
  for t in mro:
    if match is not None:
      ...
      if (t in registry and t not in cls.__mro__ and
          match not in cls.__mro__ and not issubclass(match, t)):
