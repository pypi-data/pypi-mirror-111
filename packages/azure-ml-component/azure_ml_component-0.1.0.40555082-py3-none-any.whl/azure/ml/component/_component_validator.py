...
from ._util._utils import _get_or_sanitize_python_name, _get_parameter_static_value, is_float_convertible, \
    ...
    _dumps_raw_json, basic_python_type, _is_json_string_convertible, _sanitize_python_variable_name
...
class ComponentValidator(object):
    ...
    def validate_component_inputs(workspace, provided_inputs,
                                  interface_inputs, param_python_name_dict, process_error, component_name,
        ...
        If for_definition_validate is True, it validating pipelien definition and won't check input value.
        ...
        :param component_name: Name of the validated component
        ...
        for _input in interface_inputs:
            ...
            if is_pipeline_param and for_definition_validate:
                # if a input is pipeline parameter for definition validation, link it
                ComponentValidator.link_pipeline_parameter_and_assigned_parameter(
                    provided_input_value, _input, component_name)
    ...
    def validate_component_parameters(provided_parameters,
        ...
        def validate_pipeline_parameter(param: PipelineParameter, linked_param: StructuredInterfaceParameter, _type):
            # TODO: validate range
            if param.type:
                # if param is annotated with type, it should match with linked param
                if param.type != _type:
                    process_parameter_error(
                        ValueError("Parameter '{}' type mismatched, expected type: '{}', got '{}'.".format(
                            linked_param.name, _type, param.type
                        )),
            ...
            ComponentValidator.link_pipeline_parameter_and_assigned_parameter(
                param, linked_param, component_name)

        def validate_parameter_assignment(
                param: _ParameterAssignment, linked_param: StructuredInterfaceParameter, _type):
            if _type != 'str':
                process_parameter_error(
                    ValueError("Parameter '{}' type mismatched, expected type: '{}', got 'str'.".format(
                        linked_param.name, _type
                    )),
                    ComponentValidationError.INVALID_PARAMETER,
                    linked_param.name
                )
            ...
            for pipeline_param in param.assignments_values_dict.values():
                if isinstance(pipeline_param, PipelineParameter):
                    # TODO: raise error when pipeline_param is annotated with input
                    # TODO: store a special link for parameter assignment
                    ComponentValidator.link_pipeline_parameter_and_assigned_parameter(
                        pipeline_param, linked_param, component_name)
            ...
            if provided_param_value is None:
                ...
                if for_definition_validate:
                    ...
                    if isinstance(provided_param_value, PipelineParameter):
                        validate_pipeline_parameter(provided_param_value, parameter, required_parameter_type)
                        continue

                    # if provided param is parameter assignment, only check meta
                    if isinstance(provided_param_value, _ParameterAssignment):
                        validate_parameter_assignment(provided_param_value, parameter, required_parameter_type)
                        continue
    ...
    def link_pipeline_parameter_and_assigned_parameter(
        ...
        :param component_name: validating component name
        """
        param.linked_params[":".join([component_name, assigned_param.name])] = assigned_param
...
class Diagnostic(object):
    ...
    @classmethod
    def create_diagnostic_by_component_definition(cls, definition: ComponentDefinition, error_msg: str,
                                                  error_type: str, error_var_name: str=None,
                                                  error_var_type: str=None,
                                                  component=None):
        """
        ...
        if component is not None:
            component_name = _sanitize_python_variable_name(definition._node_id_variable_name_dict[component._id])
        else:
            component_name = None
