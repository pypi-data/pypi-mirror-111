...
from azure.ml.component._restclients.designer.models import StructuredInterfaceParameter
from azureml.exceptions import UserErrorException
from azureml.core import Workspace

from azure.ml.component._util._utils import _is_empty_dir, _sanitize_python_variable_name, _extract_zip
from azure.ml.component._util._yaml_utils import YAML, YamlFlowList, YamlMultiLineStr, YamlFlowDict
from azure.ml.component._api._api import ModuleAPI, ComponentAPI, _pipeline_definition_to_dto
...
class ComponentDefinition(Component):
    ...
    def __init__(
        ...
        self._inputs = {
            name: data if not isinstance(data, dict) else InputDefinition.load({'name': name, **data})
            for name, data in inputs.items()
        }
        self._parameters = {
            name: data if not isinstance(data, dict) else ParameterDefinition.load({'name': name, **data})
            for name, data in parameters.items()
        }
        self._outputs = {
            name: data if not isinstance(data, dict) else OutputDefinition.load({'name': name, **data})
    ...
    def load(cls, yaml_file):
        ...
        with open(yaml_file) as fin:
            ...
            return definition_cls._from_dict(data, base_dir=base_dir)
    ...
    @classmethod
    def _from_dict(cls, dct, ignore_unexpected_keywords=True, base_dir=None) -> Union[
...
class CommandComponentDefinition(ComponentDefinition):
    ...
    def get_snapshot(self, target=None, overwrite=False) -> Path:
        ...
        if self._snapshot_local_cache and Path(self._snapshot_local_cache).exists():
            ...
            if target is None:
                target = mkdtemp()
            snapshot_path = self.api_caller.download_snapshot_by_id(self.snapshot_id, target)
            _extract_zip(zip_file=snapshot_path, target_dir=target)
            # remove the zip file to align with previous behaviro
            try:
    ...
    @classmethod
    def _from_dict(cls, dct, ignore_unexpected_keywords=False, base_dir=None) -> 'CommandComponentDefinition':
        """Load a component definition from a component yaml dict."""
...
class ParallelComponentDefinition(CommandComponentDefinition):
    ...
    @classmethod
    def _from_dict(cls, dct, base_dir=None) -> 'ParallelComponentDefinition':
...
class HDInsightComponentDefinition(CommandComponentDefinition):
    ...
    @classmethod
    def _from_dict(cls, dct, base_dir=None) -> 'HDInsightComponentDefinition':
...
class ScopeComponentDefinition(ComponentDefinition):
    ...
    @classmethod
    def _from_dict(cls, dct, base_dir=None) -> 'ScopeComponentDefinition':
        component_type = None if 'type' not in dct else dct.pop('type')
        if component_type != cls.TYPE.value:
            raise ValueError("The type must be %r, got %r." % (cls.TYPE.value, component_type))
...
class PipelineComponentDefinition(ComponentDefinition):
    ...
    def __init__(self, name, id=None, components: dict=None, components_variable_names=None, description=None,
        ...
        :param pipeline_parameters: List of pipeline parameters.
        :type pipeline_parameters: List[PipelineParameters]
        """
        super().__init__(
        ...
        self._module_dto = _pipeline_definition_to_dto(self)
    ...
    def generate_interface(self):
        ...
        Validate inputs/parameters/runsettings of all components inside pipeline.
        ...
        def process_definition_error(component, e: Exception, error_type, variable_name, variable_type):
            ve = ComponentValidationError(str(e), e, error_type)
            errors.append(Diagnostic.create_diagnostic_by_component_definition(
                self, ve.message, ve.error_type, variable_name, variable_type, component=component))

        # 1. validate components meta
        for component in self.components.values():
            # skip validation for local component since they don't have dto
            if component._module_dto is None:
                continue
            validation_info = component._validate_component(
                raise_error=False,
                pipeline_parameters={p.name: p for p in self._pipeline_parameters},
                is_local=False,
                default_datastore=component._resolve_default_datastore(),
                for_definition_validate=True,
                process_error_func=partial(process_definition_error, component)
            )
            component.validation_info = validation_info
            errors.extend(validation_info)
        # 2. validate pipeline parameters
        for param in self._pipeline_parameters:
            if len(param.linked_params) == 0:
                process_definition_error(
                    component=None,
                    e=ValueError("Unused pipeline parameter {}".format(param.name)),
                    error_type=ComponentValidationError.UNUSED_PARAMETER,
                    variable_name=param.name,
                    variable_type=VariableType.Parameter
                )
                # TODO: replace this error with warning and remove current warning

            linked_param_types = set(
                [p.parameter_type if isinstance(
                    p, StructuredInterfaceParameter) else "input" for p in param.linked_params.values()])
            if len(linked_param_types) > 1:
                process_definition_error(
                    component=None,
                    e=ValueError("Pipeline parameter {} has been assigned to multiple types of parameters: {}".format(
                        param.name, list(param.linked_params.keys()))),
                    error_type=ComponentValidationError.MULTIPLE_TYPES_ASSIGNMENT,
                    variable_name=param.name,
                    variable_type=VariableType.Parameter
                )
        # 3. TODO(wanhan): generate the interface according to the inferred meta, if not able to infer, set none to
        # skip future check
        return errors
