...
class SubPipelinesInfoBuilder(object):
    ...
    def build(self):
        # check if module_node_to_graph_node_mapping matches the total expanded node count
        # if not match, we cannot generate subpipeline info right, so return None
        # this could happen when the pipeline has sub pipeline node and sub pipeline registration is enabled
        if len(self.module_nodes) != len(self.module_node_to_graph_node_mapping.keys()):
            return None
    ...
    def _get_sub_graph_info(self, pipeline):
        ...
        # TODO: Move comment into init after SMT supported.
        sub_graph_info.comment = pipeline.comment
