...
class Docker:
    ...
    def _from_dict(cls, dct):
        ...
        if dct.get('base_dockerfile', None):
            # In case base_dockerfile is in 'file:xxx' format
            file_path = _parse_file_path(dct['base_dockerfile'])
            if file_path:
                return cls._from_dockerfile(file_path)
            return cls(base_dockerfile=dct['base_dockerfile'])
        return cls()
...
class CondaDependencies:
    def __init__(self, name: str = None, channels: [str] = None, dependencies: [str] = None):
...
class Conda:
    ...
    def _from_dict(cls, dct):
        ...
        if dct.get(cls.CONDA_FILE_KEY, None):
            file_path = dct[cls.CONDA_FILE_KEY]
            return cls(CondaDependencies._from_file(file_path))
        if dct.get(cls.PIP_REQUIREMENTS_KEY, None):
            file_path = dct[cls.PIP_REQUIREMENTS_KEY]
            return cls(CondaDependencies._from_pip_requirements_file(file_path))
        return cls()
...
def _parse_file_path(value: str):
    ...
    return value[len(FILE_HINT_IN_YAML):]  \
        if value and value.startswith(FILE_HINT_IN_YAML) else None
