...
def _serialize_input(input_name, component, provider, with_data=False, with_def=False):
    ...
    if input_name in component.inputs:
        ...
        entity_dict = input_def.to_dict(remove_name=True) if with_def else {}
        input = component.inputs[input_name]
        ...
        assert input_name in component._parameter_params
        param_def = component._definition.parameters[input_name]
        entity_dict = param_def.to_dict(remove_name=True) if with_def else {}
        from inspect import Parameter
        if with_def and param_def.default is Parameter.empty:
...
def _serialize_output(output: Output, with_data=False, with_def=False):
    ...
    if with_data:
        ...
        if parent is not None and output in parent.outputs.values():
    ...
    entity_dict = output_def.to_dict(remove_name=True) if with_def else {}
    ...
    if output.datastore:
        entity_dict['datastore'] = output.datastore.name
    if output._path_on_compute:
        entity_dict['path_on_compute'] = output._path_on_compute
    if output._output_mode:
        entity_dict['mode'] = output._output_mode
...
class NodeBase(ABC):
    ...
    def serialize_node_inputs_outputs(self, provider):
        ...
        for _k, _v in instance._parameter_params.items():
            ...
            if _k in instance._definition.parameters and _v == instance._definition.parameters[_k].default:
                continue
...
class PipelineNode(NodeBase):
    ...
    def export_definition(self, provider):
        ...
        inputs_dict = {
            ...
            for k, v in self.instance.inputs.items()}
        inputs_dict.update(
            {k: _serialize_input(k, self.instance, provider, with_data=self.is_primary, with_def=True)
        ...
        if self.instance.default_datastore:
            defaults_config.update({'datastore': REMOTE_REF_FORMAT.format(self.instance.default_datastore.name)})
