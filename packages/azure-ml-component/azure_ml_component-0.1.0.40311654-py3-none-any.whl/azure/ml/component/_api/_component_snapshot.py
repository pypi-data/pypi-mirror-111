...
AZUREML_HINT_IN_YAML = 'azureml:'
yaml = YAML(typ='unsafe', with_additional_representer=True)
...
class ComponentSnapshot:
    ...
    def trial_file_relative_path(self):
        ...
        if trial is None:
            return None
        return trial[len(FILE_HINT_IN_YAML):] \
            if trial and trial.startswith(FILE_HINT_IN_YAML) else None
    ...
    def trial_component(self):
        ...
        if trial is None:
            return None
        return trial[len(AZUREML_HINT_IN_YAML):] \
            if trial and trial.startswith(AZUREML_HINT_IN_YAML) else None
    ...
    def _get_sweep_yaml_file(self):
        ...
        if self.sweep._spec_file_path.as_posix().startswith(self.base_path.as_posix()):
            ...
            with open(tmp_sweep_yaml_path, 'w') as fout:
                yaml.dump(updated_spec_dict, fout)
