            ...
            if other_param.human_readable_name in self.not_required_if:
                ...
                if other_opt:
                    if current_opt:
                        ...
                        Logger.info(CLI_MESSAGES['TryHelpWithCommand'].format(ctx.command_path), stress=False)
                        ctx.abort()
                    else:
                        self.required = None
...
def modify_click_usage_error():
    ...
    def show(self, file=None):
        ...
        if self.ctx:
            ...
            Logger.info(CLI_MESSAGES['TryHelpWithCommand'].format(self.ctx.command_path), stress=False, force=True)
        else:
