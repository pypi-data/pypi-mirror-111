...
###--------------------------------------------------------------------------------------------
    ...
    def get(self, project, application, key, revision=None):
        ...
        if not found:
            raise DSOException(f"Template '{key}' not found: project={project}, application={application}")
        else:
        ...
        if revision is None:
            ...
            if not templates:
                raise DSOException(f"Revision '{revision}' not found: project={project}, application={application}, key={key}")
            result = {
                    'RevisionId': str(templates[0]['Version']),
                    'Key': key, 
                    'Contents': self.unescape_curly_brackets(templates[0]['Value']),
...
###--------------------------------------------------------------------------------------------
    ...
    def delete(self, project, application, key):
        ...
        return {'Key': key, 'Path': found[0]['Name'][:-len(key)-1]}
