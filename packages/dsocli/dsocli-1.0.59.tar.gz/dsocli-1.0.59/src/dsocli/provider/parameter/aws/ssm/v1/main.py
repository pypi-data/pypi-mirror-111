...
<<<<<<< HEAD
    def get(self, project, application, stage, key, revision):
=======
...
>>>>>>> develop
        ...
        if not found:
            ...
            if not found[0]['Type'] in ['String']:
                raise DSOException(f"Parameter '{key}' not found: project={project}, application={application}, stage={Stages.shorten(stage)}")
<<<<<<< HEAD
        Logger.info(f"Getting SSM parameter value: path={found[0]['Name']}")
        if revision:
            history = ssm.get_parameter_history(Name=found[0]['Name'])
            foundRevision = [x for x in history['Parameters'] if str(x['Version']) == str(revision)]
            if not len(foundRevision):
            ...
            result = foundRevision[0]['Value']
        else:
            result = ssm.get_parameter(Name=found[0]['Name'])['Parameter']['Value']
    ...
    def history(self, project, application, stage, key):
        ...
        if not found or len(found) == 0:
            ...
            if not found[0]['Type'] in ['String', 'StringList']:
                raise DSOException(f"Parameter '{key}' not found: project={project}, application={application}, stage={Stages.shorten(stage)}")
        Logger.info(f"Getting SSM parameter history: path={found[0]['Name']}")
        history = ssm.get_parameter_history(Name=found[0]['Name'])
        result = { "Revisions":
                    [{  "Revision": x['Version'],
                        "Date": x['LastModifiedDate'].strftime('%Y/%m/%d-%H:%M:%S'),
                        "Value": x['Value'],
                        "User": x['LastModifiedUser']
                    } for x in history['Parameters']]
                }
=======
