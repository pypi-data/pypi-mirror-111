...
config = {
    'parameters_dir' : '{0}/parameters'.format(Config.dir_name),
    'file_extension' : {
        'yaml' : 'yml',
        'json' : 'json',
    }
...
###--------------------------------------------------------------------------------------------
    ...
    def __get_param_file_extension(self):
        format = Config.parameter_spec('format')
        if format is None:
...
###--------------------------------------------------------------------------------------------
    ...
    def __get_params_root_path(self, context=None):
        if not context:
            return f"{Config.working_dir}/{config['parameters_dir']}"
        stage, env = Stages.parse_context(context)
        # if env == 'default':
        #     return f"{params_root_path}/{stage}"
        # else:
        #     return f"{params_root_path}/{stage}/{env}"
        return f"{Config.working_dir}/{config['parameters_dir']}/{stage}/{env}"
