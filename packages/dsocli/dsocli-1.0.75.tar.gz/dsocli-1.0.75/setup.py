...
current_release = int(current_version.split('.')[2])
...
new_version = f"{new_major}.{new_minor}.{new_release}"
...
class PublishCommand(Command):
    ...
    def run(self):
        ...
        os.system('{0} -m twine upload --skip-existing --repository {1} dist/*'.format(sys.executable, pypi_index))

        # self.status('Installing the new version...')
        # os.system('pip iuninstall dsocli -y')
        # os.system('pip install dsocli')

        # self.status('Pushing git tags…')
        # os.system('git tag v{0}'.format(new_version))
