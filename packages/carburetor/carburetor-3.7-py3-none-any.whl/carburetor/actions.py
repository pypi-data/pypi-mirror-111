...
def on_connect(action, param, app):
    ...
    main_image = ui.get('MainImage')
    main_image.hide()
    app.spinner = ui.get('Spinner')
    app.spinner.start()
    app.spinner.show()
    stop_button = ui.get('StopButton')
    stop_button.set_visible(True)
    main_label = ui.get('MainLabel')
    if functions.is_running():
        text_stopping = config._("Disconnecting…")
        main_label.set_label(text_stopping)
        action = 'stop'
    else:
        text_starting = config._("Connecting…")
        main_label.set_label(text_starting)
        action = 'start'
    app.task = Popen(
