...
class CloudPodManager(GQDoq):
 BACKEND='_none_'
 def __init__(self,pod_name=GQDob,config=GQDob):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise GQDoV('Not implemented')
 def pull(self):
  raise GQDoV('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert GQDoP(statuses)==GQDoP(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise GQDoV('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @GQDok
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise GQDoV('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==GQDoP([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=GQDod)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise GQDoV('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=GQDod)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=GQDof(self,'clone_dir',GQDob)
...
class PodConfigManagerMeta(GQDoF):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=GQDob
   for manager in cls.CHAIN:
    try:
     tmp=GQDof(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif GQDoJ(tmp,GQDon)and GQDoJ(result,GQDon):
       result.extend(tmp)
    except GQDoV:
     pass
   if result is not GQDob:
    return result
   raise GQDoV('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(GQDoq,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @GQDok
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise GQDoV('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(GQDoq):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=GQDod).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=GQDoP([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=GQDoH):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except GQDoV:
   if safe:
    return{}
   raise GQDoV('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(GQDoq):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise GQDoV('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise GQDoV('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise GQDoV('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
