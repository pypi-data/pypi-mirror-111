...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Waliy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',Waliv(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Walib,from_name=Walib,smtp_host=Walib,smtp_user=Walib,smtp_pass=Walib,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Walih(recipients,Walim)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Walib):
 ...
 result['From']=formataddr((Waliv(Header(from_name,'utf-8')),from_email))
 return result
