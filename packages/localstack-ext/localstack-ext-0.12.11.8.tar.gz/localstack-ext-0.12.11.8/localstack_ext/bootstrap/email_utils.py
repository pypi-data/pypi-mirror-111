...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OByuN()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',OByuT(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OByuD,from_name=OByuD,smtp_host=OByuD,smtp_user=OByuD,smtp_pass=OByuD,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OByuP(recipients,OByus)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OByuD):
 ...
 result['From']=formataddr((OByuT(Header(from_name,'utf-8')),from_email))
 return result
