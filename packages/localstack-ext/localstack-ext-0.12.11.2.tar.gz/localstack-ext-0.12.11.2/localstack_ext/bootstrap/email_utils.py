...
SENT_EMAILS=[]
EMAIL_BLACKLIST=LofNl()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',LofNT(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=LofNi,from_name=LofNi,smtp_host=LofNi,smtp_user=LofNi,smtp_pass=LofNi,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if LofNV(recipients,LofNh)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=LofNi):
 ...
 result['From']=formataddr((LofNT(Header(from_name,'utf-8')),from_email))
 return result
