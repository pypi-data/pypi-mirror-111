...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OLrJF()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',OLrJP(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OLrJl,from_name=OLrJl,smtp_host=OLrJl,smtp_user=OLrJl,smtp_pass=OLrJl,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OLrJI(recipients,OLrJj)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OLrJl):
 ...
 result['From']=formataddr((OLrJP(Header(from_name,'utf-8')),from_email))
 return result
