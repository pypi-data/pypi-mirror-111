...
SENT_EMAILS=[]
EMAIL_BLACKLIST=KUTwn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',KUTwh(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=KUTwI,from_name=KUTwI,smtp_host=KUTwI,smtp_user=KUTwI,smtp_pass=KUTwI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if KUTwz(recipients,KUTwS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=KUTwI):
 ...
 result['From']=formataddr((KUTwh(Header(from_name,'utf-8')),from_email))
 return result
