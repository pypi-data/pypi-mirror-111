...
from localstack.constants import (
    LOCALSTACK_WEB_PROCESS, LOCALSTACK_INFRA_PROCESS, TRUE_STRINGS)
...
# list of services which need to export custom ports at runtime
EXTERNAL_PORT_APIS = ('apigateway', 'apigatewayv2', 'athena', 'cloudfront', 'codecommit', 'ecs', 'ecr',
    'elasticache', 'mediastore', 'rds', 'transfer', 'kafka', 'neptune', 'azure')
...
def register_localstack_plugins():
    ...
    if is_infra_process:
        ...
        if is_api_key_configured:
            # this has been causing issues for non-Pro users - run only if the API key is configured
    ...
    with licensing.prepare_environment():
        ...
        if is_infra_process:
            # load plugins only if we're inside Docker context, to increase startup speed
            load_plugin_files()

        try:
            # start local daemon for EC2
        ...
        if is_api_key_configured:
            if os.environ.get('EDGE_PORT') and not localstack_config.EDGE_PORT_HTTP:
                LOG.warning(('!! Configuring EDGE_PORT={p} without setting EDGE_PORT_HTTP may lead ' +
                    'to issues; better leave the defaults, or set EDGE_PORT=443 and EDGE_PORT_HTTP={p}').format(
                        p=localstack_config.EDGE_PORT))
            else:
                port = localstack_config.EDGE_PORT
                localstack_config.EDGE_PORT = 443
                localstack_config.EDGE_PORT_HTTP = port

    # define API composites/dependencies
    ...
    # add Docker flags for DNS service
    docker_flags = []
    ...
    # add Docker flags for service instances (RDS, ElastiCache, ...)
    if any([is_api_enabled(api) for api in EXTERNAL_PORT_APIS]):
        docker_flags += ['-p {start}-{end}:{start}-{end}'.format(
            start=config_ext.SERVICE_INSTANCES_PORTS_START, end=config_ext.SERVICE_INSTANCES_PORTS_END)]

    # add Docker flags for kube config
    if is_api_enabled('eks'):
        kube_config = os.path.expanduser('~/.kube/config')
        if os.path.exists(kube_config):
            docker_flags += ['-v %s:/root/.kube/config' % kube_config]

    # add Docker flags for Azure AMQP
    if is_api_enabled('azure'):
        docker_flags += ['-p {port}:{port}'.format(port=5671)]

    # add Docker flags for Azure APIs
...
def load_plugin_files():
    # Note: make sure all imports and patches are wrapped in try-catch below!
    try:
        ...
        from localstack_ext.services.applicationautoscaling import (
            applicationautoscaling_starter, applicationautoscaling_listener)
        ...
        register_plugin(Plugin('application-autoscaling',
            start=applicationautoscaling_starter.start_applicationautoscaling,
            listener=applicationautoscaling_listener.UPDATE_APPLICATION_AUTOSCALING))
        ...
        register_plugin(Plugin('codecommit', start=codecommit_starter.start_codecommit,
                               listener=codecommit_listener.UPDATE_CODECOMMIT))
        register_plugin(Plugin('cognito-identity', start=cognito_starter.start_cognito_identity,
            listener=cognito_listener.UPDATE_COGNITO_IDENTITY))
        register_plugin(Plugin('cognito-idp', start=cognito_starter.start_cognito_idp,
            listener=cognito_listener.UPDATE_COGNITO))
        ...
        register_plugin(Plugin('glacier', start=glacier_starter.start_glacier,
            listener=glacier_listener.UPDATE_GLACIER))
        ...
        register_plugin(Plugin('redshift', start=redshift_starter.start_redshift,
            listener=redshift_listener.UPDATE_REDSHIFT, priority=10))
        ...
        # patch dashboard features
        dashboard_extended.patch_dashboard()

        # path edge and AWS utils
...
def patch_start_infra():
    # leave import here to fix import errors on "make init"
    ...
    def do_start_infra(asynchronous, apis, is_in_docker, *args, **kwargs):
        ...
        try:
            # temporarily disable IAM checks during startup
...
def _setup_logging():
    ...
    # logger config for postgresql_proxy below
...
def api_key_configured():
    return bool(os.environ.get('LOCALSTACK_API_KEY'))
