...
from localstack.utils.common import (
    mkdir, run, new_tmp_file, rm_rf, chmod_r, download, unzip, get_arch,
    in_docker, is_alpine, now, is_command_available)
...
def install_postgres():
    ...
    # TODO: consider removing postgresql-dev, which currently takes up 100MB+ of space
    ...
    links = {
        plpython_lib: '/usr/lib/postgresql/plpython3.so',
        '%s/plpython3u.control' % ext_target: '%s/plpython3u.control' % ext_source,
        '%s/plpython3u--1.0.sql' % ext_target: '%s/plpython3u--1.0.sql' % ext_source,
        '%s/plpython3u--unpackaged--1.0.sql' % ext_target: '%s/plpython3u--unpackaged--1.0.sql' % ext_source,
    }
...
def install_timescaledb():
    ...
    tag = '2.0.0-rc4'  # TODO update tag; currently required to fix build that is failing on master branch
...
def setup_ssl_cert():
    # make sure to keep this import here, to avoid import errors during plugin loading
    from localstack.services import generic_proxy
    target_file = generic_proxy.get_cert_pem_file_path()
    # cache file for 6 hours
...
def download_github_artifact(url, target_file):
    ...
    def do_download(url, print_error=False):
        try:
            ...
            if print_error:
                LOG.info('Unable to download local test SSL certificate from %s to %s: %s %s' %
