...
from localstack.utils.common import (
    safe_requests as requests, load_file, save_file, to_str, to_bytes, md5,
    parallelize, now_utc, str_insert, str_remove)
...
# allow auth tokens to be cached up to 24 hours
...
def read_api_key(raise_if_missing=True):
    ...
    if not key and raise_if_missing:
        raise Exception('Unable to retrieve API key. Please configure $%s in your environment' %
...
def fetch_key():
    ...
    try:
        ...
        result = requests.post('%s/activate' % config.API_URL, json.dumps(data), verify=False)
        if result.status_code >= 400:
            content = result.content
            content_type = result.headers.get('Content-Type')
            raise Exception('Received error activating key (code %s): ctype "%s" - %s' % (
                result.status_code, content_type, content))
        ...
        if log_license_issues():
            ...
            LOG.warning('Error activating API key "%s..."(%s): %s %s' % (
                api_key_prefix, len(api_key), e, traceback.format_exc()))
        # attempt to load key from cache
...
def load_cached_key(api_key):
    ...
    if cached_key.get('key_hash') != md5(api_key):
        raise Exception('Cached key was created for a different API key')
    now = now_utc()
    if (now - cached_key['timestamp']) > MAX_KEY_CACHE_DURATION_SECS:
        raise Exception('Cached key expired')
    timestamp = str(cached_key['timestamp'])
