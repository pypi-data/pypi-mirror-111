...
"""
Daemon process required to receive requests from the LocalStack Docker
container, and run commands on the local machine (e.g., starting a
local virtual machine for EC2).
"""
...
# name of the S3 bucket that indicates a local Lambda mount
...
class RequestHandlerRoot(RequestHandler):
    """ Request handler for operations that require root privileges. """
    def handle_request(self):
        request = self.request_json
        result = '{}'
        operation = request.get('op')
...
def start_ssh_forward_proxy(options):
    ...
    host = LOCAL_BIND_ADDRESS_PATTERN.replace('*', str(len(USED_BIND_ADDRESSES) + 2))
...
def create_network_interface_alias(address, interface=None):
    ...
    if is_mac_os():
        ...
        run_cmd('{sudo_cmd} ifconfig {iface} alias {addr}'.
            format(sudo_cmd=sudo_cmd, addr=address, iface=interface))
        return
    if is_linux():
        # try for Linux
        ...
        for interface in interfaces:
            try:
                ...
                run_cmd('{sudo_cmd} ifconfig {iface}:0 {addr} netmask 255.255.255.0 up'.
                    format(sudo_cmd=sudo_cmd, addr=address, iface=interface))
                return
            except Exception as e:
...
def get_os():
    ...
    if is_linux():
        return 'linux'
    # TODO: fall back to windows for now
    return 'windows'
...
def get_ip_address(ifname):
    ...
    return socket.inet_ntoa(fcntl.ioctl(
        s.fileno(),
        0x8915,  # SIOCGIFADDR
        struct.pack('256s', to_bytes(ifname[:15]))
    )[20:24])
...
def start_in_background():
    # Note: leave these imports here to avoid import errors when starting the process
    ...
    # start standard daemon process
    python_cmd = sys.executable
    cmd = '%s %s' % (python_cmd, __file__)
    run(cmd, outfile=log_file, asynchronous=True)

    # start root daemon process
    LOG.info('Attempting to obtain sudo privileges for local daemon of EC2 API ' +
        '(required to start SSH forward proxy on privileged port 22). ' +
        'You may be asked for your sudo password.')
    run('sudo ls', stdin=True)

    def start_root_daemon(*args):
        cmd = 'sudo %s %s root >> %s' % (python_cmd, __file__, log_file)
        run(cmd, outfile=log_file, stdin=True)
