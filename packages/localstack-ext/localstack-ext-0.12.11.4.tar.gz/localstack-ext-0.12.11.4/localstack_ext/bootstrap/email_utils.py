...
# keep a reference to sent emails, for testing purposes
SENT_EMAILS = []

# blacklist of email addresses to be ignored (populated from other places)
EMAIL_BLACKLIST = set()
...
def send_email(subject, message, recipients, from_email=None, from_name=None,
        smtp_host=None, smtp_user=None, smtp_pass=None, images={}):
    ...
    if not smtp_host:
        if is_local_test_mode():
            entry = {
                'time': now_utc(), 'smtp_host': smtp_host, 'smtp_user': smtp_user, 'smtp_pass': smtp_pass,
                'from_email': from_email, 'from_name': from_name,
                'subject': subject, 'message': message, 'recipients': recipients
            }
...
def sign_message(msg):
    # TODO!
    # sig = dkim.sign(msg.as_string(), 'myselector', from_domain, self.dkim_private,
...
def construct_message(subject, message, from_name, from_email, images=None):
    ...
    if images:
        ...
        # set plain email text - TODO send proper plain-text message, with HTML tags removed...
