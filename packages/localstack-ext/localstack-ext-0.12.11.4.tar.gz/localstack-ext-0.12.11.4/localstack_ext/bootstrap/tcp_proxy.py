...
from socket import (
    AF_INET, AF_INET6, SOCK_STREAM, IPV6_V6ONLY, SO_REUSEADDR,
    IPPROTO_TCP, IPPROTO_IPV6, TCP_NODELAY, SOL_SOCKET)
...
def tcp_connect(six, addr, port, blk):
    """Return a connected client socket (blocking on connect...)"""
    s = socket.socket(AF_INET6 if six else AF_INET, SOCK_STREAM)
    s.setsockopt(IPPROTO_TCP, TCP_NODELAY, 1)
    s.connect((addr, port))
    s.setblocking(blk)
...
class Proxy(object):
    """A client connection and the peer connection it proxies to"""
...
def server_loop(opt):
    ...
    while qs:
        ...
        # meaning it wasn't fully drained last time
        rr, r, w, e = [], [], [], []
        for q in qs:
            q.pre_wait(rr, r, w, e)
