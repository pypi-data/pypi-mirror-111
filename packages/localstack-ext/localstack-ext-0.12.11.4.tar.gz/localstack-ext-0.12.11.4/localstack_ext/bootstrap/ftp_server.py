...
from localstack.utils.common import (
    new_tmp_dir, FuncThread, save_file, load_file, wait_for_port_open, TMP_THREADS, ShellCommandThread
)
...
# exposed ports that can be used for uploading data from the client (TODO do we need more ports here?)
FTP_PASSIVE_PORTS = [ext_config.SERVICE_INSTANCES_PORTS_END - 2,
    ext_config.SERVICE_INSTANCES_PORTS_END - 1, ext_config.SERVICE_INSTANCES_PORTS_END]

# whether to start FTP servers in subprocesses
# (defaults to True, because we've seen SEGV faults if running in the same process)
USE_SUBPROCESS = True

# state of directory mapppings
DIRECTORY_MAPPING = {}
...
def apply_patches():
    ...
    extended_proto_cmds.update(
        {
            'SITE ADDUSER': dict(
                perm='m', auth=True, arg=True, help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.'
            )
        }
    )
...
def add_ftp_user(user, server_port):
    ...
    ftp.sendcmd('SITE ADDUSER  {} {} {} {}'.format(
        user.username, FTP_USER_DEFAULT_PASSWD, user_dir, FTP_USER_PERMISSIONS)
    )
    ftp.quit()
...
def start_ftp(port):
    if USE_SUBPROCESS:
        ...
        thread = ShellCommandThread(cmd, outfile=subprocess.PIPE, env_vars=env_vars, quiet=False)
        thread.start()
        time.sleep(2)
    else:
        thread = do_start_ftp(port, asynchronous=True)
    time.sleep(2)
...
def do_start_ftp(port, asynchronous=True):
    ...
    # configure handler and TLS settings
    ...
    # Note: This needs to be an IP address, cannot be a hostname :/ for FTP PASV 227 response
...
def main():
    do_start_ftp(int(sys.argv[1]), asynchronous=False)
