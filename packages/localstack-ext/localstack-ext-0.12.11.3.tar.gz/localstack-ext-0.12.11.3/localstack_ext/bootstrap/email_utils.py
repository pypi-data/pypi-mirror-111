...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qtyIM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',qtyIX(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qtyIQ,from_name=qtyIQ,smtp_host=qtyIQ,smtp_user=qtyIQ,smtp_pass=qtyIQ,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qtyIg(recipients,qtyIK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qtyIQ):
 ...
 result['From']=formataddr((qtyIX(Header(from_name,'utf-8')),from_email))
 return result
