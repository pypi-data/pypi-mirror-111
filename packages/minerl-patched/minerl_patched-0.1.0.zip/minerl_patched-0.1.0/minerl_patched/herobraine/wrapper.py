...
class EnvWrapper(EnvSpec):
    ...
    def wrap_observation(self, obs: OrderedDict):
        ...
        if minerl_patched.utils.test.SHOULD_ASSERT: assert obs in self.env_to_wrap.observation_space

        wrapped_obs = self._wrap_observation(obs)

        if minerl_patched.utils.test.SHOULD_ASSERT: assert wrapped_obs in self.observation_space
        return wrapped_obs
    ...
    def wrap_action(self, act: OrderedDict):
        ...
        if minerl_patched.utils.test.SHOULD_ASSERT: assert act in self.env_to_wrap.action_space

        wrapped_act = self._wrap_action(act)

        if minerl_patched.utils.test.SHOULD_ASSERT: assert wrapped_act in self.action_space
        return wrapped_act
    ...
    def unwrap_observation(self, obs: OrderedDict) -> OrderedDict:
        obs = copy.deepcopy(obs)
        if minerl_patched.utils.test.SHOULD_ASSERT: assert obs in self.observation_space
    ...
    def unwrap_action(self, act: OrderedDict) -> OrderedDict:
        act = copy.deepcopy(act)
        if minerl_patched.utils.test.SHOULD_ASSERT: assert act in self.action_space
