...
from minerl_patched.data.util import download_with_resume

import logging

from minerl_patched.data.version import VERSION_FILE_NAME, DATA_VERSION, assert_version
import tempfile
import sys
import coloredlogs
...
def download(directory=None, resolution='low', texture_pack=0, update_environment_variables=True, disable_cache=False,
    ...
    try:
        ...
        if experiment is not None:
            logger.error("Is \"{}\" a valid minerl_patched environment?".format(experiment))
        return None
