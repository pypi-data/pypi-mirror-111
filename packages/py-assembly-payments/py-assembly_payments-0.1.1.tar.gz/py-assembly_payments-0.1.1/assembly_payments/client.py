...
PROD_VIRTUAL_ACCOUNTS_URL = "https://au-0000.api.assemblypay.com"
TEST_BASE_URL = "https://test.api.promisepay.com"
TEST_AUTH_URL = "https://au-0000.sandbox.auth.assemblypay.com"
TEST_VIRTUAL_ACCOUNTS_URL = "https://sandbox.au-0000.api.assemblypay.com"

class AssemblyClient:
    def __init__(self, client_id=None, client_secret=None, scope=None, grant_type=None, use_production=False, logging=False):
        self.client_id = client_id or os.environ.get("ASSEMBLY_CLIENT_ID", None)
        self.client_secret = client_secret or os.environ.get("ASSEMBLY_CLIENT_SECRET", None)
        ...
        self.virtual_accounts_url = TEST_VIRTUAL_ACCOUNTS_URL
        if use_production:
            self.base_url = PROD_BASE_URL
            self.auth_url = PROD_AUTH_URL
            self.virtual_accounts_url = PROD_VIRTUAL_ACCOUNTS_URL

        self._initialise_services()

    def get_auth(self):
        def refresh_token():
            return TokenService(base_url=self.base_url, auth_url=self.auth_url, logging=self.logging).create(
    ...
    def _initialise_services(self):
        kwargs = dict(base_url=self.base_url, auth_url=self.auth_url, virtual_accounts_url=self.virtual_accounts_url , get_auth=self.get_auth, logging=self.logging)
