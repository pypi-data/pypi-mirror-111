...
class DistanceController(Controller):
    ...
    def _get_threshold_mean(self):
        return np.mean(self._get_distances())

    def _get_threshold_median(self):
        return np.median(self._get_distances())

    def _get_threshold_mab(self) -> float:
        return self.__mab.predict(self._compute_controller_context()) / 100
