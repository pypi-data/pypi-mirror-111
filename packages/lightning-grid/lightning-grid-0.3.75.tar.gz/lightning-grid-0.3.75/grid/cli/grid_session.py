...
from grid.cli.grid_run import _check_run_name_is_valid, _get_instance_types, _resolve_instance_type_nickname
...
@click.option(
    ...
    type=bool,
    is_flag=True,
    help='Fetch session from everyone in the team when flag is passed'
)
...
def create(
    ...
    cluster: Optional[str], instance_type: str, datastore_name: Optional[str], datastore_version: Optional[str],
    datastore_mount_dir: Optional[str], disk_size: Optional[int], use_spot: Optional[bool]
) -> None:
    """Creates a session."""
    client = Grid()
...
def mount(interactive_node, mount_dir):
    r"""
    Mount session directory to local.
