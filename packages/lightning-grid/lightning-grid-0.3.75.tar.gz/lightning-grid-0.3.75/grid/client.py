...
from grid.core.repository import LocalRepository
...
"""
    ...
    async def grpc_channel():
        ...
        # python doesn't support insecure credentials without a lot of headache!!!
        if env.GRID_API_CA_FILE:
            with open(env.GRID_API_CA_FILE, 'rb') as f:
                channel_creds = grpc.ssl_channel_credentials(f.read())
        else:
            channel_creds = grpc.ssl_channel_credentials()

        if env.GRID_API_URL:
            # This is non-default, thus echo it
            print(f"GRID_API_URL: {env.GRID_URL}")
            url = parse.urlparse(env.GRID_API_URL)
        else:
            # Print non-default
        ...
        async with grpc.aio.secure_channel(
            f"{url.hostname}:{url.port or 443}", grpc.composite_channel_credentials(
                channel_creds,
                call_credentials,
            )
        ) as channel:
    ...
    @staticmethod
    def _add_git_root_path(entrypoint: Union[str, Path]) -> str:
    ...
    def get_experiment_id(experiment_name: str) -> str:
        ...
        try:
            ...
            from grid.core import Experiment as ExperimentGridObject
            spinner.start()
