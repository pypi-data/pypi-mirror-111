...
from grid.cli.utilities import read_config_callback, validate_config, validate_disk_size_callback
...
def get_credentials(client: Grid, grid_credential: Optional[str] = None) -> Dict[str, str]:
    ...
    if grid_credential:
        ...
        raise click.ClickException(
            f'Credential {grid_credential} does not exist. ' + 'Use grid credentials to see available credential IDs.'
        )
...
def _parse_script_args(args) -> Optional:
...
@click.pass_context
def run(
    ctx, script: str, entrypoint: Path, config: Optional[Dict], name: Optional[str], cluster: Optional[str],
    credential: Optional[str], strategy: Optional[str], trials: Optional[int], instance_type: str, gpus: Optional[int],
    disk_size: Optional[int], description: Optional[str], ignore_warnings: bool, script_args_json: Optional[str],
    ...
    use_spot: Optional[bool], localdir: Optional[bool], dockerfile: Optional[str]
) -> None:
    ...
    if not config:
        ...
        if not _grid_config.get('compute') or _grid_config['compute']['train']:
