...
from certora_cli.certoraUtils import OPTION_OUTPUT_VERIFY, fatal_error, is_windows, remove_and_recreate_dir, getcwd, abs_posix_path
from certora_cli.certoraUtils import debug_print_, read_json_file, replace_file_name
...
from certora_cli.certoraUtils import split_by_commas_ignore_parenthesis, get_closest_strings, flatten_set_list
...
class CertoraBuildGenerator:
        ...
        def is_payable_parameter_type(base_type_node: Dict[str, Any]) -> bool:
            if "stateMutability" in base_type_node:
                assert "name" in base_type_node and base_type_node["name"] == "address" or \
                       "nodeType" in base_type_node and base_type_node["nodeType"] == "FunctionTypeName", \
    ...
    def get_standard_json_data(self, sdc_name: str) -> Dict[str, Any]:
        json_file = f"{self.config_path}/{sdc_name}.standard.json.stdout"
        ...
        while contract_work_list:
            ...
            for contract_name in contract_list:
                ...
                for ref in internal_refs:
                    ...
                    assert len(set(contract_files)) == 1
    ...
    def get_contract_in_sdc(self,
        ...
        debug_print(f"Internal Functions of {contract_name}: {internal_funcs}")
    ...
    def generate_function_finder(self, calleeName: Optional[str], f: Func) -> Optional[Tuple[str, str, str]]:
        """
        Generates solidity source which will be inserted into the contract. Its sole purpose is to allow the
        tool to find the start PC of the internal function f
        :param calleeName: a qualifier to use when writing the call to f (if needed)
        :param f: the internal function whose "finder" we are generating
        :return: (function_def, finder_id_exp, internal_id_exp) where:
                function_def is a solidity definition of a new external function whose sole purpose is to help the
                                tool find f
                finder_id_exp is the functionIdExp of the new external function
                internal_id_exp is the functionIdExp of f
        """
        if f.isConstructor:
            return None

        for arg in f.fullArgs + f.returns:
            debug_print(f"Checking arg type in internal function: {arg}")
            if len(arg.array_dims) > 0 or len(arg.components) > 0 or arg.base_type == "bytes" or \
                    arg.base_type == "string" or arg.is_function or arg.is_mapping or arg.is_enum or arg.is_contract:
                debug_print(f"Not generating a function finder for {f.signature()}.")
                return None
        # special magic name
        name = f"certoraFunctionFinder{self.function_finder_generator_idx}"
        self.function_finder_generator_idx += 1
        # generated param names
        param_names = [f"p{i}" for i in range(0, len(f.fullArgs))]
        param_types_for_finder = [p.base_type for p in f.fullArgs]
        param_type_for_fun_id_exp = [p.signature() for p in f.fullArgs]
        # params with their types
        params = map(lambda t, p: f"{t} {p}", param_types_for_finder, param_names)
        return_types = ",".join([r.base_type for r in f.returns])
        param_types_for_fun_id_exp_joined = ','.join(param_type_for_fun_id_exp)

        # id exps for use in the "function_finder" mapping
        finder_id_exp = f"{name}({param_types_for_fun_id_exp_joined})"
        internal_id_exp = f"{f.name}({param_types_for_fun_id_exp_joined})"
        returns_something = len(return_types) > 0
        if returns_something:
            return_type = f"returns ({return_types})"
            return_string = "return"
        else:
            return_type = ""
            return_string = ""
        if calleeName is None:
            internal_func_call_id = f.name
        else:
            internal_func_call_id = f"{calleeName}.{f.name}"

        # the generated code:
        function_def = f"\tfunction {name}({', '.join(params)}) external {return_type} {'{'}" + \
                       f"\n\t\trequire(block.gaslimit == 0xbeef1b01);\n\t\t{return_string} " \
                       f"{internal_func_call_id}({', '.join(param_names)});\n\t{'}'}\n"
        return function_def, finder_id_exp, internal_id_exp

    def generate_contract_function_finder(self, f: Func) -> Optional[Tuple[str, str, str]]:
        return self.generate_function_finder(None, f)

    def generate_library_function_finder(self, lib_contract_name: str, f: Func) -> Optional[Tuple[str, str, str]]:
        return self.generate_function_finder(lib_contract_name, f)

    @staticmethod
    def generate_library_import(file_absolute_path: str, library_name: str) -> str:
        return f"\nimport {'{'}{library_name}{'}'} from '{file_absolute_path}';"

    def add_auto_finders(self, build_arg_contract_file: str, out: str, contracts: List[ContractInSDC],
                         original_file: str, primary_contract_file: str, primary_contract: str,
                         do_not_generate: List[str]) -> Dict[str, Dict[str, str]]:
        function_finder_by_contract: Dict[str, Dict[str, str]] = dict()
        library_functions_to_add = []
        # [(file, library)]
        library_imports_to_add = []  # type: List[Tuple[str, str]]
        for c in contracts:
            def_node = self.get_contract_def_node_ref(build_arg_contract_file, c.original_file, c.name)
            is_lib = self.is_library_def_node(c.original_file, def_node, build_arg_contract_file)
            if is_lib:
                nodes = self.asts[build_arg_contract_file][primary_contract_file]
                # look for any reference to c.name
                all_refs = [node for node in nodes
                            if "name" in nodes[node].keys() and nodes[node]["name"] == c.name and
                            "referencedDeclaration" in nodes[node].keys()]
                # collect all that are references to another contract that also happens to have the same name, this
                # would count as a collision (probably)
                colliding_refs = [nodes[node]["name"] for node in all_refs if
                                  nodes[node]["referencedDeclaration"] != def_node and
                                  (lambda declaration:
                                   "nodeType" in declaration.keys() and
                                   declaration["nodeType"] == "ContractDefinition")
                                  (self.get_node_from_asts(self.asts, build_arg_contract_file,
                                                           nodes[node]["referencedDeclaration"]))]
                # collect any instance where we reference the same library, if so it must already be imported
                already_referenced = [nodes[node]["name"] for node in all_refs if
                                      nodes[node]["referencedDeclaration"] == def_node]
                # if there's a collision, do not import, do not generate finder: we just won't have a finder :/
                if len(colliding_refs) == 0:
                    # private library functions are not visible to contracts
                    library_functions_to_add \
                        .extend([self.generate_library_function_finder(c.name, f) for f in c.internal_funcs
                                 if f.visibility != "private"])
                    # only import if there is not already an import (multiple imports of the same library are
                    # actually okay but I felt like not doing that)
                    if len(already_referenced) == 0:
                        library_imports_to_add.append((c.original_file, c.name))

        with open(build_arg_contract_file) as c_input:
            with open(out, "w+") as output:
                total_read = 0
                # this filter seems fragile but I'm not sure what else to do
                # TODO: do we only want to instrument the primary contract of a file? How do we make sure we are
                #       only instrumenting contracts whose info gets passed to .certora_build.json?
                contracts_in_file_sorted = [c for c in contracts if
                                            get_file_basename(c.original_file) == get_file_basename(original_file)]
                # sort by start position of contract
                contracts_in_file_sorted.sort(key=lambda contract: contract.source_bytes[0])
                for c in contracts_in_file_sorted:
                    function_finder_for_c: Dict[str, str] = dict()
                    (start, length) = c.source_bytes
                    to_read = start - total_read + length
                    debug_print(f"start: {start}, length: {length}, total read: {total_read}, to read: "
                                f"{to_read} at contract {c.name}")
                    total_read += to_read
                    to_write = c_input.buffer.read(to_read - 1)
                    output.buffer.write(to_write)
                    if c.name == primary_contract:
                        contract_functions_to_add = [self.generate_contract_function_finder(f)
                                                     for f in c.internal_funcs]
                        for generate_result in contract_functions_to_add + library_functions_to_add:
                            if generate_result is not None:
                                finder_def, finder_id_exp, internal_id_exp = generate_result
                                if internal_id_exp not in do_not_generate:
                                    function_finder_for_c[finder_id_exp] = internal_id_exp
                                    output.buffer.write(bytes(finder_def, "utf-8"))
                    function_finder_by_contract[c.name] = function_finder_for_c
                    to_write = c_input.buffer.read(1)
                    output.buffer.write(to_write)
                to_write = c_input.buffer.read(-1)
                output.buffer.write(to_write)
                # instead of figuring out where to write imports at the beginning (especially if there is a pragma)
                # it turns out we can write imports at any top-level point in the file!
                for filename, library_name in library_imports_to_add:
                    output.buffer.write(bytes(self.generate_library_import(filename, library_name), "utf-8"))
        return function_finder_by_contract
    ...
    def __find_closest_methods(method_name: str, methods: List[Func]) -> List[str]:
        ...
        for name in possible_method_names:
            for method in methods:
                if method.name == name:
                    curr_method_arg_types = [arg.base_type for arg in method.fullArgs]
                    possible_methods.append(f"{name}({','.join(curr_method_arg_types)})")
    ...
    def __verify_method(self, method_input: str, sdc_pre_finder: SDC) -> None:
        method_name = method_input.split('(')[0]
        method_params = split_by_commas_ignore_parenthesis(method_input.split('(')[1].replace(")", ''))
        ...
        for method in public_methods:
            if method.name == method_name:  # Correct name, now we check parameter types
                curr_method_arg_types = [arg.base_type for arg in method.fullArgs]
                if len(curr_method_arg_types) == len(method_params):
                    matching_args = True
                    for i, arg in enumerate(method_params):
                        if curr_method_arg_types[i] != method_params[i]:
                            matching_args = False
                            break
                    if matching_args:  # An exact match was found
                        return

                # A method with the same name but different parameters exists
                possible_methods.append(f"{method_name}({','.join(curr_method_arg_types)})")
                ...
                if len(curr_method_arg_types) == len(method_params):
                    ...
                    if matching_args:  # An exact match was found
                        raise AttributeError(
                            f"Method {method_input} is {method.visibility}. Please change it to external or public")
        ...
        if len(possible_methods) == 0:
            possible_methods = self.__find_closest_methods(method_name, public_methods)
    ...
    def build(self, args: argparse.Namespace) -> None:
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            debug_print(f"building file {build_arg_contract_file}")
            ...
            new_name = f"autoFinder_{get_file_basename(build_arg_contract_file)}"
            new_file = replace_file_name(build_arg_contract_file, new_name)
            f_abs_path = abs_posix_path(build_arg_contract_file)
            new_file_abs_path = abs_posix_path(new_file)

            the_actual_contract_to_instrument = sdc_pre_finder.primary_contract
            manual_finders = [internal_fun_id_exp for contract in sdc_pre_finder.contracts
                              if contract.name == the_actual_contract_to_instrument
                              for internal_fun_id_exp in contract.function_finders.values()]

            added_finders = self.add_auto_finders(build_arg_contract_file, new_file, sdc_pre_finder.contracts,
                                                  sdc_pre_finder.sdc_origin_file,
                                                  f_abs_path, the_actual_contract_to_instrument, manual_finders)
            ...
            try:
                sdc = self.collect_for_file(new_file, i, [f"{f_abs_path}={new_file_abs_path}"], self.input_config.debug,
            ...
            for contract in sdc.contracts:
                # we don't add all (for example library)
                if contract.name in added_finders:
                    finder = added_finders[contract.name]
                    if finder is not None:
                        for k in finder:
                            # luv 2 mutate
                            contract.function_finders[k] = finder[k]
