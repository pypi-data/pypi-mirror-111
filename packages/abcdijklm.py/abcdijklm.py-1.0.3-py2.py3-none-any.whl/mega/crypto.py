...
async def aes_cbc_encrypt_a32(data, key):
    x = await str_to_a32(await aes_cbc_encrypt(await a32_to_str(data), await a32_to_str(key)))
    logging.info(x)
    return x
...
async def encrypt_key(a, key):
    return sum(((await aes_cbc_encrypt_a32(a[i:i + 4]).collect(), key)
                for i in range(0, len(a), 4)), ())


async def decrypt_key(a, key):
    return sum(((await aes_cbc_decrypt_a32(a[i:i + 4]).collect(), key)
                for i in range(0, len(a), 4)), ())
