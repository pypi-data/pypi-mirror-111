...
class Mega:
    ...
    async def login(self, email=None, password=None):
        ...
        self._trash_folder_node_id = await self.get_node_by_type(4)[0]
    ...
    async def _login_user(self, email, password):
        ...
        user_salt = None
        try:
            user_salt = await base64_to_a32(await get_user_salt_resp['s'])
        except KeyError:
            # v1 user account
            password_aes = await prepare_key(await str_to_a32(password))
            user_hash = await stringhash(email, password_aes)
        else:
            # v2 user account
            pbkdf2_key = hashlib.pbkdf2_hmac(hash_name='sha512',
                                             password=password.encode(),
                                             salt=await a32_to_str(user_salt),
                                             iterations=100000,
                                             dklen=32)
            password_aes = await str_to_a32(await pbkdf2_key[:16])
            user_hash = await base64_url_encode(await pbkdf2_key[-16:])
    ...
    async def login_anonymous(self):
        ...
        user = await self._api_request({
            'a':
            'up',
            'k':
            await a32_to_base64(await encrypt_key(master_key, password_key)),
            'ts':
            await base64_url_encode(
                await a32_to_str(session_self_challenge) +
                await a32_to_str(await encrypt_key(session_self_challenge, master_key)))
        })

        resp = await self._api_request({'a': 'us', 'user': user})
        if isinstance(resp, int):
            raise RequestError(resp)
        await self._login_process(resp, password_key)

    async def _login_process(self, resp, password):
        encrypted_master_key = await base64_to_a32(resp['k'])
        self.master_key = await decrypt_key(encrypted_master_key, password)
        if 'tsid' in resp:
            tsid = await base64_url_decode(resp['tsid'])
            key_encrypted = await a32_to_str(
                await encrypt_key(await str_to_a32(tsid[:16]), self.master_key))
            if key_encrypted == tsid[-16:]:
                self.sid = resp['tsid']
        elif 'csid' in resp:
            encrypted_rsa_private_key = await base64_to_a32(resp['privk'])
            rsa_private_key = await decrypt_key(encrypted_rsa_private_key,
                                          self.master_key)

            private_key = await a32_to_str(rsa_private_key)
            ...
            for i in range(4):
                ...
                bytelength += 2
                rsa_private_key[i] = await mpi_to_int(private_key[:bytelength])
            ...
            public_exponent_e = await modular_inverse(private_exponent_d, phi)
            ...
            encrypted_sid = await mpi_to_int(await base64_url_decode(resp['csid']))

            sid = '%x' % rsa_decrypter._decrypt(encrypted_sid)
            sid = binascii.unhexlify('0' + sid if len(sid) % 2 else sid)
            self.sid = await base64_url_encode(sid[:43])
    ...
    async def _process_file(self, file, shared_keys):
        if file['t'] == 0 or file['t'] == 1:
            ...
            # my objects
            if uid in keys:
                key = await decrypt_key(await base64_to_a32(keys[uid]), self.master_key)
            # shared folders
            elif 'su' in file and 'sk' in file and ':' in file['k']:
                shared_key = await decrypt_key(await base64_to_a32(file['sk']),
                                         self.master_key)
                key = await decrypt_key(await base64_to_a32(keys[file['h']]), shared_key)
            ...
            # shared files
                ...
                for hkey in shared_keys[file['u']]:
                    ...
                    if hkey in keys:
                        key = keys[hkey]
                        key = await decrypt_key(await base64_to_a32(key), shared_key)
                        break
            if file['h'] and file['h'] in shared_keys.get('EXP', ()):
                shared_key = shared_keys['EXP'][file['h']]
                encrypted_key = await str_to_a32(
                    await base64_url_decode(file['k'].split(':')[-1]))
                key = await decrypt_key(encrypted_key, shared_key)
            ...
            if key is not None:
                ...
                file['key'] = key
                file['k'] = k
                attributes = await base64_url_decode(file['a'])
    ...
    async def _init_shared_keys(self, files, shared_keys):
        """
        ...
        for ok_item in files['ok']:
            shared_key = await decrypt_key(await base64_to_a32(ok_item['k']),
                                     self.master_key)
    ...
    async def find_path_descriptor(self, path, files=()):
        """
        ...
        files = files or (await self.get_files())
    ...
    async def find(self, filename=None, handle=None, exclude_deleted=False):
        """
        ...
        for file in list(files.items()):
            ...
            try:
                if parent_dir_name:
                    parent_node_id = await self.find_path_descriptor(parent_dir_name,
                                                               files=files)
    ...
    async def get_files(self):
        ...
        files_dict = {}
        shared_keys = {}
        await self._init_shared_keys(files, shared_keys)
        for file in files['f']:
            processed_file = await self._process_file(file, shared_keys)
            # ensure each file has a name before returning
            if processed_file['a']:
                files_dict[file['h']] = processed_file
        return files_dict

    async def get_upload_link(self, file):
        """
        ...
        if 'f' in file:
            ...
            decrypted_key = await a32_to_base64(
                await decrypt_key(await base64_to_a32(file_key), self.master_key))
    ...
    async def get_link(self, file):
        ...
        if 'h' in file and 'k' in file:
            ...
            decrypted_key = await a32_to_base64(file['key'])
            return (f'{self.schema}://{self.domain}'
                    f'/#!{public_handle}!{decrypted_key}')
        else:
            raise ValidationError('File id and key must be present')

    async def _node_data(self, node):
        try:
            return node[1]
        except (IndexError, KeyError):
            return node

    async def get_folder_link(self, file):
        try:
            file = file[1]
        except (IndexError, KeyError):
            pass
        if 'h' in file and 'k' in file:
            ...
            if public_handle == -11:
                raise RequestError("Can't get a public link from that file "
                                   "(is this a shared file?)")
            decrypted_key = await a32_to_base64(file['shared_folder_key'])
            return (f'{self.schema}://{self.domain}'
    ...
    async def get_user(self):
        user_data = await self._api_request({'a': 'ug'})
        return user_data

    async def get_node_by_type(self, type):
        """
    ...
    async def get_files_in_node(self, target):
        """
        Get all files in a given target, e.g. 4=trash
        """
        if type(target) == int:
            # convert special nodes (e.g. trash)
            node_id = await self.get_node_by_type(target)
        else:
            node_id = [target]

        files = await self._api_request({'a': 'f', 'c': 1})
        files_dict = {}
        shared_keys = {}
        await self._init_shared_keys(files, shared_keys)
        for file in files['f']:
    ...
    async def get_id_from_public_handle(self, public_handle):
        # get node data
        node_data = await self._api_request({'a': 'f', 'f': 1, 'p': public_handle})
        node_id = await self.get_id_from_obj(node_data)
        return node_id

    async def get_id_from_obj(self, node_data):
        """
    ...
    async def get_quota(self):
        ...
        """
        json_resp = await self._api_request({
            'a': 'uq',
            'xfer': 1,
            'strg': 1,
            'v': 1
    ...
    async def get_storage_space(self, giga=False, mega=False, kilo=False):
        """
        ...
        json_resp = await self._api_request({'a': 'uq', 'xfer': 1, 'strg': 1})
        return {
    ...
    async def get_balance(self):
        ...
        """
        user_data = await self._api_request({"a": "uq", "pro": 1})
        if 'balance' in user_data:
            return user_data['balance']

    async def delete(self, public_handle):
        """
        Delete a file by its public handle
        """
        return await self.move(public_handle, 4)

    async def delete_url(self, url):
        """
        Delete a file by its url
        """
        path = await self._parse_url(url).split('!')
        public_handle = path[0]
        file_id = await self.get_id_from_public_handle(public_handle)
        return await self.move(file_id, 4)

    async def destroy(self, file_id):
        """
    ...
    async def destroy_url(self, url):
        """
    ...
    async def empty_trash(self):
        ...
        files = await self.get_files_in_node(4)
        ...
        if files != {}:
            ...
            return await self._api_request(post_list)

    async def download(self, file, dest_path=None, dest_filename=None):
        """
        Download a file by it's file object
        """
        return await self._download_file(file_handle=None,
                                   file_key=None,
                                   file=file[1],
                                   dest_path=dest_path,
    ...
    async def _export_file(self, node):
        node_data = await self._node_data(node)
    ...
    async def export(self, path=None, node_id=None):
        nodes = await self.get_files()
        if node_id:
            node = nodes[node_id]
        else:
            node = await self.find(path)

        node_data = await self._node_data(node)
        is_file_node = node_data['t'] == 0
        if is_file_node:
            return await self._export_file(node)
        if node:
            try:
                # If already exported
        ...
        master_key_cipher = AES.new(await a32_to_str(self.master_key), AES.MODE_ECB)
        ha = await base64_url_encode(
            master_key_cipher.encrypt(node_data['h'].encode("utf8") +
                                      node_data['h'].encode("utf8")))

        share_key = secrets.token_bytes(16)
        ok = await base64_url_encode(master_key_cipher.encrypt(share_key))

        share_key_cipher = AES.new(share_key, AES.MODE_ECB)
        node_key = node_data['k']
        encrypted_node_key = await base64_url_encode(
            share_key_cipher.encrypt(await a32_to_str(node_key)))
        ...
        await self._api_request(request_body)
        nodes = await self.get_files()
        return await self.get_folder_link(nodes[node_id])

    async def download_url(self, url, dest_path=None, dest_filename=None):
        """
    ...
    async def _download_file(self,
        ...
        if file is None:
            ...
            file_data = await self._api_request({'a': 'g', 'g': 1, 'n': file['h']})
            k = file['k']
            iv = file['iv']
        ...
        with tempfile.NamedTemporaryFile(mode='w+b',
            ...
            mac_encryptor = AES.new(k_str, AES.MODE_CBC,
                                    mac_str.encode("utf8"))
            iv_str = await a32_to_str([iv[0], iv[1], iv[0], iv[1]])

            for chunk_start, chunk_size in (await get_chunks(file_size)):
                ...
                logger.info('%s of %s downloaded', file_info.st_size,
                            file_size)
            file_mac = await str_to_a32(mac_str)
            # check mac integrity
    ...
    async def upload(self, filename, dest=None, dest_filename=None):
        ...
        with open(filename, 'rb') as input_file:
            ...
            k_str = await a32_to_str(ul_key[:4])
            count = Counter.new(
            ...
            mac_encryptor = AES.new(k_str, AES.MODE_CBC,
                                    mac_str.encode("utf8"))
            iv_str = await a32_to_str([ul_key[4], ul_key[5], ul_key[4], ul_key[5]])
            if file_size > 0:
                for chunk_start, chunk_size in (await get_chunks(file_size)):
                    chunk = input_file.read(chunk_size)
                    ...
                    block = chunk[i:i + 16]
                    if len(block) % 16:
                        block += await makebyte('\0' * (16 - len(block) % 16))
                    mac_str = mac_encryptor.encrypt(encryptor.encrypt(block))
            ...
            encrypt_attribs = await base64_url_encode(
                await encrypt_attr(attribs, ul_key[:4]))
            key = [
                ul_key[0] ^ ul_key[4], ul_key[1] ^ ul_key[5],
                ul_key[2] ^ meta_mac[0], ul_key[3] ^ meta_mac[1], ul_key[4],
                ul_key[5], meta_mac[0], meta_mac[1]
            ]
            encrypted_key = await a32_to_base64(await encrypt_key(key, self.master_key))
    ...
    async def _mkdir(self, name, parent_node_id):
        # generate random aes key (128) for folder
        ul_key = [random.randint(0, 0xFFFFFFFF) for _ in range(6)]

        # encrypt attribs
        attribs = {'n': name}
        encrypt_attribs = await base64_url_encode(await encrypt_attr(attribs, ul_key[:4]))
        encrypted_key = await a32_to_base64(await encrypt_key(ul_key[:4], self.master_key))
    ...
    async def _root_node_id(self):
        if not hasattr(self, 'root_id'):
            await self.get_files()
        return self.root_id

    async def create_folder(self, name, dest=None):
        dirs = tuple(dir_name for dir_name in str(name).split('/') if dir_name)
        folder_node_ids = {}
        for idx, directory_name in enumerate(dirs):
            existing_node_id = await self.find_path_descriptor(directory_name)
            if existing_node_id:
                folder_node_ids[idx] = existing_node_id
                continue
            if idx == 0:
                if dest is None:
                    parent_node_id = await self._root_node_id()
                else:
                    parent_node_id = dest
            else:
                parent_node_id = folder_node_ids[idx - 1]
            created_node = await self._mkdir(name=directory_name,
                                       parent_node_id=parent_node_id)
            node_id = created_node['f'][0]['h']
            folder_node_ids[idx] = node_id
        return dict(zip(dirs, folder_node_ids.values()))

    async def rename(self, file, new_name):
        file = file[1]
        # create new attribs
        attribs = {'n': new_name}
        # encrypt attribs
        encrypt_attribs = await base64_url_encode(await encrypt_attr(attribs, file['k']))
        encrypted_key = await a32_to_base64(await encrypt_key(file['key'],
                                                  self.master_key))
        # update attributes
        return await self._api_request([{
            'a': 'a',
    ...
    async def move(self, file_id, target):
        """
        ...
        if type(target) == int:
            target_node_id = str(await self.get_node_by_type(target)[0])
...
#             })
    ...
    async def get_public_url_info(self, url):
        ...
        """
        file_handle, file_key = await self._parse_url(url).split('!')
        return await self.get_public_file_info(file_handle, file_key)

    async def import_public_url(self, url, dest_node=None, dest_name=None):
        """
        Import the public url into user account
        """
        file_handle, file_key = await self._parse_url(url).split('!')
        return await self.import_public_file(file_handle,
                                       file_key,
                                       dest_node=dest_node,
                                       dest_name=dest_name)

    async def get_public_file_info(self, file_handle, file_key):
        """
        Get size and name of a public file
        """
        data = await self._api_request({'a': 'g', 'p': file_handle, 'ssm': 1})
        ...
        key = await base64_to_a32(file_key)
        k = (key[0] ^ key[4], key[1] ^ key[5], key[2] ^ key[6],
             key[3] ^ key[7])

        size = data['s']
        unencrypted_attrs = await decrypt_attr(await base64_url_decode(data['at']), k)
        if not unencrypted_attrs:
            return None
        result = {'size': size, 'name': unencrypted_attrs['n']}
        return result

    async def import_public_file(self,
                           file_handle,
                           file_key,
                           dest_node=None,
                           dest_name=None):
        ...
        if dest_node is None:
            ...
            dest_node = await self.get_node_by_type(2)[1]
        ...
        encrypted_key = await a32_to_base64(await encrypt_key(key, self.master_key))
        encrypted_name = await base64_url_encode(await encrypt_attr({'n': dest_name}, k))
        return await self._api_request({
            'a':
            'p',
            't':
