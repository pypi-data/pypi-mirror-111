...
class BertTokenTaggerDetokenizer:
    ...
    @staticmethod
    def detokenize_line(spacy_detok, offsets, labels):
    ...
    def _bert_batch_tokenize(self, lines):
    ...
    def _get_sentence_input(self, tokenized_sent_ids, tokenized_sent_attention_mask):
    ...
    def _resolve_sentence_pred(self, sent, input_ids, offsets, label_indices):

        # join potentially split predictions
        if len(label_indices) > 1:
            label_indices = self.merge_labels(label_indices)[: len(input_ids)]
        else:
        ...
        result = self.detokenize_line(sent, offsets, labels)
    ...
    def predict(self, sent):
        ...
        chunked_ids, chunked_attention = self._get_sentence_input(
            tokenized_sent_ids, tokenized_sent_attention_mask
        )

        # get labels
        input_ids = torch.tensor(chunked_ids, device=self.device)
        attention_mask = torch.tensor(chunked_attention, device=self.device)
        ...
        result = self._resolve_sentence_pred(
            ...
            tokenized_sentence.offset_mapping,
            label_indices,
        )
    ...
    def _get_lines(self, input_f, if_strip=False):
        count = 0
        batch = []
    ...
    def _get_batch(self, input_f, if_strip=False):
        ...
        if_next, sentences_batch = self._get_lines(input_f, if_strip=if_strip)
        tokenized_sentences_batch = self._bert_batch_tokenize(sentences_batch)
        ...
        for i in range(len(sentences_batch)):
            ...
            chunked_ids, chunked_attention = self._get_sentence_input(
                tokenized_sentences_batch.input_ids[i],
                tokenized_sentences_batch.attention_mask[i],
            )

            chunks_count = len(chunked_ids)

            # remember how to cut batch into sentences
            batch_split.append([count, count + chunks_count])
        ...
        with open(input_path, "r", encoding="utf8") as f_in, open(
            ...
            with tqdm(total=total) as pbar:
                while if_next:
                    ...
                    # get labels
                    input_ids = torch.tensor(chunked_ids_batch, device=self.device)
                    attention_mask = torch.tensor(
                        chunked_attention_batch, device=self.device
                    )
                    ...
                    for i, (start, end) in enumerate(batch_split):
                        sentence_offsets = tokenized_sentences_batch.offset_mapping[i]
                        sentence_input_ids = tokenized_sentences_batch.input_ids[i]
