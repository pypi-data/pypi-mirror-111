...
from .. import HNDLR, LOGS, SUDOHNDLR, udB, ultroid_bot
    ...
    def decorator(func):
        ...
        if pattern is not None:
            if pattern.startswith(r"\#"):
                ...
                args["pattern"] = re.compile(SUDOHNDLR + pattern)

        async def sudowrapper(ult):
            if ult.out:
                return
            return await wrapper(ult)

        if udB.get("SUDO_HNDLR") != udB.get("HNDLR"):
            ultroid_bot.add_event_handler(sudowrapper, events.NewMessage(**args))
