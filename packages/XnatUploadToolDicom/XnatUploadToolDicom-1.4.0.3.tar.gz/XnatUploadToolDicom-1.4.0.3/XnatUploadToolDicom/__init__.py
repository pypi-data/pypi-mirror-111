...
class XnatUploadToolDicom:
    ...
    def buildWorkflowXml(self, project, experiment_id, modality, pipeline_name, status, justification):
        self.logger.info("Generating workflow xml for %s" % experiment_id)
        ...
        attributes = {
            'ExternalID': project, 'ID': experiment_id,
            'data_type': modality, 'type': "WEB_SERVICE",
    ...
    def uploadWorkflowXml(self, xml):
        ...
        self.httpsess.put(url, data=xml, headers=headers)

    def getExperimentMetaData(self, project, subject, session):
        url = "%s/data/archive/projects/%s/subjects/%s/experiments/%s?format=json" % (self.host, project, subject, session)
        sessionjson = json.loads(self.httpsess.get(url).text)
        return sessionjson["items"][0]["data_fields"]["ID"], sessionjson["items"][0]["meta"]["xsi:type"]
    ...
    def createWorkflowEntry(self, project, subject, session, workflow, status, justification):
        try:
            ...
            experimentid, xsitype = self.getExperimentMetaData(project, subject, session)
            self.logger.info("Using XNAT experiment Id: %s" % experimentid)
            self.logger.info("Using XNAT modality: %s" % xsitype)
