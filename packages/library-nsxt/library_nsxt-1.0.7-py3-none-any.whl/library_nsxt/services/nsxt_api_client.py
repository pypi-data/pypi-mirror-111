...
_PATH_LOAD_BALANCER_VIRTUAL_SERVERS_STATISTICS = "/loadbalancer/services/%s/virtual-servers/statistics%s"
...
class NSXTAPIClient:
    ...
    # FIREWALL METHODS
    # ------------------------------------------------------------------------------------------
    def get_nsx_groups(self, idgroup=None, info=False):
        """
        Get all info of all groups or a single group if 'idgroup' parameter is filled in JSON format
        :param idgroup: grup-id to retrive information
        :param info: flag True / False, output the info about all groups
        :return: write in loggin system all group's info (or the id-group info if this parameter is passed). In JSON format
        """
        url = ""
        if not idgroup:
            url += URL_LIST_GROUPS
        else:
            url += URL_GET_GROUP % idgroup
    ...
    def check_if_nsgroup_exist(self, group_name):
        ...
        """
        status, content = self.get_nsx_groups()
    ...
    def get_firewallsection_rules(self, id_section, identificator=None, info=None):
        ...
        :return: status code , Json with info
        """
        url = URL_LIST_RULES
        if id_section:
    ...
    def get_id_from_firewall_inventory_section(self, section_name):
        ...
        """
        result, content = self.get_firewall_inventory_sections()
        idcontent = ""
        if result == 200:
            json_content = json.loads(content)
            sources_list = json_content["results"]
    ...
    def get_transport_nodes(self):
        ...
        :return: List of dictionaries with all nodes of NSX.
        :rtype: list
        """
        edge_nodes = self.session.get(_PATH_TRANSPORT_NODES, False)["results"]
        return edge_nodes
    ...
    def get_load_balancer_virtual_servers_statistics(self, lb_service_id):
        """
        Returns the statistics list of virtual servers in given load balancer service.
        :param str lb_service_id: The load balancer service id.
        :return: List of dictionaries with all virtual servers statistics of the given load balancer service.
        :rtype: list
        """
        lb_virtual_servers_stats = self.session.get(_PATH_LOAD_BALANCER_VIRTUAL_SERVERS_STATISTICS %(lb_service_id, _SOURCE_REALTIME), False)["results"]
        return lb_virtual_servers_stats

    def get_node_load_balancers(self, display_name):
        """
        Returns all load balancers of the specific transport node and all virtual servers on each load balancer with their statistics.
        :param str display_name: Transport node display name.
        :return: List of dictionaries with all load balancers and their virtual servers with their statistics.
        :rtype: list
        """
        logical_routers = self.get_node_logical_routers(display_name)
        ...
        for lb in load_balancer_services:
            for lr in logical_routers:
                if lb["attachment"]["target_id"] == lr["logical_router_id"]:
                    ...
                    for virtual_server in lb_status["lb_status"]["virtual_servers"]:
                        if virtual_server["status"] == "UP":
                            vs_num_up += 1
                        for virtual_server_debug_info in self.get_load_balancers_virtual_servers_debug_info(lb["id"])["virtual_servers"]:
                            if virtual_server["virtual_server_id"] == virtual_server_debug_info["id"]:
                            ...
                            for virtual_server_statistics in self.get_load_balancer_virtual_servers_statistics(lb["id"]):
                                if  virtual_server["virtual_server_id"] == virtual_server_statistics["virtual_server_id"]:
                                    virtual_server["statistics"] = virtual_server_statistics["statistics"]
    ...
    def get_load_balancers_virtual_servers_debug_info(self, lb_service_id):
        """
        .
        :param str lb_service_id: The load balancer service id.
        ...
        :rtype: list
        """
        lb_and_vs_debug_info = self.session.get(_PATH_LOAD_BALANCER_VIRTUAL_SERVERS_DEBUG_INFO %lb_service_id, False)
        return lb_and_vs_debug_info
