...
def bytesto(bytes, to, bsize=1024):
    """
    Converts bytes to megabytes, etc.
    sample code:
...
class DSMConnection():
    ...
    def get_snapshots(self):
        ...
        stdout = self._get_json(response)
        for i in range(len(stdout)):
            self.replays[stdout[i]['globalIndex']] = {}
            self.replays[stdout[i]['globalIndex']]['instanceId'] = stdout[i]['instanceId']
            self.replays[stdout[i]['globalIndex']]['instanceName'] = stdout[i]['instanceName']
            self.replays[stdout[i]['globalIndex']]['expires'] = stdout[i]['expires']
            self.replays[stdout[i]['globalIndex']]['scName'] = stdout[i]['scName']
            self.replays[stdout[i]['globalIndex']]['freezeTime'] = stdout[i]['freezeTime']
            self.replays[stdout[i]['globalIndex']]['expireTime'] = stdout[i]['expireTime']
            self.replays[stdout[i]['globalIndex']]['source'] = stdout[i]['source']
            self.replays[stdout[i]['globalIndex']]['description'] = stdout[i]['description']
            self.replays[stdout[i]['globalIndex']]['size'] = \
                str(bytesto(stdout[i]['size'].replace('Bytes', ''), 'g')) + ' GB'

            if stdout[i].get('parent') is not None:
                self.replays[stdout[i]['globalIndex']]['parent'] = {}
                self.replays[stdout[i]['globalIndex']]['parent']['instanceName'] = \
                    stdout[i]['parent']['instanceName']
                self.replays[stdout[i]['globalIndex']]['parent']['instanceId'] = \
                    stdout[i]['parent']['instanceId']

            if stdout[i].get('replayProfile') is not None:
                self.replays[stdout[i]['globalIndex']]['replayProfile'] = {}
                self.replays[stdout[i]['globalIndex']]['replayProfile']['instanceName'] = \
                    stdout[i]['replayProfile']['instanceName']

            if stdout[i].get('createVolume') is not None:
                self.replays[stdout[i]['globalIndex']]['createVolume'] = {}
                self.replays[stdout[i]['globalIndex']]['createVolume']['instanceName'] = \
                    stdout[i]['createVolume']['instanceName']
                self.replays[stdout[i]['globalIndex']]['createVolume']['instanceId'] = \
    ...
    def run_list_object(self, object_type):
        ...
        """
        function_switcher = {
            'scs': (self.get_scs, 'scs'),
            'servers': (self.get_servers, 'servers'),
            'volumes': (self.get_volumes, 'volumes'),
            'snapshots': (self.get_snapshots, 'replays'),
            'volFolders': (self.get_volume_folders, 'volume_folders'),
            'snapshotProfiles': (self.get_snapshot_profiles, 'rply_profiles'),
            'recycledVols': (self.get_recycled_volumes, 'recycled_vols')
        }
