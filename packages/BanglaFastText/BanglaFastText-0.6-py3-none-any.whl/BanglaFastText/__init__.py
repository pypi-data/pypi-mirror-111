...
class BanglaFasttext:
    ...
    def sent_embd(self,corpus):
        ...
        for sent in tqdm(corpus):
            ...
            embd = [self.div_norm(self.model.wv[i]) for i in sent.split()]
            emd_corpus.append(list(np.mean(embd, axis=0)))
        return np.asarray(emd_corpus)


    def word_similarity(self, word1, word2):
        return cosine_similarity(self.model.wv[word1].reshape(1, -1), self.model.wv[word2].reshape(1, -1))[0]
    def sent_similarity(self, sent1, sent2):
        sent1=self.sent_embd(sent1)
        sent2=self.sent_embd(sent2)

        return cosine_similarity(self.model.wv[sent1].reshape(1, -1), self.model.wv[sent2].reshape(1, -1))[0]
