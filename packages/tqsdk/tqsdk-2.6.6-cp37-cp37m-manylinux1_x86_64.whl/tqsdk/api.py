...
from tqsdk.trade_extension import TradeExtension
...
class TqApi(object):
    ...
    def query_atm_options(self, underlying_symbol, underlying_price, price_level, option_class, exercise_year: int = None,
        ...
        if self._stock is False or self._loop.is_running():
            raise Exception("不支持（_stock is False 或者在协程中）当前接口调用")
        query_result = self._query_options_by_underlying(underlying_symbol)
        options, option_0_index = self._get_options_sorted(query_result, underlying_price, option_class, exercise_year, exercise_month, has_A)
        rst_options = []
    ...
    def _get_options_sorted(self, query_result, underlying_price, option_class, exercise_year, exercise_month, has_A):
    ...
    def _setup_connection(self):
        ...
        trade_extension = TradeExtension(self)
        trade_extension_send_chan = TqChan(self, chan_name="send to trade_extension")
        trade_extension_recv_chan = TqChan(self, chan_name="recv from trade_extension")
        self.create_task(trade_extension._run(trade_extension_send_chan, trade_extension_recv_chan, self._send_chan, self._recv_chan))
        self._send_chan, self._recv_chan = trade_extension_send_chan, trade_extension_recv_chan
