...
class ElasticsearchFunctions:
    ...
    def create_index_pattern(self, dest_es_url, index_patter_name, namespace, date_field):
        ...
        if response.status_code == 200:
            ...
            print(f"error creating index {index_patter_name} on {dest_es_url}", sys.stderr)
            return False
    ...
    def get_object_id(self, kib_url, namespace, type_str, name):
        ...
        if response.status_code != 200:
            print(f"{response.status_code}, {self.config.ES_USER}, {self.config.ES_PASSWORD}, {url}", sys.stderr)
            return None
