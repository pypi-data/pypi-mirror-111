...
def creat_axs(nplot,map_extend,ncol = None,height  = None,width = None,dpi = 300,sup_title = None,sup_fontsize = 12,
    ...
    for p in range(nplot):
        ...
        if len(ax_index)>1:
            ...
            ix = slon + 0.001*(elon - slon) * sup_fontsize / rect1[2]
            iy = elat - 0.006*(elat - slat) * sup_fontsize / rect1[3]

            plt.text(ix, iy, ax_index[p], bbox=dict(fc='white', ec='white'),zorder=100)
...
def add_contourf(ax,grd,cmap ="rainbow",clevs= None,add_colorbar = True):
    x = grd['lon'].values
    y = grd['lat'].values
