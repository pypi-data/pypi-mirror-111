...
import datetime
from .cognito_utils import perform_infinstor_login, token_renewer, check_for_custom_token, read_token_file, get_customerinfo
...
def setup_aws_credentials():
    ...
    if (os.path.exists(credsfile)):
        ...
        try:
            ...
            builtins.log.error(str(err), exc_info=True)
        try:
...
class LoginHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def get(self, path=""):
        ...
        if (check_for_custom_token()):
            ...
            _, _, _, _, service = read_token_file(tokfile)
            builtins.log.info("service =" + service)
            rv = json.dumps({"authenticated": False, "service": service})
            builtins.log.info('LoginHandler.get: not previously logged in. no custom token. ret=' + rv)
            self.finish(rv)
    ...
    def post(self, path=""):
        ...
        try:
            ...
            service = req["infinstorservicename"]
            username = req["infinstorusername"]
            password = req["infinstorpassword"]

            builtins.log.info("LoginHandler.post: Entered. service=" + service
                                + ", username=" + username)

            builtins.service = service
            builtins.clientid = self.get_clientid(service)
    ...
    def setup_vars(self, response_json):
        ...
        builtins.log.info("LoginHandler: Creating new boto3 resource using new endpoint="
    ...
    def post(self, path=""):
        ...
        if 'xform_local_files' in req:
            ...
            if 'xform_local_files_zip_filelist' in xform_local_files:
                ...
                # to send as x-www-form-urlencoded, need to url encode the string, since base64 has '+' and '/' and the former needs to be URL encoded
                payload = payload + "&xform_local_files=" + quote(xform_local_files_jsonstr)
...
class AddPeriodicRunHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def create_mlflow_experiment(self, experiment_name):
        for retry in range(2):
            ...
            if (builtins.idtoken.startswith('Custom')):
                headers['Authorization'] = builtins.idtoken
            else:
...
class RunTransformGraphHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def post(self, path=""):
        ...
        payload = "dagid=" + req["dagid"] + "&dagJson=" + quoted_graph_json
        for retry in range(2):
            headers = {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Authorization': builtins.idtoken
...
class listVMHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def post(self, path=""):
        for retry in range(2):
            ...
            url = 'https://mlflow.' + builtins.service + '/api/2.0/mlflow/projects/list-singlevm'
            try:
                response = requests.get(url, headers=headers)
                resp_json = response.json()
                response.raise_for_status()
...
class getRunStatusHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def post(self, path=""):
        ...
        for retry in range(2):
            ...
            url = 'https://mlflow.' + builtins.service + '/api/2.0/mlflow/runs/getdagexecinfo' + query
            try:
                response = requests.get(url, headers=headers)
                resp_json = response.json()
                response.raise_for_status()
...
class stopVMHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def post(self, path=""):
        ...
        for retry in range(2):
            ...
            url = 'https://mlflow.' + builtins.service + '/api/2.0/mlflow/projects/stop-singlevm'
            try:
                response = requests.post(url, data=payload, headers=headers)
                resp_json = response.json()
                response.raise_for_status()
                ...
                for lineb in response.iter_lines():
                    ...
                    try:
                        ...
                        print("stopVM: Login expired. Retrying")
                        login_expired = True
                if login_expired == True:
                    perform_infinstor_login(builtins.cognito_username, builtins.cognito_password)
                    continue
