        ...
        for header in packet_headers:
            if header['choice'] == 'ethernet':
                ...
                for field in header['arp']:
                    if field not in header_info['arp']:
                        raise SnappiTrexException('\'{}\' is not a supported ARP field'.format(field))
                    field_info = header_info['arp'][field]
                    Validation.validate_value_option(header['arp'][field], 'ARP', field_info['length'])
