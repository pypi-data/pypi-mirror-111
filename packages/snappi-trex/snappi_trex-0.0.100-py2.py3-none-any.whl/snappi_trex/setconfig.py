    ...
    def set_packet_headers(packet_headers):
        ...
        for header in packet_headers:
            ...
            if header['choice'] == 'ethernet':
                pkt_headers.append(Ether()); layers.append('Ethernet')
                layer_cnt['Ethernet'] = layer_cnt['Ethernet']+1 if 'Ethernet' in layer_cnt else 1

            # VLAN HEADER 
            if header['choice'] == 'vlan':
                pkt_headers.append(Dot1Q()); layers.append('Dot1Q')
                layer_cnt['Dot1Q'] = layer_cnt['Dot1Q']+1 if 'Dot1Q' in layer_cnt else 1

            # ARP HEADER 
            elif header['choice'] ==  'arp':
                pkt_headers.append(ARP()); layers.append('ARP')
                layer_cnt['ARP'] = layer_cnt['ARPC']+1 if 'ARP' in layer_cnt else 1
