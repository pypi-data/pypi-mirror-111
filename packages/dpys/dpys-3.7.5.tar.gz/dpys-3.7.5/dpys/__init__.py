    ...
    async def display(ctx, dir):
            ...
            async for entry in cursor:
                ...
                async with db.execute("SELECT role,emoji,channel,msg_id FROM rr WHERE guild = ? and msg_id = ?", (guild,entry[0])) as f:
                    ...
                    async for entry in f:
                        ...
                        msg += f"Emoji: {emoji} Role: {role} Channel: {channel} Message ID: {msg_id}\n"
                        msg_limit += f"Channel: {channel} Message ID: {msg_id}\n"
                        msg_limit_limit += f"Message ID: {msg_id}\n"
                if len(msg) > 1010:
                    if len(msg_limit) > 1010:
                        embed.add_field(name=f"Reaction Role #{number}", inline=False, value=msg_limit_limit)
                    else:
        ...
        if number > 0:
            if limit == "limit":
                ...
                embed.set_footer(text=f"Total Reaction Roles | {number}")
                await ctx.reply("One of your reaction roles went over the Discord limit. It will still work perfectly but only essintial data will be displayed in this command to save space.", delete_after=10)
            await ctx.send(embed=embed, reference= ctx.message.reference or ctx.message)
        else:
