...
class AzureScanner(BaseCloudScanner):
    ...
    def scan(self) -> str:
        ...
        self.account_data_dir = init_directory(settings.output_path, account_name, settings.clean)
        scan_commands = load_yaml(settings.commands_path)
        resource_groups = self._get_resource_groups(headers, subscription_id)
        ...
        for non_dependable_command in non_dependable_commands:
            non_dependable_tasks.append(ThreadedFunctionData(
                ...
                (non_dependable_command, subscription_id, headers, resource_groups),
        ...
        for dependable_command in dependable_commands:
            dependable_tasks.append(ThreadedFunctionData(
                self._execute_scan_commands,
                (dependable_command, subscription_id, headers, resource_groups),
    ...
    def _execute_scan_commands(self, scan_command: dict, subscription_id: str, headers: dict, resource_groups: List[str]) -> None:
        try:
    ...
    def _get_resource_groups(self, headers: dict, subscription_id: str) -> List[str]:
