...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _create_cluster(
        ...
        *,
        ...
        session_id = session.headers.get("x-session-id", get_uuid())
        self._verify_account(account)
        backend_options = {**self._default_backend_options, **(backend_options or {})}
        environ = environ or {}
        logging_context = {
            # Note that in this case the session is the same length
            # as the request.
...
def set_backend_options(
    ...
    registry_type: Literal["ecr", "docker_hub"] = "ecr",
    ...
    registry_type
        ...
        configuration registry_* arguments and specifying registry_type='docker_hub'

    registry_uri
        The container registry URI. Defaults to docker.io

    registry_username
        A registry username (should be lowercased).
    ...
    if use_coiled_defaults is True:
        ...
        if create_vpc is True:
            ...
            if not all(
                ...
                raise NotImplementedError(
                    "Missing GCP parameters for Customer-Hosted Option"
                )
            ...
            if gcp_service_creds_file:
                if not os.path.isfile(gcp_service_creds_file):
                    raise Exception(
                        "param: gcp_service_creds_file must be a valid file"
                    )
                with open(gcp_service_creds_file, "r") as json_file:
                    backend_options["options"]["gcp_service_creds_dict"] = json.load(
                        json_file
                    )
            else:
        ...
        if create_vpc is True:
            raise NotImplementedError("VPC for Azure is not yet implemented")
    ...
    with Cloud(account=account) as cloud:
        return cloud.set_backend_options(
            backend_options,
            account=account,
            log_output=log_output,
        )
