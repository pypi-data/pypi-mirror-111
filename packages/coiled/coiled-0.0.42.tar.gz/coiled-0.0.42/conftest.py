...
def get_options(backend_name):
    if backend_name == "k8s":
        ...
        return options
    elif backend_name == "ecs":
        return {}


if strtobool(environ.get("USE_NEW_BACKEND_TESTS", "n")):
    ...
    @pytest.fixture(
        params=[
            pytest.param(
                "k8s",
                marks=[
                    pytest.mark.test_group("core-k8s"),
                ],
            ),
        ],
    )
...
@pytest.fixture(scope="function")
def use_coiled_proxy(settings):
    settings.PROXY_CLUSTERS_THROUGH_COILED = True
    settings.DEFAULT_PROTOCOL = "wss://"
