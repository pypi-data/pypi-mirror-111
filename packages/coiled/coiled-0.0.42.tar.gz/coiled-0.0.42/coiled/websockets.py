...
class NotificationWebsocketConnector(WebsocketConnectorBase[str]):
    def __init__(
        ...
        self.operation_id = logging_context["operation_id"]
        self.log_output = log_output

    async def on_message(self, message_data: Dict) -> None:
        # This could be there as a None value
        data = message_data.get("data") or {}
        # Ignore notifications that are unrelated to this action
        if (
            not data.get("logging_context", {}).get("operation_id", "")
            == self.operation_id
        ):
            return
...
class WebsocketConnector(WebsocketConnectorBase[str]):
    ...
    async def stream_messages(self):
        try:
            return await aiostream.stream.merge(self, self.notification_connector)
        except StreamEmpty:
            # Sometimes one of the streams can be empty. For instance
            # if you have everything setup correctly (senv, cconf, etc)
            # then the notifications stream will be empty starting up a cluster.
            pass


class JobsWebsocketConnector(WebsocketConnector):
    async def on_message(self, message_data: Dict) -> None:
        if message_data.get("type") == "job_status_changed":
            await self.close()
            return
        await super().on_message(message_data=message_data)
