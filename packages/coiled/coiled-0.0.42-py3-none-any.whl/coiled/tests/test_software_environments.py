...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-slow-group-1")
...
@pytest.mark.asyncio
@pytest.mark.timeout(5 * 60 * 60)  # seconds
@pytest.mark.test_group("software-env-slow-group-2")
...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-slow-group-7")
...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-slow-group-3")
...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-slow-group-4")
...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-test-docker-images")
async def test_docker_images(
    ...
    async with coiled.Cluster(asynchronous=True, configuration="my-config") as cluster:
        async with Client(cluster, asynchronous=True) as client:
            ...
            def test_import():
                try:
                    # Unfortunately disables for the entire module
...
@pytest.mark.asyncio
@pytest.mark.test_group("software-env-slow-group-5")
...
async def test_docker_build_reports_failure(
    ...
    before_envs = await cloud.list_software_environments()
    out = io.StringIO()
    with pytest.raises(ServerError, match="Docker build failed with returncode") as e:
        await cloud.create_software_environment(
            name="env-1",
            conda=["sqlite"],
...
@pytest.mark.test_group("software-env-slow-group-6")
@pytest.mark.asyncio
