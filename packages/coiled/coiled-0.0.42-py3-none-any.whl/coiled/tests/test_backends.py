...
async def test_create_cluster_with_account_in_config(
    ...
    async with Client(cluster, asynchronous=True) as client:
        await client.wait_for_workers(10, timeout=4 * 60)
...
@pytest.mark.asyncio
@pytest.mark.xfail(reason="Flaky: https://github.com/coiled/cloud/issues/2936")
async def test_notifications(
    cloud, cleanup, dhl_cluster_config, cluster_with_create_time
):
    """

    NOTE: we need a way to filter out notifications from a given cluster
    creation event otherwise we'll get noise here.
    """
    cluster, create_time = cluster_with_create_time
    notifications = await cloud.get_notifications(json=True)
    assert len(notifications) > 0
    since_create = [
        notification
        for notification in notifications
        if notification["timestamp"] > create_time
    ]
    assert len(since_create) > 0
    msgs = [notification["msg"] for notification in notifications]
    event_types = [notification["event_type"] for notification in notifications]
    assert "create_scheduler" in event_types
    assert "create_worker" in event_types
    async with Client(cluster, asynchronous=True) as client:
        await client.wait_for_workers(4, timeout=4 * 60)
    notifications = await cloud.get_notifications(json=True)
    since_create = [
    ...
    msgs = [notification["msg"] for notification in notifications]
    event_types = [notification["event_type"] for notification in notifications]
    assert len([msg for msg in msgs if "Successfully pulled docker" in msg]) > 0
