...
async def __fetch_img_data(session: ClientSession, headers, tile_data, image_tiles) -> None:
...
@backoff.on_exception(backoff.expo, aiohttp.web.HTTPServerError, max_time=60)
async def __fetch_json(session: ClientSession, headers, url: str) -> dict:
...
async def __calculate_image_tiles(session: ClientSession, url: str, headers: dict, crop_x: int = None, crop_y: int = None,
    ...
    info = await __fetch_json(session, headers, info_json_url)
...
async def __download_manager(url: str, headers: dict, x: int = None, y: int = None, width: int = None, height: int = None) -> ndarray:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tiles, _, _, _, corr_width, corr_height = await __calculate_image_tiles(session, url, headers, x, y, width, height)
        ...
        for image_details in tiles:
            tasks.append(__fetch_img_data(session, headers, image_details, image_tiles))
...
def iiif_image_from_url(url: str, headers: dict={}, x: int = None, y: int = None, width: int = None, height: int = None) -> ndarray:
    ...
    return loop.run_until_complete(__download_manager(url, headers, x, y, width, height))
