...
from lino.core.actions import (ShowEmptyTable, ShowDetail, QuickLinkAction,
...
class Renderer(JsRenderer, JsCacheRenderer):
    ...
    def py2js_converter(self, v):
        ...
        if isinstance(v, Action):
            ...
            # href_target = v.get_a_href_target()
            # if href_target:
            #     result.update(href=href_target)

            if isinstance(v, QuickLinkAction):
            ...
            # if v.help_text: result['help_text'] = v.help_text
    ...
    def instance_handler(self, ar, obj, ba, **status):
        if ar and hasattr(ar, 'get_status'):
            status.update(ar.get_status())
    ...
    def action_call(self, request, bound_action, status):
        ...
        if isinstance(a, ShowEmptyTable) and an != 'grid':
    ...
    def actions2json(self, actions_list):  # 20210517
        ...
        for ba in actions_list:
            ...
            if (lh or ba.action.window_type):
                # and k not in [
                #     "default_action", "insert_action", "detail_action"]:
                ...
                if ba.action.window_type:
                    ...
                    for a in ba.actor.get_toolbar_actions(
                        ...
                        if a.action.combo_group == combo_group\
                            and combo_group is not None:
                            previous = tbas.pop()
    ...
    def actor2json(self, v):
        ...
        if item_layout is not None:
            # print("20210629", v, "item_layout is", item_layout)
            result.update(item_layout=item_layout._formpanel_name)
        # mt + slave-tables
