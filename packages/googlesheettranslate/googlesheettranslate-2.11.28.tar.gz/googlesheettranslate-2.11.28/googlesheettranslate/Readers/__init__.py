...
from Readers.Err import TransformError
from Readers.transformers.Android import Android
from Readers.transformers.Jsi18n import Jsi18n
from Readers.transformers.PyLan import PyLan
...
class Reader:
    ...
    def LoopExcel(self, input_file_path: str) -> "Reader":
        ...
        for index, row in df.iterrows():
            # trim line
            raw_key = str(row[self.key]).translate(str.maketrans('', '', ' \n\t\r'))
            raw_value = str(row[self.column])
            ...
            if index > 0:
                self.AppendCacheLine(
                    self.transformerEngine.transformKeyValue(raw_key, raw_value, index == df.size - 1)
                )
