...
/*********************************************************************/
{
	...
	//给p分配一个浮点数（指针）空间 这里的(double *)就是强制类型转换
	...
	// Set the uniform grid spacing only channel x z  uniform
	...
	for (q=4; q<=8; q+=2 ) {
		...
		char fname[80];//长度为80的字符串

		// Stencil size equals q   权重尺寸等于 q
...
/*
 * Computes the barycenter interpolation weights for an arbitrarily spaced mesh.计算插值权重
 */
/********************************************************************/
void baryctrwt( const int q, const double *x, double *w ) 
/********************************************************************/
 ...
 * an equispaced stencil and calls baryctrwt to compute the weights.计算均匀网格插值权重
 */
/********************************************************************/
void baryctrwt_uniform( const int q, const double dx, double *w ) 
/********************************************************************/
