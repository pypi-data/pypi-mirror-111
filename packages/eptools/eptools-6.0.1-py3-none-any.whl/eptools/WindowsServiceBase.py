...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    def connect_to_db(self,retry=0):
        try:
            ...
            if retry < 4:
                ...
                if self.logger:
                    self.logger.error("SQL connection error, ocr", e)
                return False
    ...
    def SvcDoRun(self):
        ...
        if self.setup_db():
            ...
            if self.logger:
                self.logger.debug("failed setup")
            ...
            while self.isrunning:
                if datetime.datetime.now() > checker:
                    ...
                    if datetime.datetime.now().hour == self._daily_:
                        ...
                        try:
                            self.logger.debug("----daily loop service: " + self._svc_name_ + "--" + str(datetime.datetime.now()) + "----")
                            result = str(self.loop())
                            self.write_to_db(status='finish',options=[result,result])
                            self.logger.debug("----end loop service: " + self._svc_name_ + "--" + str(datetime.datetime.now()) + "----")
