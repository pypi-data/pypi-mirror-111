    ...
    try:
        ...
        while True:
            ...
            if (calAction == 'quit'):
                ...
                addOverviewSectionToReportFile(reportFile, listOfTestResults, calAction, failuresFound(listOfFailures))
                reportFile.close()

            # End of Loop
            if 'calibrate' in calAction:
                # if we have passed calibration, move on to verification
                if failuresFound(listOfFailures) != True:
                    calAction = 'verify'
                else:
...
#python -m quarchpy.calibration -mUSB:QTL1999-01-002 -acalibrate -i192.168.1.210 -pC:\\Users\\sboon\\Desktop\\CalTesting
