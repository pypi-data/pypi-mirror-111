...
def saturate_set_column_formula_step(
    ...
    Furthermore, manipulates the incoming formula to try and
    make it valid if it is not. Does not always succede
    """
    ...
    try:
        ...
        # is invalid
        parse_formula(event['new_formula'], column_header, throw_errors=True)
    except:
        event['new_formula'] = _get_fixed_invalid_formula(event['new_formula'], column_header)
...
def execute_set_column_formula_step(
    ...
    new_type = get_mito_type(new_step['dfs'][sheet_index][column_header])
