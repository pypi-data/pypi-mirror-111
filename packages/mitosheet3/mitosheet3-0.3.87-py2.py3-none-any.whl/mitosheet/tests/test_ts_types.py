...
from mitosheet.steps.pivot import PIVOT_AGGREGATION_TYPES
from mitosheet.steps.sort import ASCENDING, DESCENDING
from mitosheet.steps.filter import BOOLEAN_FILTER_CONDITIONS, FC_EMPTY, FC_NOT_EMPTY, STRING_FILTER_CONDITIONS, NUMBER_FILTER_CONDITIONS, DATETIME_FILTER_CONDITIONS
...
def test_column_types_match():
    column_mito_type = get_enum_from_ts_file('./src/components/Mito.tsx', 'ColumnMitoType')
...
def test_date_type_in_mito_match():
    data_type_in_mito = get_enum_from_ts_file('./src/components/Mito.tsx', 'DataTypeInMito')
...
def test_sort_direction_match():
    sort_direcion = get_enum_from_ts_file('./src/components/taskpanes/ControlPanel/SortCard.tsx', 'SortDirection')
...
def test_filter_conditions_match():
    boolean_filter_conditions = get_enum_from_ts_file('./src/components/taskpanes/ControlPanel/filter/filterTypes.tsx', 'BooleanFilterCondition')
    ...
    number_filter_conditions = get_enum_from_ts_file('./src/components/taskpanes/ControlPanel/filter/filterTypes.tsx', 'NumberFilterCondition')
    ...
    string_filter_conditions = get_enum_from_ts_file('./src/components/taskpanes/ControlPanel/filter/filterTypes.tsx', 'StringFilterCondition')
    ...
    datetime_filter_conditions = get_enum_from_ts_file('./src/components/taskpanes/ControlPanel/filter/filterTypes.tsx', 'DatetimeFilterCondition')
...
def test_step_types_match():
    step_types = get_enum_from_ts_file('./src/types/StepTypes.ts', 'StepType')
    assert set(step_types.values()) == set([step_object['step_type'] for step_object in STEPS] + ['initialize'])
