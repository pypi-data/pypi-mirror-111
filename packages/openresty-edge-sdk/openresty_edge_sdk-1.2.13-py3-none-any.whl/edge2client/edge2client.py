...
class Edge2Client(object):
    ...
    def search_app(self, app_domain=None, upstream_ip=None,
                   upstream_domain=None, page=None, pagesize=None):

        if not app_domain and not upstream_ip:
            raise Exception('no app domain or upstram ip arg specified')

        if app_domain and upstream_ip:
            raise Exception('only one of app domain or '
                            'upstram ip arg specified')
        ...
        if app_domain:
            ...
            url = 'search/upstream?domain=' + upstream_domain
    ...
    def put_app_user(self, id=None, name=None, read=True,
        ...
        body = {'uid': uid, 'read': read, 'write': write, 'release': release}
    ...
    def put_global_upstream(self, up_id=None, http_verb='PUT', name=None,
                            servers=None, health_checker=None, ssl=False, gid=None):
        if http_verb == 'PUT' and not up_id:
            raise Exception('no active upstream selected')
