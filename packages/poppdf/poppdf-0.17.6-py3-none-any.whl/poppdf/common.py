...
from kdmt.geometry import Point, Rectangle
...
def parse_pages(root, load_page_nums=None, require_image=False, select_image='first'):
    """
    ...
    for p in root.findall('page'):  # parse all pages
        ...
        for im in p.findall('image'):
            im_dict = create_image_dict(im)
            try:
                Rectangle(im_dict['topleft'], im_dict['bottomright'])
            except ValueError:
                # seems like there are rectangles with zero area
                continue  # -> skip them
            page['images'].append(im_dict)

        # create the page dict structure


        for f in p.findall('fontspec'):
            fnt=create_font_dict(f)
            fonts[fnt["id"]]=fnt
        # add the text boxes to the page
        for t in p.findall('text'):
            t_dict = create_text_dict(t)
            t_dict["is_bold"] = False
            b= t.findall('b')
            if len(b)>0:
                t_dict["is_bold"] = True

            try:
                Rectangle(t_dict['topleft'], t_dict['bottomright'])
                if t_dict["top"] <= 0 or t_dict["left"] <=0:
                    raise ValueError
            ...
            t_dict['value'] = ' '.join(t.itertext())
            t_dict['value']=normalize(t_dict['value'])
            if t_dict['value'].strip() =="":
                continue
            if t_dict['font'] in fonts:
                t_dict['font']=fonts[t_dict['font']]
            else:
                print(t_dict)
            page['texts'].append(t_dict)

        page['text']=sorted(page['texts'], key=lambda t: (int(t["top"]), int(t["left"])))
...
def create_font_dict(f):
    """
    From an XML element <f>, create a font dict structure and return it.
    """
    t_color = f.attrib['color']
    t_family = f.attrib['family']
    t_id = int(float(f.attrib['id']))
    t_size=int(float(f.attrib['size']))
...
def create_image_dict(im):
    """
    From an XML element <f>, create a font dict structure and return it.
    """
    t_top = int(float(im.attrib['top']))
    t_left = int(float(im.attrib['left']))
    t_width = int(float(im.attrib['width']))
    t_right=int(float(im.attrib['height']))
    ...
    update_object_dict_pos(image, Point(int(float(im.attrib['left'])), int(float(im.attrib['top']))))
    return image
...
def create_text_dict(t, value=None):
    ...
    t_font = int(float(t.attrib['font']))
    ...
    update_object_dict_pos(text, Point(int(float(t.attrib['left'])), int(float(t.attrib['top']))))
...
def update_object_dict_pos(t, pos=None, update_node=False):
    """
    ...
    t_bottom = t["bottom"] if "bottom" in t else t_top + t['height']
    t_right = t["right"] if "right" in t else t_left + t['width']

    t_top=int(t_top)
    t_bottom=int(t_bottom)
    t_right=int(t_right)
    t_left=int(t_left)
