"""
    poppler is a light wrapper for the poppler-utils tools that can convert your
...
from poppdf.common import read_xml
import pytesseract
from poppdf.alto_xml import process_alto_xml
from PIL import Image
from poppdf.alto import parse
import cv2
import numpy as np
...
def image_from_path(
    ...
    for _ in range(thread_count):
        ...
        output_files.append(thread_output_file)
    images = []
...
def xml_from_path(
    ...
    for _ in range(thread_count):
        ...
        args = [_get_command_path("pdftohtml", poppler_path)] + args
        ...
        ouput_files.append(thread_output_file)
    tree=[]
    i=0
    for uid, proc in processes:
        try:
        ...
        tr, root=read_xml(os.path.join(output_folder,ouput_files[i]+'.xml'))
        ...
        if len(tree) == 1:
            ...
            root__xml = None
            for data in tree:
                for page in data.iter('page'):
                    if root__xml is None:
                        root__xml = data
                        insertion_point = root__xml.findall("./page")[0]
                    else:
                        insertion_point.extend(page)

    return tree

def xml_ocr_from_image(
    page_image,
    laguages='eng',
    size=None,
    options = "--psm 3",
    ...
    page_image = cv2.cvtColor(np.asarray(page_image), cv2.COLOR_BGR2GRAY)

    if thresholding=='adaptive':
        page_image = cv2.adaptiveThreshold(page_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)

    elif thresholding=="simple":
        _, page_image=cv2.threshold(page_image, 170, 255, cv2.THRESH_BINARY, cv2.THRESH_OTSU)
    alto_xml = pytesseract.image_to_alto_xml(page_image, config=options, lang=laguages)
    alto=parse(alto_xml, size[0] / page_image.shape[1],
                                          size[1] / page_image.shape[0])
...
def text_from_path(
    ...
    for _ in range(thread_count):
        ...
        args = [_get_command_path("pdftotext", poppler_path)] + args
    ...
    for uid, proc in processes:
        ...
        with open(os.path.join(output_folder,output_files[i]), "r") as f:
                string+=f.read()
        i+=1
    shutil.rmtree(output_folder)
    return string

def image_info_from_path(
    pdf_path, image_root, userpw=None, poppler_path=None, timeout=None,  first_page=None, last_page=None, output_format='png'
):
    try:
        command = [_get_command_path("pdfimages", poppler_path), pdf_path, image_root]
        ...
        elif output_format in ["png", "jp2", "tiff"]:
        ...
        d =[]
        keys=[]
        for i, field in enumerate(out.decode("utf8", "ignore").split("\n")):
            if i==0:
                keys = [f for f in field.split(" ") if f !=""]
                continue
            if i==1:
                continue
            values = [f for f in field.split(" ") if f !=""]
            if values:
...
def _build_command(
    ...
    if fmt=="":
        pass
    elif fmt not in ["pgm", "ppm"]:
        args.append("-" + fmt)

    elif fmt in ["jpeg", "jpg"] and jpegopt:
    ...
    if complex:
        args.append("-c")
    if do_not_merge_paragraphs:
        args.append("-nomerge")

    if userpw is not None and userpw:
