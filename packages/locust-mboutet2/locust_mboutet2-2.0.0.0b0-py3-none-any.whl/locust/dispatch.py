...
class UsersDispatcher(Iterator):
    ...
    def __init__(self, worker_nodes: "List[WorkerNode]", user_classes_count: Dict[str, int], spawn_rate: float):
        ...
        self._user_count_per_dispatch = max(1, math.floor(self._spawn_rate)) * 1

        self._wait_between_dispatch = self._user_count_per_dispatch / self._spawn_rate

        # aggregated_users_left_to_assigned = {
        #     user_class: sum(
        #         user_classes_count[user_class] for user_classes_count in self._users_left_to_assigned.values()
        #     )
    ...
    def _users_to_dispatch_for_current_iteration(self) -> Dict[str, Dict[str, int]]:
        ...
        # print(time.perf_counter() - t0_rel)
    ...
    def _try_next_worker_in_order_to_stay_balanced_during_ramp_up(
        ...
        ideal_worker_node_ids = list(
            map(
                itemgetter(0),
                filter(lambda x: lt(sub(add(x[1], 1), min_workers_user_count), 2), workers_user_count.items()),
            )
        )
    ...
    def _workers_user_count(self) -> Dict[str, int]:
        ...
        # a = map(sum, map(dict.values, self._dispatched_users.values()))
        # b = dict(zip(self._dispatched_users.keys(), map(sum, map(dict.values, self._dispatched_users.values()))))
        return dict(zip(self._dispatched_users.keys(), map(sum, map(dict.values, self._dispatched_users.values()))))
    ...
    def _worker_is_full(self, worker_node_id: str) -> bool:
        ...
        result = self._worker_is_full_cache.get(worker_node_id, self._workers_user_count[worker_node_id] >= self._workers_desired_user_count[worker_node_id])
