...
def get_json_tree(request):
    ...
    if request.method == 'POST':
        ...
        models = YSYangModels.get_instance(user)
        if ((not models) or
                models.modelnames != sorted(names) or
                models.ctx.repository != ys or
                models.ctx.repository.is_stale or
                not all(
                    [isinstance(m, YSGnmiYangtree)
                     for m in models.yangs.values()]
                )):
...
def build_get_request(request):
    ...
    try:
        request_str = ''
        jsondata['platform'] = devprofile.gnmi.platform
...
def show_replay(request):
    ...
    try:
        ...
        return JsonResponse({'gnmi_replay': gnmi_replay})
