...
class ImportsFinder:
    ...
    def find(self, src):
        """Find imports in source code src.

        Uses the tokenize module instead of ast in previous Brython version,
        so that this script can be run with CPython versions older than the
        one implemented in Brython.
        """
        imports = set()
        importing = None
...
class ModulesFinder:
    ...
    def norm_indent(self, script):
        """Scripts in Brython page may start with an indent.

        remove it before building the AST.
        """
        indent = None
        lines = []
    ...
    def inspect(self):
        """Walk the directory to find all pages with Brython scripts.

        parse them to get the list of modules needed to make them run.
        """
        ...
        for dirname, dirnames, filenames in os.walk(self.directory):
            ...
            for filename in filenames:
                ...
                if ext.lower() == '.html':
                    ...
                    with open(path, encoding=charset_detector.encoding) \
                            as fobj:
    ...
    def make_brython_modules(self, path):
        """Build brython_modules.js from the list of modules needed the app."""
    ...
    def make_setup(self):
        """Make the setup script (setup.py).

        and the entry point script for the application.
        """
...
def load_stdlib_sitepackages(search_path=None):
    """Search brython_stdlib.js.

    and load it Load site-packages from the current directory
    :return:
...
def is_package(folder):
    """Test if folder is a package.

    ie has __init__.py and all the folders above until os.getcwd() also have
    __init__.py.
            ...
            if dirname == os.getcwd():
                ...
                with open(path, encoding="utf-8") as fobj:
                    try:
                        ...
                        logger.error("Unable to read %s: %s", path, ex)
