...
class Graph(object):  # pylint: disable=too-many-public-methods
    ...
    * edges_hdf: hana_ml.DataFrame of the edges
    ...
    def describe(self) -> pd.Series:
        ...
        Descriptive statistics include degree, density, counts (edges, vertices, self loops)
        ...
        """.format(source_col=self.edge_source_column,
                   target_col=self.edge_target_column,
                   schema=self.edge_tbl_schema,
                   edges=self.edge_tbl_name)

        degree_df = self.connection_context.sql(degree_sql).collect()
        ...
        """.format(e_schema=self.edge_tbl_schema,
                   edges=self.edge_tbl_name,
                   v_schema=self.vertex_tbl_schema,
                   vertices=self.vertex_tbl_name)

        density_df = self.connection_context.sql(density_sql).collect()

        # Self Loops
        selfloops_sql = """
            SELECT count(*) AS num_self_loops
            FROM "{schema}"."{edges}"
            WHERE "{source_col}" = "{target_col}"
        """.format(schema=self.edge_tbl_schema,
                   edges=self.edge_tbl_name,
                   source_col=self.edge_source_column,
                   target_col=self.edge_target_column)

        selfloops_df = self.connection_context.sql(selfloops_sql).collect()
        ...
        return (pd.Series(desc)
                .append(degree_df.iloc[0])
                .append(density_df.iloc[0])
                .append(selfloops_df.iloc[0]))
