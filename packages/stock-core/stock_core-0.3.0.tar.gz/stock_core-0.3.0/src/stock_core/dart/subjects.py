...
from stock_core.dart import save_db
...
def 유통주식계산(code: str, date: str) -> float:
    ...
    corp = load_db.Corps(code=code, col='c101')
...
def make_dart_dict(dart_tuple) -> dict:
    ...
    try:
        c101 = load_db.Corps(code=dart_tuple.stock_code, col='c101').get_recent()[0]
...
class DartSubject:
    ...
    def correcting(self):
        ...
        if price <= low52 * 0.8:  # 최근 주가가 바닥이면....
            self.text += f'\n최근 주가({price}원)이 52주최저({low52}원)보다 20% 저렴.'
            correct_rate = 2
        elif low52 * 0.8 <= price <= low52 * 1.2:
...
class 공급계약체결(DartSubject):
    ...
    def scoring(self):
        ...
        def check_past_contract() -> list:
            ...
            df = save_db.MakeDF(code=self.intro['code'],
...
class 현물배당결정(DartSubject):
    ...
    def process(self):
        ...
        """
        super().process()
        self.data['배당성향'] = load_db.Corps(code=self.intro['code'], col='c104y').find_c104(period='y', title='현금배당성향(%)')
...
class 자산재평가실시결정(DartSubject):
    ...
    def process(self):
        ...
        c103_자본총계y = load_db.Corps(code=self.intro['code'], col='c103재무상태표y').find_c103(page='재무상태표y', title='자본총계')
        if self.echo:
            pprint.pprint(self.data)
...
class 매출액또는손익구조(DartSubject):
    ...
    def scoring(self) -> (int, str):
        ...
        c103손익계산서q = load_db.Corps(code=self.intro['code'], col='c103손익계산서q')
