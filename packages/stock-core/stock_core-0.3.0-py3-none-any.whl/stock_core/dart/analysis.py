...
import time
from stock_core.dart import subjects, save_db
...
class AnalyseOneSubject:
    ...
    def notify_and_write_record(dart_dict: dict, cls: subjects.DartSubject):
        ...
        data = {'code': dart_dict['code'],
                ...
                'point': cls.point,
                'text': cls.text}
        save_noti_record = save_db.SaveNotiRecord(data=data)
        if 'sqlite' in save_noti_record.setting.contents['activated_db']:
            save_noti_record.to_sqlite()
        if 'mongo' in save_noti_record.setting.contents['activated_db']:
            save_noti_record.to_mongo()
    ...
    def distribute_to_corps_db(dart_dict: dict, cls: subjects.DartSubject, is_noti: bool):
        ...
        data = {'code': dart_dict['code'],
                ...
                'text': cls.text,
                'is_noti': is_noti}
        save_to_each_corps = save_db.SaveToEachCorps(data=data)
        if 'sqlite' in save_to_each_corps.setting.contents['activated_db']:
            save_to_each_corps.to_sqlite()
        if 'mongo' in save_to_each_corps.setting.contents['activated_db']:
            save_to_each_corps.to_mongo()
