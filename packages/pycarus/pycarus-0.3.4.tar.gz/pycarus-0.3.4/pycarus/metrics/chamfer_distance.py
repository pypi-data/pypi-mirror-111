...
from pycarus.geometry.pcd import batchify


def chamfer(pcd_src: torch.Tensor, pcd_trg: torch.Tensor) -> float:
    ...
    Args:
        pcd_src: The source point cloud(s) with shape ([B,] NUM_POINTS, 3).
        pcd_trg: The target point cloud(s) with shape ([B,] NUM_POINTS, 3).

    Returns:
        The Chamfer Distance using the squared euclidean distance.
    """
    use_cuda = pcd_src.device != torch.device("cpu")

    _, [pcd_src, pcd_trg] = batchify([pcd_src, pcd_trg], 3)

    distance = 0.0
    if use_cuda:
        not_chamfer_cuda = find_spec("chamfer_gpu") is None
    ...
    distance = float(torch.mean(dist_x_y) + torch.mean(dist_y_x))
