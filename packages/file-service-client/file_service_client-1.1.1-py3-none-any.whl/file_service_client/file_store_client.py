...
from .models import FileStoreSchema
...
class FileStoreServiceClient(E360ServiceClient):
    ...
    def create(self, file_path: pathlib.Path, **kwargs: Any) -> FileStoreSchema:
        """
        ...
        with file_path.open('rb') as file_handle:
            ...
            response = self.client.post('', files=file_upload, headers=headers, raises=True, **kwargs)
            response_json = response.json()
            return FileStoreSchema.Schema(many=True).load(response_json)[0]

    def get_by_id(self, *args: Any, **kwargs: Any) -> None:  # pragma: no cover
        """This client does not support object retrieval"""
        raise NotImplementedError('This method is not supported, use `self.get_file_bytes` instead.')
