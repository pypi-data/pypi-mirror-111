...
class PIMessageProcessor(MessageProcessorBase):
    ...
    MSG_HANDLERS = {
        0x0c501000: {'name': 'temp, voltage', 'sensor': SensorModel.PI48, 'type': SensorDataType.NotApplicable, 'frequency': 10, 'is_extended': True, 'parser': parse_pi_tempvolt},
        0x0c501001: {'name': 'firmware', 'sensor': SensorModel.PI48, 'type': SensorDataType.NotApplicable, 'frequency': 10, 'is_extended': True, 'parser': parse_pi_firmware},
        0x0c501002: {'name': 'device info', 'sensor': SensorModel.PI48, 'type': SensorDataType.NotApplicable, 'frequency': 10, 'is_extended': True, 'parser': parse_pi_device},
        0x0c501003: {'name': 'protocol info', 'sensor': SensorModel.PI48, 'type': SensorDataType.NotApplicable, 'frequency': 10, 'is_extended': True, 'parser': parse_pi_proto},
        0x0c501010: {'name': 'gyro', 'sensor': SensorModel.PI48, 'type': SensorDataType.Gyroscope, 'frequency': 1, 'is_extended': True, 'parser': parse_pi_fixpoint_fraction9},
        0x0c501011: {'name': 'acc', 'sensor': SensorModel.PI48, 'type': SensorDataType.Accelerometer, 'frequency': 1, 'is_extended': True, 'parser': parse_pi_fixpoint_fraction11},
    }
   
    def process_message(self, message: CanMessage, data_collection: CanMessageCollection) -> ProcessedCanDataItem:
        hdlr = self.MSG_HANDLERS[message.msg_id]
        parser = hdlr['parser']
        if parser:
            data = parser(message.data)
            return ProcessedCanDataItem(0, hdlr['sensor'], hdlr['type'], data)
        return '', []
