...
from gbdmigration.utilities import RAW_DOCUMENTS as RAW_DOCUMENTS_PLACE_HOLDER
from gbdmigration.utilities import GBD_DOCUMENTS as GBD_DOCUMENTS_PLACE_HOLDER
from gbdmigration.utilities import IMAGES_BUCKET as IMAGES_BUCKET_PLACE_HOLDER
        ...
        for root, dirs, files in os.walk(os.path.join(temp, folders[0])):
            for f in files:
                if f.endswith('.dynamo'):
                    ...
                    break
        if appnums_path:
            prefix = appnums_path[0][0:4]
        self._parallel_process(appnums_path)
        if prefix:
            msgbus.get_indexing_bus().send_message(self.run_id,
                                                   prefix,
    ...
    def migrate_files(self):
        for f in os.listdir(self.root):
            if f.endswith('.dynamo'):
                continue
            if f.endswith('.json'):
                res = self._do_store(f, GBD_DOCUMENTS, store_subpath=self.st13)
            elif f.endswith('.xml'):
