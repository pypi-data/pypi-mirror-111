...
class Baseline(dict):
    ...
    def __init__(self, payload, *, log_level=logging.INFO, **kwargs):
        ...
        if payload == "":
            ...
            self.source_release_date = self.fixlets[0].source_release_date
            self.modification_time = self.fixlets[0].modication_time
            self.cis_benchmark = self.fixlets[0].benchmark_version
            self.xccdf_benchmark = self.fixlets[0].xccdf_benchmark
