...
def cli_qa_create(obj, microservices):
    QA("create", obj["project"], obj["config"], obj["tokens"], microservices).run()


@qa.command("update", help="Update QA env")
@click.argument("arguments", nargs=-1, required=True)
@click.pass_obj
def cli_qa_update(obj, arguments):
    QA("update", obj["project"], obj["config"], obj["tokens"], arguments).run()
...
def cli_qa_delete(obj, qa_hash):
    QA("delete", obj["project"], obj["config"], obj["tokens"], qa_hash).run()
...
def cli_qa_freeze(obj, qa_hash):
    QA("freeze", obj["project"], obj["config"], obj["tokens"], qa_hash).run()
...
def cli_qa_unfreeze(obj, qa_hash):
    QA("unfreeze", obj["project"], obj["config"], obj["tokens"], qa_hash).run()
...
def cli_qa_describe(obj, qa_hash, json):
    flags = {"qa_hash": qa_hash, "json": json}
